...
function applyCustomBackground(item = null) {
    ...
    if (customCode && customCode.trim() !== "") {
        // Если фон тот же самый, что и сейчас, ничего не делаем
        if (iframe.srcdoc === customCode && iframe.style.display === 'block') {
            return;
        }

        // Прячем фон перед сменой контента, чтобы не было "скачка"
        iframe.classList.remove('is-visible');

        // Устанавливаем обработчик события 'load'. Он сработает,
        // когда контент внутри iframe будет готов.
        iframe.onload = () => {
            // Плавно показываем фон
            iframe.classList.add('is-visible');
            // Очищаем обработчик, чтобы он не сработал повторно
            iframe.onload = null;
        };

        // Включаем iframe и передаем ему код. Это запускает загрузку.
        iframe.style.display = 'block';
        iframe.srcdoc = customCode;

    } else {
        // Если фона быть не должно, плавно его убираем
        iframe.classList.remove('is-visible');
    }
}
// ======================================================
