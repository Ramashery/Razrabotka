...
(function () {
  ...
  // ─── ГЕОЛОКАЦИЯ ────────────────────────────────────────────────────────────
  // Шаг 1: запрашиваем бэкенд (он знает IP пользователя и пишет гео в Firestore).
  // Шаг 2: если бэкенд недоступен (Render спит или ошибка) — делаем fallback:
  //   спрашиваем ip-api.com напрямую из браузера и сами пишем в analytics_geo/{sid}.
  function fetchGeo(sessionId) {
    setTimeout(function() {
      if (GEO_ENDPOINT && GEO_ENDPOINT.indexOf('YOUR_BACKEND_URL') === -1) {
        ...
        // Бэкенд не настроен — сразу идём в fallback
        fetchGeoFallback(sessionId);
      }
    }, 2000);
  }

  // Fallback: определяем гео через ip-api.com и пишем в Firestore напрямую из браузера.
  // ip-api.com бесплатен для некоммерческого использования (до 45 req/min).
    ...
    try {
      ...
      xhr.onload = function() {
        ...
        try {
          ...
          writeGeoToFirestore(sessionId, d.country, d.city || '', d.regionName || '');
        } catch(e) {}
      };
      xhr.send();
    } catch(e) {}
  ...
  // Записывает гео в analytics_geo/{sessionId} через Firestore REST API.
  function writeGeoToFirestore(sessionId, country, city, region) {
    try {
      var docUrl = 'https://firestore.googleapis.com/v1/projects/' + PROJECT_ID +
        '/databases/(default)/documents/analytics_geo/' +
        encodeURIComponent(sessionId) + '?key=' + API_KEY;
