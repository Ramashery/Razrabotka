/**
 * DC Analytics Tracker v3.2 — Digital Craft Tbilisi
 *
 * Новое в v3.2:
 *   - Блокировка запуска для поисковых ботов (Googlebot, etc.)
 *     Решает проблему деиндексации из-за CORS-ошибок и XHR-запросов Firestore
 *
 * Новое в v3.1:
...
(function () {
  ...
  // ─── ЗАЩИТА ОТ БОТОВ ───────────────────────────────────────────────────────
  // Поисковые роботы не должны запускать трекер:
  // 1. Их сессии засоряют аналитику фиктивными данными
  // 2. XHR-запросы к Firestore и /geo вызывают CORS-ошибки в Google Search Console
  // 3. Эти ошибки помечают страницы как "проблемные" → деиндексация
  var BOT_PATTERN = /bot|googlebot|bingbot|yandex|baiduspider|crawler|spider|robot|crawling|facebookexternalhit|slurp|duckduckbot|ia_archiver|twitterbot|linkedinbot|whatsapp|applebot|petalbot|semrushbot|ahrefsbot|mj12bot|dotbot/i;
  if (BOT_PATTERN.test(navigator.userAgent)) return;

  // Дополнительная проверка: headless браузеры (используются краулерами)
    ...
    setTimeout(function() {
      ...
      if (GEO_ENDPOINT && GEO_ENDPOINT.indexOf('YOUR_BACKEND_URL') === -1) {
        try {
          ...
          xhr.timeout = 10000;
          xhr.onload = function() {
            // Render в спящем режиме возвращает 200 но с HTML, а не JSON.
            // Проверяем что ответ реально JSON с полем country.
            try {
              var d = JSON.parse(xhr.responseText);
              if (d && d.country) {
                // Бэкенд вернул гео — пишем его прямо в документ сессии
                patchSessionGeo(sessionId, d.country, d.city || '', d.region || d.regionName || '');
                return;
              }
            } catch(e) {}
            // Ответ не JSON или нет country — идём в fallback
  ...
  // Fallback: пробуем несколько бесплатных сервисов по очереди
  var GEO_PROVIDERS = [
    {
      url: 'https://ipapi.co/json/',
      parse: function(d) {
        if (!d || d.error || !d.country_name) return null;
    ...
    },
    {
      ...
      parse: function(d) {
        if (!d || !d.countryName) return null;
        return { country: d.countryName, city: d.cityName || '', region: d.regionName || '' };
      }
    },
    {
      url: 'https://api.country.is/',
      parse: function(d) {
        // Возвращает только country code — используем как последний резерв
        if (!d || !d.country) return null;
        return { country: d.country, city: '', region: '' };
      }
    }
  ];

  function fetchGeoFallback(sessionId) {
    tryGeoProvider(sessionId, 0);
  }

  function tryGeoProvider(sessionId, idx) {
    if (idx >= GEO_PROVIDERS.length) return; // все провайдеры исчерпаны
    var provider = GEO_PROVIDERS[idx];
    try {
      var xhr = new XMLHttpRequest();
      xhr.open('GET', provider.url, true);
      xhr.timeout = 8000;
      xhr.onload = function() {
        if (xhr.status !== 200) { tryGeoProvider(sessionId, idx + 1); return; }
        try {
          var d = JSON.parse(xhr.responseText);
          var geo = provider.parse(d);
          if (!geo) { tryGeoProvider(sessionId, idx + 1); return; }
          patchSessionGeo(sessionId, geo.country, geo.city, geo.region);
        } catch(e) { tryGeoProvider(sessionId, idx + 1); }
      };
      xhr.onerror   = function() { tryGeoProvider(sessionId, idx + 1); };
      xhr.ontimeout = function() { tryGeoProvider(sessionId, idx + 1); };
      xhr.send();
    } catch(e) { tryGeoProvider(sessionId, idx + 1); }
  }
