...
function generateAdminItemFormHTML(item, key) { 
    ...
    <h4>Sitemap & Translation Settings</h4>
    <label>Translation Group Key (одинаковый для всех языковых версий этой страницы)</label>
    <input type="text" class="admin-input-translationGroupKey" value="${item.translationGroupKey || ''}" placeholder="Например: seo-for-restaurants" disabled>
    
    <label>Sitemap: Last Modified Date (оставьте пустым для авто-обновления)</label>
    ...
    <input type="number" step="0.1" min="0.1" max="1.0" class="admin-input-sitemapPriority" value="${item.sitemapPriority || '0.7'}" disabled>
    ...
    <select class="admin-input-sitemapChangefreq" disabled>
        ...
        <option value="daily" ${item.sitemapChangefreq === 'daily' ? 'selected' : ''}>Daily</option>
        ...
        <option value="always" ${item.sitemapChangefreq === 'always' ? 'selected' : ''}>Always</option>
        <option value="hourly" ${item.sitemapChangefreq === 'hourly' ? 'selected' : ''}>Hourly</option>
        <option value="never" ${item.sitemapChangefreq === 'never' ? 'selected' : ''}>Never</option>
    </select>
    ...
    <h4>Social Media Sharing (Open Graph)</h4>
    <label>OG Title (If different from SEO Title)</label><input type="text" class="admin-input-ogTitle" value="${item.ogTitle || ''}" disabled>
    <label>OG Description (If different from Meta Description)</label><textarea class="admin-input-ogDescription" rows="2" disabled>${item.ogDescription || ''}</textarea>
    <label>OG Image URL (1200x630px recommended)</label><input type="text" class="admin-input-ogImage" value="${item.ogImage || ''}" disabled>
