...
const supportedLangs = ['en', 'ka', 'ru', 'uk']; // Define supported languages for dropdowns
...
function renderAdminPanel() {
    ...
    renderAdminCarouselItems('carouselItems'); // NEW: Рендерим секцию для элементов карусели
}
...
function renderAdminCarouselItems(key) {
    ...
    const listsHTML = langOrder.map(lang => {
        ...
        const itemsListHTML = groupedItems[lang]
            ...
            .map(item => {
                const archivedClass = item.status === 'archived' ? 'is-archived' : '';
                return `<li class="admin-list-item ${archivedClass}" data-id="${item.id}" data-key="${key}" data-status="${item.status || 'published'}">${item.title || 'No Title'}<span class="admin-list-item-slug"> (Order: ${item.order || 0})</span></li>`;
            })
            .join('');
...
async function handleAdminActions(e) { 
    ...
    try { 
        ...
        const key = target.dataset.key || (itemEl ? itemEl.dataset.key : null); // Ensure key is retrieved
        ...
        switch(action) { 
            ...
            case 'save': { 
                ...
                if (key === 'carouselItems') { // NEW: Обработка элементов карусели
                    updatedData = {
                ...
                } else { // Существующая логика для других коллекций
            ...
            case 'add': 
                ...
                let newItemData;
                if (key === 'carouselItems') { // NEW: Добавление нового элемента карусели
                    newItemData = {
                        lang: defaultLang,
                        order: siteData[key].length, // Присваиваем следующий порядковый номер
    ...
    document.querySelector('.admin-content').addEventListener('click', e => {
        ...
        if (listItem) {
            ...
            if (itemData) {
                ...
                // NEW: Используем правильную функцию для генерации формы в зависимости от коллекции
