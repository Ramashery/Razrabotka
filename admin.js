...
function generateAdminItemFormHTML(item, key) { 
    ...
    // ЛОГИКА ОТОБРАЖЕНИЯ:
    // Если в базе уже true ИЛИ если язык 'en' - галочка стоит визуально.
    // Обратите внимание: это только визуально. Реальная запись происходит при кнопке Save.
...
function generateAdminCarouselItemFormHTML(item, key) {
    ...
    // --- НОВОЕ: Генерируем опции для автозаполнения статьями ---
    ...
    return `<div class="admin-item" data-id="${item.id}" data-key="${key}" data-status="${item.status || 'published'}">
        <div class="admin-item-content">
            ...
            <div class="admin-autofill-box">
                ...
                <button type="button" class="admin-btn autofill-btn" style="margin-top: 10px;" disabled>Apply Autofill</button>
            </div>
        ...
        switch(action) { 
            ...
            case 'save': { 
                ...
                if (key === 'carouselItems') { 
                    ...
                    // --- КЛЮЧЕВАЯ ЛОГИКА ---
                    // 1. Если язык 'en' -> ВСЕГДА true (независимо от галочки).
                    // 2. Если другой язык -> берем значение галочки.
                    ...
                    updatedData = { 
                        ...
                        isXDefault: finalIsXDefault, // Save result
                ...
                renderAdminPanel(); 
                alert('Item saved! (Note: English items are set to x-default automatically)'); 
                break; 
            } 
            case 'delete': 
            ...
            case 'add': 
                ...
                if (key === 'carouselItems') { 
                    ...
                    // При создании по умолчанию язык 'en', поэтому isXDefault = true
...
function initAdminEventListeners() {
    ...
    // Listen to changes in the admin content area
    document.querySelector('.admin-content').addEventListener('click', e => {
        ...
        // --- НОВЫЙ КОД: ОБРАБОТЧИК КНОПКИ АВТОЗАПОЛНЕНИЯ ---
        if (e.target.classList.contains('autofill-btn')) {
            ...
            // Подставляем значения из dataset выбранного <option>
    ...
    // Дополнительный обработчик: если пользователь меняет язык в форме на 'en', 
    // ставим галочку x-default визуально.
        ...
        if (e.target.classList.contains('admin-input-lang')) {
            ...
            if (checkbox) {
                if (e.target.value === 'en') {
                    ...
                    // Если переключили с EN на другое, галочку лучше снять, если пользователь сам не поставил
