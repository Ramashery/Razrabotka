...
function generateAdminItemFormHTML(item, key) { 
    ...
    // UI logic: If it's English, we visually check it (though the real logic is in Save)
    // or we respect the saved value.
    const isChecked = item.isXDefault ? 'checked' : '';
    ...
    return `<div class="admin-item" data-id="${item.id}" data-key="${key}" data-status="${item.status || 'published'}">
        <div class="admin-item-content">
            ...
            <div style="display: flex; align-items: center; gap: 10px; margin-top: 5px;">
                ...
                <label for="xdefault-${item.id}" style="margin-bottom: 0; cursor: pointer;">Is x-default for this group (SEO). English is auto-default.</label>
            </div>
        ...
        switch(action) { 
            ...
            case 'save': { 
                ...
                if (key === 'carouselItems') { 
                    ...
                    const checkboxChecked = itemEl.querySelector('.admin-input-isXDefault').checked;
                    
                    // ЛОГИКА: Если язык 'en', то isXDefault всегда true. Иначе - как в чекбоксе.
                    const finalIsXDefault = selectedLang === 'en' ? true : checkboxChecked;
                    ...
                    updatedData = { 
                        ...
                        isXDefault: finalIsXDefault, // Применяем логику
            ...
            case 'add': 
                ...
                if (key === 'carouselItems') { 
                    ...
                    // При добавлении язык по умолчанию 'en', поэтому isXDefault ставим true
