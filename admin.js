...
function generateAdminItemFormHTML(item, key) { 
    ...
    <div style="display: flex; gap: 15px; align-items: flex-end; margin-bottom: 10px;">
        ...
        <div style="flex: 1; padding-bottom: 5px;">
            <label style="display: flex; align-items: center; gap: 8px; cursor: pointer; padding: 8px; border: 1px solid var(--color-border); border-radius: 4px; background: rgba(0,0,0,0.1);">
                <input type="checkbox" class="admin-input-isXDefault" ${item.isXDefault ? 'checked' : ''} style="width: auto; margin: 0;" disabled>
                <span style="font-size: 0.85rem;">is x-default</span>
            </label>
        </div>
    </div>
            ...
            case 'save': { 
                ...
                if (key === 'carouselItems') { 
                    ...
                    updatedData = { 
                        ...
                        isXDefault: itemEl.querySelector('.admin-input-isXDefault').checked,
            ...
            case 'add': 
                ...
                if (key === 'carouselItems') { 
                    ...
                    newItemData = { 
                        id: newId, 
                        lang: defaultLang, 
                        isXDefault: defaultLang === 'en', // ПО-УМОЛЧАНИЮ EN СТАНОВИТСЯ X-DEFAULT
                        region: '', 
                        urlSlug: newTitle.toLowerCase().replace(/\s+/g, '-'), 
                        title: newTitle, 
