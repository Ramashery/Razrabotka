...
function renderAdminHome() { 
    ...
    <!-- ПОЛЕ ДАТЫ ДЛЯ ГЛАВНОЙ СТРАНИЦЫ -->
    <label for="home-lastModified" style="color:var(--color-accent); margin-top:15px; border-top:1px solid var(--color-border); padding-top:10px;">Sitemap Last Modified (Оставьте пустым для текущей даты)</label>
...
// ================== ОБНОВЛЕННАЯ ФОРМА ДЛЯ ЭЛЕМЕНТОВ (ВКЛЮЧАЯ РЕГИОН) ==================
...
async function handleAdminActions(e) { 
    ...
    try { 
        ...
        // ================== ОБНОВЛЕННЫЙ БЛОК СОХРАНЕНИЯ HOME ==================
        if (action === 'save-home') { 
            ...
            // Логика сохранения даты для Home
        ...
        switch(action) { 
            ...
            case 'save': { 
                ...
                // ================== ОБНОВЛЕННЫЙ БЛОК СОХРАНЕНИЯ ITEM (С РЕГИОНОМ) ==================
            ...
            case 'add': 
                ...
                const newItemData = { 
                    lang: defaultLang, 
                    region: '', // Пустой регион по умолчанию
                    urlSlug: newSlug, 
                    title: newTitle, 
