...
def get_sessions(days=30, limit=5000):
    """Загружает сессии за последние N дней + джойнит geo из analytics_geo."""
    ...
    # Загружаем гео только для сессий без страны
    no_geo = [s for s in sessions if not s.get('country')]
    if no_geo:
        sids = {s.get('id') for s in no_geo if s.get('id')}
        geo_map = {}
        # Читаем analytics_geo по sid (один документ на сессию — дёшево)
        for sid in sids:
            try:
...
def patch_geo_by_sid(sid, geo, ip):
    """Пишет гео-данные в отдельную коллекцию analytics_geo.
    Не требует поиска сессии — экономит квоту Firestore на чтение.
    Дашборд джойнит данные по sid при рендере."""
    if not sid:
        return
    try:
...
def geo_endpoint():
    ...
    Бэкенд определяет IP → страна → пишет в analytics_geo (1 запись на сессию).
    Дашборд читает analytics_geo и джойнит по sid — без дорогого поиска.
    """
