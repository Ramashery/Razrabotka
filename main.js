...
function scrollToElementWithOffset(elementId, offset = 120) {
    ...
    // Получаем высоту самого элемента
    const elementHeight = element.offsetHeight;
    
    // Рассчитываем позицию так, чтобы элемент оказался в центре экрана
    // Центр экрана = elementTop - (высота окна / 2) + (высота элемента / 2)
...
function handleNavigation(e) {
    ...
    if (targetUrl.pathname === window.location.pathname && targetUrl.hash) {
        setTimeout(() => {
            ...
            // Используем нашу функцию для скролла с offset
