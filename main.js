...
function hydrateStaticPage(siteData) {
    ...
    if (item) {
        ...
        // 1. Применяем динамический фон из админки
        applyCustomBackground(item);
        
        // 2. Обновляем мета-теги (для соцсетей и актуальности)
        updateMetaTags(item);

        // 3. Обновляем главное изображение, если оно есть в HTML
        ...
        if (mainImageEl && item.media && item.media.length > 0) {
            ...
            // Если в HTML заглушка или старая картинка -> меняем на новую
            if (freshImage && mainImageEl.getAttribute('src') !== freshImage) {
                mainImageEl.style.opacity = '0'; // Плавный переход
                setTimeout(() => {
        ...
        // 4. Можно обновить цену, если она есть
        const priceEl = document.querySelector('.detail-price');
        if (priceEl && item.price) {
             priceEl.textContent = item.price;
        }
    } else {
        // Если данные в базе не найдены, но файл статический существует - 
        // просто применяем дефолтный фон главной страницы
...
function initApp() {
    ...
    if (isStaticPage) {
        ...
        loadData().then((freshSiteData) => {
            ...
            // Запускаем гидратацию: подмену фона и картинок
            hydrateStaticPage(freshSiteData);
            
            // Инициализация UI для статического контента
            initFloatingObservers();
            initParagraphObservers(); 
            initMobileSliders();
            initDesktopCarousels();
            
            // Рендер похожих постов, если их нет в статике
            const path = window.location.pathname;
            const detailPageRegex = /^\/(?:([a-z]{2})\/)?(services|portfolio|blog|contact)\/([a-zA-Z0-9-]+)\/?$/;
            const match = path.match(detailPageRegex);
            if (match) {
        ...
        return; // ВАЖНО: Прерываем выполнение, чтобы routeAndRender не очистил страницу
    }
