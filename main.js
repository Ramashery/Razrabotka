...
async function initApp() {
    // Не инициализировать Firebase для поисковых ботов.
    // Googlebot видит XHR-запросы к Firestore как ошибки загрузки ресурсов,
    // а запросы к /geo вызывают CORS-ошибки — это помечает страницы
    // как "просканировано, но не проиндексировано" в Google Search Console.
    // Статический HTML уже содержит весь контент, нужный для индексации.
    const BOT_PATTERN = /bot|googlebot|bingbot|yandex|baiduspider|crawler|spider|robot|crawling|facebookexternalhit|slurp|duckduckbot|ia_archiver|twitterbot|linkedinbot|whatsapp|applebot|petalbot|semrushbot|ahrefsbot|mj12bot|dotbot/i;
    if (BOT_PATTERN.test(navigator.userAgent) || navigator.webdriver) {
        console.log("[initApp] Bot detected. Skipping Firebase initialization.");
        renderMenu();
        document.documentElement.style.setProperty('--main-visibility', 'visible');
        return;
    }
