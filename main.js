...
// --- MENU TOGGLE LOGIC (НОВАЯ ФУНКЦИЯ) ---
...
function initMenuToggle() {
    ...
    const navMenu = document.querySelector('.nav-menu'); // основное меню

    if (!menuToggle || !navOverlay || !navMenu) return;
    ...
    // 1. Обработчик для кнопки меню (существующая логика)
    ...
    // 2. Закрытие при клике на оверлей (существующая логика)
    ...
    // 3. НОВОЕ: Закрытие при клике вне области меню (как в TOC)
    ...
    // 4. НОВОЕ: Закрытие при клике на якорную ссылку в меню
    navMenu.addEventListener('click', (event) => {
        if (event.target.closest('a')) {
            setTimeout(closeMenu, 100); // небольшая задержка для плавности
        }
    });
...
function initStaticEventListeners() { 
    ...
    if (menuToggle && navOverlay) {
        // Используем новую функцию initMenuToggle вместо старого кода
        initMenuToggle();
    }
