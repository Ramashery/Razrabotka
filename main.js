...
// --- НОВАЯ ЛОГИКА ФОРМАТИРОВАНИЯ ДЛЯ КЛИЕНТА ---
    ...
    const html_parts = blocks.map(block => {
        ...
        if (carouselMatch && siteData) {
            ...
            const slidesHtml = carouselItems.map(item => `
                ${item.linkUrl ? `<a href="${item.linkUrl}" class="card-link">` : ''}
                <div class="card">
                    ...
                    <div class="inner">
                        ...
                        ${item.imageUrl ? `<img src="${item.imageUrl}" alt="${item.title}" class="card-image">` : ''}
...
function initializeCarousel() {
    ...
    // ВАЖНО: Ищем по классу carousel-container, а не mzaCarousel
    ...
    if (carouselRoots.length > 0) {
        ...
        console.log(`GlowCarousel: Initialized ${glowCarouselInstances.length} instances.`);
    }
}
