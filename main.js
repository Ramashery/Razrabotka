...
// === ИЗМЕНЕНИЕ №1: Функция стала асинхронной и теперь управляет прокруткой ===
async function routeAndRender(isPopState = false) {
    ...
    if (match) {
        ...
        // Дожидаемся полной загрузки и отрисовки главной страницы
    ...
    // Логика прокрутки перенесена сюда и выполняется ПОСЛЕ отрисовки
    const hash = window.location.hash;
    if (hash) {
        // Используем requestAnimationFrame, чтобы браузер успел "увидеть" новый элемент
...
// === ИЗМЕНЕНИЕ №2: Функция полностью переписана для корректной обработки навигации ===
function handleNavigation(e) {
    ...
    // Если мы уже на нужной странице и просто хотим прокрутить к якорю
    ...
    // Если URL полностью совпадает, просто скроллим вверх
    if (targetUrl.href === window.location.href) {
        window.scrollTo({ top: 0, behavior: 'smooth' });
        return;
    }

    // Логика для перехода на другую страницу (включая главную с якорем)
    setTimeout(() => {
        ...
        setTimeout(() => {
            // Сразу устанавливаем правильный URL
            window.history.pushState({}, '', targetUrl.href);
            mainContentEl.style.transition = 'none';
            
            // Вызываем обновленную routeAndRender, которая сама позаботится о прокрутке
            routeAndRender(); 
            ...
            requestAnimationFrame(() => {
                requestAnimationFrame(() => {
                    ...
                    // Если у ссылки не было хэша, прокручиваем страницу вверх
                    if (!targetUrl.hash) {
