...
// --- CAROUSEL FIX: Глобальная переменная для хранения HTML карусели ---
let ssrCarouselHtml = ''; 

const mainContentEl = document.querySelector('main');
let floatingObserver, animateOnceObserver, animateAlwaysObserver;

// Карта транслитерации для грузинского (Mkhedruli)
...
function formatContentHtml(content) {
    ...
    for (let i = 0; i < html_parts.length; i += GROUP_SIZE) {
        ...
        if (group.length > 0) groupedHtml.push(`<div class="content-group">${group.join('')}</div>`);
    }
...
function renderDetailPage(collection, slug, lang) {
    ...
    // --- CAROUSEL FIX: Вставка сохраненного HTML карусели ---
    if (ssrCarouselHtml) {
        // Создаем временный элемент для парсинга HTML строки
        const tempDiv = document.createElement('div');
        tempDiv.innerHTML = ssrCarouselHtml;
        // Переносим детей в mainContentEl
        while (tempDiv.firstChild) {
            mainContentEl.appendChild(tempDiv.firstChild);
        }
    }

    initializeCarousel(); // Инициализация после вставки в DOM
...
async function navigateToHome(hash = '') {
    try {
        ...
        if (hash) {
            setTimeout(() => { scrollToElementWithOffset(hash.substring(1), 120); }, 100);
        }
    } catch (error) {
...
async function hydrateStaticPage() {
    ...
    // --- CAROUSEL FIX: Захват SSR HTML ---
    const ssrSource = document.getElementById('ssr-carousel-source');
    if (ssrSource) {
        ssrCarouselHtml = ssrSource.innerHTML;
        ssrSource.remove();
    }
