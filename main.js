...
// --- ПЛАВНЫЙ ФЕЙД АНИМАЦИЙ (custom-background-iframe + hero/menu/footer
// уже содержат srcdoc из Python) ---
// ID всех iframe'ов с пользовательскими HTML/JS/CSS анимациями, которые
// можно задать в админ-панели: общий фон + 3 секционные анимации.
const CUSTOM_ANIMATION_IFRAME_IDS = [
    'custom-background-iframe',
    'hero-animation-iframe',
    'menu-animation-iframe',
    'footer-animation-iframe'
];

function initCustomBackgroundFade() {
    CUSTOM_ANIMATION_IFRAME_IDS.forEach(id => {
...
// --- ФОРВАРДИНГ УКАЗАТЕЛЯ В IFRAME'Ы АНИМАЦИЙ ---
// Все кастомные анимационные iframe'ы рисуются с отрицательным z-index, а
// остальной контент лежит поверх них в stacking order. Из-за этого
// pointerdown/pointermove/touchstart над страницей никогда физически не
// долетают до документа внутри iframe — браузер отдаёт их родительскому
// body. Поэтому координаты указателя нужно явно пересылать через
// postMessage, а сама анимация внутри srcdoc должна слушать "message" (см.
// соответствующий обработчик в background_html / heroAnimationHtml /
// menuAnimationHtml / footerAnimationHtml).
function initBackgroundPointerForwarding() {
    const iframes = CUSTOM_ANIMATION_IFRAME_IDS
        .map(id => document.getElementById(id))
        .filter(iframe => iframe && iframe.style.display !== 'none');
