...
function renderDetailPage(collection, slug, lang) {
    ...
    const detailPageHTML = `
        <section>
            <div class="detail-page-header">
                <!-- ИСПРАВЛЕНО: Добавлен класс 'is-visible' для H1 для немедленного отображения -->
    ...
    // ИСПРАВЛЕНО: Применяем классы анимации сразу после вставки HTML
    document.querySelectorAll('.detail-content > .content-group').forEach(el => el.classList.add('floating-item'));
    document.querySelectorAll('.detail-page-header .detail-price').forEach(el => el.classList.add('animate-on-scroll'));
    ...
    // renderRelatedPosts теперь вызывается отдельно, поэтому его классы добавляются ниже
...
function renderRelatedPosts(currentCollection, currentSlug, currentLang) {
    ...
    // ИСПРАВЛЕНО: Добавляем классы анимации для связанных постов сразу после их вставки
    document.querySelectorAll('#related-posts .item-card').forEach(el => el.classList.add('animate-on-scroll'));
    document.querySelectorAll('#related-posts h2').forEach(el => el.classList.add('animate-on-scroll'));
...
async function routeAndRender(isPopState = false, hash = '') {
    ...
    // ИСПРАВЛЕНО: Эти классы теперь добавляются внутри renderDetailPage/hydrateHomePageContent
    // document.querySelectorAll('.detail-content > .content-group').forEach(el => el.classList.add('floating-item'));
    // document.querySelectorAll('.detail-price, #related-posts .item-card').forEach(el => el.classList.add('animate-on-scroll'));
    setupObservers(); // Инициализируем наблюдатели после того, как все классы добавлены
...
function hydrateHomePageContent() {
    ...
    if (preloadedDataEl) {
        try {
            ...
            ['services', 'portfolio', 'blog', 'contact'].forEach(key => {
                const hasFullData = siteData[key] && siteData[key].length > 0 && siteData[key][0].h1;
    ...
    // ИСПРАВЛЕНО: Применяем классы анимации сразу после вставки HTML для главной страницы
        ...
        if (match) {
            ...
            if (currentItem) {
                ...
                if (!document.getElementById('related-posts')) {
                    // Если related-posts еще нет, рендерим и добавляем классы
...
function initStaticEventListeners() { 
    ...
    if (menuToggle && navOverlay) { 
        menuToggle.addEventListener('click', () => { 
            ...
            navOverlay.classList.remove('is-active'); // Ensure overlay closes when menu toggles
        }); 
    }
