...
async function hydrateStaticPage() {
    ...
    // 1. Мгновенно скрываем лоадер, чтобы PageSpeed увидел контент
    const loader = document.getElementById('loader');
    if (loader) loader.classList.add('hidden');
    
    // 2. Сразу рендерим меню и футер
    renderMenu();
    ...
    // 3. "Оживляем" анимации для контента, который уже есть на странице (БЕЗ мигания)
    ...
    // 4. Запускаем наблюдателей для будущих элементов
    setupObservers();

    // 5. Загружаем данные из Firebase в фоне (это больше не блокирует отображение)
    try {
        siteData = await loadData();
        
        // 6. Находим данные для ТЕКУЩЕЙ страницы
        ...
        if (match) {
            ...
            if (currentItem) {
                // 7. Применяем фон (он появится, как только данные загрузятся)
                applyCustomBackground(currentItem);

                // 8. Рендерим блок "Related Posts"
                renderRelatedPosts(collection, slug, itemLang);
                
                // 9. Навешиваем анимацию на новые элементы
...
async function initApp() { 
    ...
    if (isStaticPage) { 
        // Вызываем гидратацию. Она сама разберется с лоадером и фоновой загрузкой.
        hydrateStaticPage();
    } else { 
        // Логика для главной SPA страницы остается прежней
        try { 
            siteData = await loadData(); 
            renderMenu(); 
            routeAndRender(); 
