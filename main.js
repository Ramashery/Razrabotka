...
function renderDetailPage(collection, slug, lang) {
    ...
    if (rawContent.trim().startsWith('[TOC]')) {
        ...
        if (tocItems.length > 0) {
            ...
            tocItems.forEach(item => {
                ...
                tocListHtml += `<li class="${className}"><a href="#${item.slug}" class="toc-link">${item.text}</a></li>`;
            });
            tocListHtml += '</ul>';
    ...
    if (floatingTocWrapper && tocContentPanel && tocToggleBtn) {
        if (tocHtmlContent) {
            ...
            // --- ДОБАВЛЯЕМ ОБРАБОТЧИКИ ДЛЯ ССЫЛОК TOC С OFFSET ---
            setTimeout(() => {
                const tocLinks = tocContentPanel.querySelectorAll('.toc-link');
                tocLinks.forEach(link => {
                    link.addEventListener('click', function(e) {
                        ...
                        if (targetElement) {
                            // Закрываем TOC панель при клике на ссылку
                            toggleBtn.setAttribute('aria-expanded', 'false');
                            contentPanel.setAttribute('aria-hidden', 'true');
                            contentPanel.classList.remove('is-visible');
                            toggleBtn.classList.remove('is-active');
                            
                            // Рассчитываем позицию с учетом offset
                            const elementPosition = targetElement.getBoundingClientRect().top;
                            const offsetPosition = elementPosition + window.pageYOffset - 120; // 120px отступ сверху
                            
                            // Плавный скролл к элементу с offset
                            window.scrollTo({
                                top: offsetPosition,
                                behavior: 'smooth'
                            });
...
function handleNavigation(e) {
    ...
    if (targetUrl.pathname === window.location.pathname && targetUrl.hash) {
        if (link.closest('.toc-content-panel')) {
            // Клик по ссылке в TOC обрабатывается в renderDetailPage
            return;
        }
    }
    ...
    if (targetUrl.pathname === window.location.pathname && targetUrl.hash) {
        setTimeout(() => {
            ...
            if (targetElement) {
                ...
                // Используем offset для якорных ссылок тоже
                const elementPosition = targetElement.getBoundingClientRect().top;
...
function hydrateHomePageContent() {
    ...
    if (hash) {
        requestAnimationFrame(() => {
            ...
            if (targetElement) {
                // Используем offset для якорных ссылок при загрузке страницы
                const elementPosition = targetElement.getBoundingClientRect().top;
                const offsetPosition = elementPosition + window.pageYOffset - 120;
