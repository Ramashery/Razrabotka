...
// renderSection теперь используется для ДИНАМИЧЕСКИ генерируемого контента (например, связанных постов)
// или для адаптации статически сгенерированного контента главной страницы
function renderSection(key, title, items) {
    ...
    if (isMobile) {
        ...
        generatedContentHtml = `<div class="desktop-grid-wrapper">${desktopGridsHTML}</div>`;
    }
    // Здесь мы перезаписываем содержимое section, это нормально для renderSection
    // (например, для связанных постов, которые всегда динамические)
...
function renderDetailPage(collection, slug, lang) {
    ...
    if (floatingTocWrapper && tocContentPanel && tocToggleBtn) {
        ...
        // Если floatingTocWrapper отсутствует, это либо домашняя страница, либо что-то пошло не так.
        // Для домашней страницы он не нужен, для внутренних должен быть в template.html
        if (floatingTocWrapper) floatingTocWrapper.style.display = 'none'; // Убедиться, что скрыт, если контента нет
    }

    renderRelatedPosts(collection, slug, lang); // Эта функция добавит свои элементы и классы
    document.getElementById('site-footer').style.display = 'none';
}

function renderRelatedPosts(currentCollection, currentSlug, currentLang) {
    // В отличие от главной страницы, связанные посты всегда динамически генерируются.
    // Поэтому здесь используется full render (как в renderSection)
...
function hydrateHomePageContent() {
    ...
    if (preloadedDataEl) {
        try {
            ...
            // Заполняем siteData.home данными из прелоадера, если она еще не полна из loadData()
            if (!siteData.home || Object.keys(siteData.home).length === 0 || !siteData.home.h1) {
                 siteData.home = preloadedSections.home || {};
            }
            // Для коллекций: siteData[key] уже должен быть полон после await loadData() в initApp.
            // preloadedSections[key] (урезанные данные) здесь не перезаписывают siteData.
    ...
    sectionsToAdapt.forEach(key => {
        ...
        if (sectionEl) {
            const isMobile = window.matchMedia("(max-width: 767px)").matches;
            const existingGrid = sectionEl.querySelector('.item-grid'); // Статическая сетка
            let mobileSlidersContainer = sectionEl.querySelector('.mobile-sliders-container');
            ...
            if (isMobile) {
                if (existingGrid) {
                    existingGrid.style.display = 'none'; // Скрываем статическую сетку на мобильных
                }
                ...
                if (existingGrid) {
                    existingGrid.style.display = ''; // Показываем статическую сетку
                ...
                if (mobileSlidersContainer) {
                    mobileSlidersContainer.remove(); // Удаляем контейнер мобильных слайдеров, если он был
                }
            }
        }
    });
    ...
    initDesktopCarousels(); // Инициализируем десктопные карусели (если есть, обычно нет на главной в статике)
...
async function hydrateStaticPage() {
    renderMenu();
    // Элементы на статической странице уже могут иметь эти классы из HTML, но гарантируем
    ...
    try {
        siteData = await loadData(); // Сначала загружаем полные данные
        const path = window.location.pathname;
        const detailPageRegex = /^\/(?:([a-z]{2})\/)?(services|portfolio|blog|contact)\/([a-zA-Z0-9-]+)\/?$/;
        const match = path.match(detailPageRegex);
        if (match) {
            ...
            if (currentItem) {
                ...
                // Связанные посты будут отрисованы динамически, их классы добавятся в renderRelatedPosts
            ...
            // Это прямая загрузка главной страницы, убеждаемся, что используются полные данные
