...
function initScrollToTopButton() {
    ...
    button.addEventListener('click', () => {
        // Ищем оглавление. Вы можете использовать любой селектор,
        // например, если у вашего TOC будет класс .table-of-contents
        const tocElement = document.querySelector('#toc, .table-of-contents');

        if (tocElement) {
            // Если оглавление найдено, скроллим к нему
            tocElement.scrollIntoView({ behavior: 'smooth', block: 'start' });
        } else {
            // Иначе скроллим в самый верх страницы
            window.scrollTo({ top: 0, behavior: 'smooth' });
        }
    });
}
...
async function initApp() { 
    ...
    initScrollToTopButton(); // <--- ВЫЗОВ НОВОЙ ФУНКЦИИ
