    ...
    animateOnceObserver = new IntersectionObserver((entries, observer) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                // --- НОВАЯ ЛОГИКА ДЛЯ ИЗОБРАЖЕНИЙ В КОНТЕНТЕ ---
                const lazyImage = target.querySelector('img.lazy-load-image[data-src]');
                if (lazyImage) {
                    lazyImage.onload = () => {
                        lazyImage.classList.add('loaded');
                        lazyImage.onload = null; // Удаляем обработчик после срабатывания
                    };
...
function formatContentHtml(content, siteData = null, lang = 'en') {
    ...
    const html_parts = blocks.map(block => {
        ...
        if (carouselMatch && siteData) {
            ...
            const placeholder_svg = "data:image/svg+xml,%3Csvg%20xmlns='http://www.w3.org/2000/svg'%20viewBox='0%200%201%201'%3E%3C/svg%3E";
            return `<p class="animate-on-scroll" style="margin: 1.5em 0;"><img data-src="${trimmedBlock}" src="${placeholder_svg}" class="lazy-load-image" alt="Embedded content" style="max-width: 100%; height: auto; display: block; margin: 0 auto; border-radius: 4px; border: 1px solid var(--color-border);" /></p>`;
        } else {
    ...
    if (isMobile) {
        const mobileSlidersHTML = langOrder.map(lang => {
            ...
            const slidesHTML = langItems.map((item, index) => {
                ...
                return `<a href="${itemUrl}" class="item-card ${index === 0 ? 'active' : ''}"><div class="item-card__image" 
                role="img" aria-label="${item.mainImageAlt || item.title || ''}"
                data-bg-src="${imageUrl}"></div><div class="item-card__content"><h3>${item.title}</h3><div class="card-subtitle">${item.subtitle}</div><p>${item.description}</p></div></a>`
            }).join('');
        ...
        const desktopGridsHTML = langOrder.map(lang => {
            ...
            const slidesHTML = slides.map((slideItems, index) => {
                const cardsHTML = slideItems.map(item => {
                    ...
                    const imageUrl = (mediaArray.find && mediaArray.find(url => !/youtube|vimeo/.test(url))) || '';
                    // Lazy Load: data-bg-src
                    return `<a href="${itemUrl}" class="item-card"><div class="item-card__image"
                    role="img" aria-label="${item.mainImageAlt || item.title || ''}"
                    data-bg-src="${imageUrl}"></div><div class="item-card__content"><h3>${item.title}</h3><div class="card-subtitle">${item.subtitle}</div><p>${item.description}</p></div></a>`;
                }).join('');
...
function renderRelatedPosts(currentCollection, currentSlug, currentLang) {
    ...
    const itemsHTML = relatedItems.map(item => {
        ...
        const imageUrl = (mediaArray.find && mediaArray.find(url => !/youtube|vimeo/.test(url))) || '';
        // Lazy Load: data-bg-src
        return `<a href="${itemUrl}" class="item-card animate-on-scroll"><div class="item-card__image"
        role="img" aria-label="${item.mainImageAlt || item.title || ''}"
        data-bg-src="${imageUrl}"></div><div class="item-card__content"><h3>${item.title}</h3><div class="card-subtitle">${item.subtitle}</div><p>${item.description}</p></div></a>`
    }).join('');
...
async function navigateToHome(hash = '') {
    ...
    try {
        ...
        // Apply background immediately if siteData.home is already populated (Firebase loaded).
        // If not yet loaded (lazy Firebase still pending), skip here — loadFirebaseWhenIdle().then()
        // will call applyCustomBackground(siteData.home) once data arrives.
        if (siteData.home && siteData.home.backgroundHtml) {
...
function hydrateHomePageContent() {
    ...
    // NOTE: applyCustomBackground(siteData.home) is NOT called here because
    // siteData.home.backgroundHtml is only available after Firebase loads.
    // It is applied in loadFirebaseWhenIdle().then() once Firebase data arrives.
...
async function hydrateStaticPage() {
    ...
    // Firebase is loaded lazily via requestIdleCallback — AFTER the static HTML
    // is fully rendered and visible. This means:
    //   • Googlebot sees complete page content immediately (static HTML is in DOM)
    //   • Firebase XHR connections (Listen/channel) only open once the browser is idle
    //   • Bots that don't support requestIdleCallback simply never execute Firebase,
    //     which is correct — they should index the static HTML content only.
    //
    // The 4000ms timeout in requestIdleCallback is a fallback for real browsers
    // where the idle callback might otherwise be delayed too long (e.g. on slow devices).
    // For real users, Firebase will load quickly and dynamic features (related posts,
    // custom backgrounds, carousels) will enhance the already-visible static content.
    // Firebase is loaded only for real human browsers — NOT for crawlers/bots.
    //
    // WHY: Google's Web Rendering Service (WRS) is headless Chrome and DOES support
    // requestIdleCallback — so deferring via idle callback alone is not enough to
    // prevent Googlebot from opening Firestore XHR connections. The XHR errors
    // (firestore.googleapis.com/Listen/channel) still appear in GSC's resource log
    // because WRS executes the idle callback after the initial render.
    //
    // SOLUTION: Detect known bot User-Agent strings and skip Firebase entirely.
    // Static HTML already contains all indexable content, so bots need nothing
    // from Firebase. Real users get full dynamic features (backgrounds, related posts).
    //
    // This is a standard, Google-approved pattern — serving the same HTML to bots
    // and users, just skipping non-essential JS for crawlers.
    const BOT_UA_PATTERN = /Googlebot|Google-InspectionTool|AdsBot|Mediapartners|bingbot|YandexBot|DuckDuckBot|Baiduspider|Slurp|facebookexternalhit|Twitterbot|rogerbot|linkedinbot|embedly|quora|outbrain|pinterest|slackbot|vkShare|W3C_Validator|lighthouse|HeadlessChrome/i;
    const isBot = BOT_UA_PATTERN.test(navigator.userAgent);

    if (isBot) {
        console.log("[hydrateStaticPage] Bot detected — skipping Firebase entirely. Static HTML will be indexed.");
        siteData = JSON.parse(JSON.stringify(initialSiteData));
    } else {
        // Real browser: load Firebase when idle to avoid blocking Time to Interactive.
        const loadFirebaseWhenIdle = () => {
            try {
                ...
                console.log("[hydrateStaticPage] Firebase init error:", e.message);
                siteData = JSON.parse(JSON.stringify(initialSiteData));
                return;
            }
            ...
            loadData()
                .then(data => {
                    siteData = data;
                    console.log("[hydrateStaticPage] Firebase data loaded (idle).");
                    // Apply dynamic enhancements now that we have live data
                    const match = window.location.pathname.match(/^\/(?:([a-z]{2})\/)?(services|portfolio|blog|contact)\/([a-zA-Z0-9-]+)\/?$/);
                    if (match) {
                        // Detail page: apply custom background and related posts
                        const [, lang, col, slug] = match;
                        ...
                        // Homepage: re-apply custom background now that siteData.home.backgroundHtml
                        // is available from Firebase (it was empty in initialSiteData).
                        applyCustomBackground(siteData.home);
                    }
                })
                .catch(err => {
                    console.log("[hydrateStaticPage] Firebase load failed — using static HTML.", err.message);
                    siteData = JSON.parse(JSON.stringify(initialSiteData));
                });
        };

        if ('requestIdleCallback' in window) {
            requestIdleCallback(loadFirebaseWhenIdle, { timeout: 4000 });
        } else {
            setTimeout(loadFirebaseWhenIdle, 2000);
        }
    }

    // Dynamic enhancements (custom background, related posts) are applied
    // inside loadFirebaseWhenIdle().then() once Firebase data is available.
    // Homepage dynamic content is hydrated here synchronously using preloaded-data
    // embedded in the HTML, which does not require Firebase.
    const isHomepage = !/^\/(?:[a-z]{2}\/)?(services|portfolio|blog|contact)\/[a-zA-Z0-9-]+\/?$/.test(window.location.pathname);
    if (isHomepage) {
        // Homepage: content is rendered from preloaded-data in the HTML (no Firebase needed).
        hydrateHomePageContent();
    }
    // Detail page static HTML already has full content — no action needed here.
    // Dynamic enhancements are applied later in loadFirebaseWhenIdle.
...
async function initApp() {
    // IMPORTANT: Firebase is NOT initialized here for static pages.
    // On pages with data-static-page="true", Firebase init is deferred and
    // executed lazily via requestIdleCallback inside hydrateStaticPage().
    //
    // WHY: Googlebot executes JS fully before indexing. When firebase.initializeApp()
    // and db = firebase.firestore() were called unconditionally here, Firestore
    // immediately opened XHR long-polling connections (Listen/channel) to
    // firestore.googleapis.com — connections that Googlebot cannot complete.
    // This caused the bot to hang for 20+ seconds waiting for Firebase,
    // consuming its rendering budget and leaving detail pages un-indexed.
    // Yandex and Bing were unaffected because they index the static HTML
    // without waiting for JS completion.
    //
    // FIX: Firebase init is deferred to after the static HTML is already
    // visible to the bot. The browser-idle callback fires only once the page
    // is rendered, so bots that don't execute idle callbacks simply skip Firebase
    // entirely — which is fine because the static HTML already contains all content.
    ...
    if (document.body.dataset.staticPage === 'true') { 
        ...
        // On detail pages the static HTML already has full content rendered
        // by the Python generator. We must NOT call routeAndRender here because
        // it would invoke renderDetailPage (which overwrites <main>) or
        // navigateToHome (which replaces content with the homepage).
        // routeAndRender is still used for subsequent SPA navigations via
        // handleNavigation / popstate listeners set up in initStaticEventListeners.
        const isDetailPage = /^\/(?:[a-z]{2}\/)?(services|portfolio|blog|contact)\/[a-zA-Z0-9-]+\/?$/.test(window.location.pathname);
        if (!isDetailPage) {
            // Homepage with data-static-page="true": run the router normally.
            routeAndRender(false, window.location.hash);
        } else {
            // Detail page: just set up observers and visibility — content is already in DOM.
            requestAnimationFrame(() => setupObservers());
            document.documentElement.style.setProperty('--main-visibility', 'visible');
            updateScrollButtonVisibility();
        ...
        // Non-static (legacy SPA) mode: initialize Firebase immediately.
        firebase.initializeApp(firebaseConfig);
        db = firebase.firestore();
