...
function renderRelatedPosts(currentCollection, currentSlug, currentLang) {
    ...
    // ИСПРАВЛЕНИЕ: Используем 6 ссылок
...
async function routeAndRender(isPopState = false) {
    ...
    // Проверка видимости кнопки сразу
...
function handleNavigation(e) {
    ...
    setTimeout(() => {
        // ИСПРАВЛЕНИЕ: Мягкое исчезновение
        ...
        setTimeout(() => {
            ...
            // Скроллим наверх ПОКА контент скрыт
            if (!targetUrl.hash) { window.scrollTo({ top: 0, behavior: 'auto' }); }

            requestAnimationFrame(() => {
                requestAnimationFrame(() => {
                    // ИСПРАВЛЕНИЕ: Мягкое появление снизу
                    mainContentEl.style.transition = 'opacity 0.6s ease-out, transform 0.6s ease-out';
