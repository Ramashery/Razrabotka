...
function renderDetailPage(collection, slug, lang) {
    ...
    // Резервный вариант для H1 (сохранил на всякий случай, но теперь должен работать основной)
    ...
    if (preloadedDataEl) {
        try {
            ...
            ['services', 'portfolio', 'blog', 'contact'].forEach(key => {
                // ПРОВЕРКА: Если у нас уже есть данные в siteData и в них есть поле 'h1' (признак полных данных),
                // то мы НЕ перезаписываем их урезанными данными из preloadedSections (где h1 нет).
