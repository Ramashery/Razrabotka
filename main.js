...
// Прикрепляется один раз глобально, чтобы избежать дублирования слушателей
let floatingTocToggleInitialized = false;
function initFloatingTocToggle() {
    if (floatingTocToggleInitialized) return; // Инициализируем только один раз
    ...
    toggleBtn.addEventListener('click', (event) => {
        event.stopPropagation(); // Предотвращаем закрытие панелью, если клик был по кнопке
    ...
    // Закрываем панель при клике вне ее, но только если она видима
...
function renderDetailPage(collection, slug, lang) {
    ...
    if (!item) {
        ...
        // Убедимся, что плавающий TOC скрыт на 404 странице
        const floatingTocWrapper = document.getElementById('floating-toc-wrapper');
    ...
    let tocHtmlContent = ''; // Это будет внутренний HTML для панели оглавления
    ...
    const tocToggleBtn = document.getElementById('toc-toggle-btn'); // Получаем кнопку
    
    if (floatingTocWrapper && tocContentPanel && tocToggleBtn) {
        if (tocHtmlContent) {
            tocContentPanel.innerHTML = tocHtmlContent; // Заполняем панель содержимым
            floatingTocWrapper.style.display = 'flex'; // Показываем обертку
            // Сбрасываем состояние TOC на закрытое при загрузке новой страницы
            ...
            // Инициализируем переключатель (если еще не инициализирован, то ничего не произойдет)
            initFloatingTocToggle(); 
        } else {
            floatingTocWrapper.style.display = 'none'; // Скрываем обертку, если TOC не нужен
            tocContentPanel.innerHTML = ''; // Очищаем содержимое панели
        }
    }
...
async function navigateToHome() {
    try {
        ...
        // Убедимся, что плавающий TOC скрыт при переходе на главную
        const floatingTocWrapper = document.getElementById('floating-toc-wrapper');
        if (floatingTocWrapper) {
            floatingTocWrapper.style.display = 'none';
            // Также убедимся, что панель закрыта
            const tocContentPanel = document.getElementById('toc-content-panel');
...
function handleNavigation(e) {
    ...
    if (targetUrl.pathname === window.location.pathname && targetUrl.hash) {
        ...
        if (link.closest('.toc-content-panel')) {
    ...
    try {
        ...
        if (match) {
            ...
            if (currentItem) {
                ...
                const floatingTocWrapper = document.getElementById('floating-toc-wrapper');
                if (floatingTocWrapper && floatingTocWrapper.style.display !== 'none') {
                    // Убедимся, что содержимое уже заполнено Jinja
                    initFloatingTocToggle();
                }
            }
        } else {
