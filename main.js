    ...
    html_parts.forEach(part => {
        if (part.includes('carousel-container')) {
            ...
            if (temp_group.length >= GROUP_SIZE) {
                grouped_html.push(`<div class="content-group">${temp_group.join('')}</div>`);
                temp_group = [];
            }
        }
    });
...
function renderSection(key, title, items) {
    ...
    // FIX: Reverted to English names as requested
    ...
    if (isMobile) {
        const mobileSlidersHTML = langOrder.map(lang => {
            ...
            return `<div class="language-slider-block"><h4 class="desktop-lang-title">${langNames[lang] || lang}</h4><div class="cross-fade-slider">${slidesHTML}</div><div class="slider-nav">${dotsHTML}</div></div>`;
        }).join('');
        ...
        const desktopGridsHTML = langOrder.map(lang => {
            ...
            return `<div class="desktop-language-group"><h4 class="desktop-lang-title">${langNames[lang] || lang}</h4><div class="desktop-carousel-container">${slidesHTML}</div>${slides.length > 1 ? `<div class="desktop-slider-nav">${dotsHTML}</div>` : ''}</div>`;
        }).join('');
...
function renderDetailPage(collection, slug, lang) {
    ...
    if (floatingTocWrapper && tocContentPanel && tocToggleBtn) {
        if (tocHtmlContent) {
            ...
            // Сброс состояния при загрузке новой страницы
...
function handleGlobalClick(e) {
    ...
    // 1. MENU TOGGLE LOGIC
    const menuBtn = target.closest('.menu-toggle');
    const overlay = document.querySelector('.nav-overlay');
    const body = document.body;
    const menuBtnEl = document.querySelector('.menu-toggle'); // Actual element
    ...
    if (overlay && target === overlay && body.classList.contains('nav-is-open')) {
    ...
    // 3. TOC TOGGLE LOGIC
    const tocBtn = target.closest('#toc-toggle-btn');
    const tocPanel = document.getElementById('toc-content-panel');
    const tocWrapper = document.getElementById('floating-toc-wrapper');
    const tocBtnEl = document.getElementById('toc-toggle-btn'); // Actual element

    if (tocBtn && tocPanel) {
        e.stopPropagation(); // Stop propagation to avoid immediate closing
        const isExpanded = tocBtnEl.getAttribute('aria-expanded') === 'true';
        tocBtnEl.setAttribute('aria-expanded', !isExpanded);
        tocPanel.setAttribute('aria-hidden', isExpanded);
        tocPanel.classList.toggle('is-visible', !isExpanded);
        tocBtnEl.classList.toggle('is-active', !isExpanded);
        return;
    }

    // 4. CLOSE TOC ON LINK CLICK OR OUTSIDE CLICK
    if (tocPanel && tocPanel.classList.contains('is-visible')) {
        const clickedInsidePanel = tocPanel.contains(target);
        const clickedLink = target.tagName === 'A' && clickedInsidePanel;
        
        // Close if clicked outside OR clicked a link inside
        if (!clickedInsidePanel || clickedLink) {
            tocBtnEl.setAttribute('aria-expanded', 'false');
            tocPanel.setAttribute('aria-hidden', 'true');
            tocPanel.classList.remove('is-visible');
            tocBtnEl.classList.remove('is-active');
        }
    }

    // 5. QUIZ BUTTON
    if (target.id === 'quiz-submit-button') {
        calculateQuizResult();
    }
...
function initStaticEventListeners() { 
    ...
    // Unified Global Click Listener for UI interactions
