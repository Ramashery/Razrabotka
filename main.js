...
// === ФУНКЦИЯ ДЛЯ ПРОКРУТКИ К ЭЛЕМЕНТУ (УЧИТЫВАЯ CSS scroll-margin-top) ===
function scrollAnchorIntoView(elementId, delay = 50) {
    const element = document.getElementById(elementId);
    if (!element) return;

    // Используем setTimeout, чтобы дать браузеру время на рендеринг и применение CSS
    setTimeout(() => {
        element.scrollIntoView({
            behavior: 'smooth',
            block: 'start' // 'start' в сочетании со scroll-margin-top обеспечит правильное позиционирование
        });
    }, delay);
}
...
async function navigateToHome(hash = '') {
    try {
        ...
        if (newMain) {
            ...
            if (hash) {
                // Задержка после полной отрисовки главной страницы
                scrollAnchorIntoView(hash.substring(1), 150); 
            }
        }
    } catch (error) {
...
function handleNavigation(e) {
    ...
    if (targetUrl.pathname === window.location.pathname && targetUrl.hash) {
        setTimeout(() => {
            ...
            // Задержка для якорных ссылок на той же странице
            scrollAnchorIntoView(hash, 50); 
        }, menuCloseDelay);
        return;
    }
...
function hydrateHomePageContent() {
    ...
    if (hash) {
        // Задержка при начальной загрузке главной страницы с хешем
        requestAnimationFrame(() => {
            scrollAnchorIntoView(hash.substring(1), 150); 
        });
    }
