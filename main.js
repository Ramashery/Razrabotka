...
function formatContentHtml(content) {
    ...
    // Функция для "декодирования" HTML. Она превращает &lt; обратно в <
    ...
    const html = blocks.map(block => {
        ...
        // --- ИЗМЕНЕНИЕ 1: Проверяем наличие и обычных, и экранированных тегов ---
        const isAlreadyHtml = /^<(p|div|h[1-6]|ul|ol|li|blockquote|hr|table|pre)/i.test(trimmedBlock);
        const isEscapedHtml = /^&lt;(p|div|h[1-6]|ul|ol|li|blockquote|hr|table|pre)/i.test(trimmedBlock);

        const youtubeMatch = trimmedBlock.match(youtubeRegex);
        const imageMatch = trimmedBlock.match(imageRegex);

        if (isAlreadyHtml) {
            // Если это уже обычный HTML, возвращаем как есть
            return trimmedBlock;
        } else if (isEscapedHtml) {
            // --- ИЗМЕНЕНИЕ 2: Если это экранированный HTML (наш случай!), декодируем его ---
            return decodeHtmlEntities(trimmedBlock);
            ...
            // Если это простой текст, оборачиваем в <p>
