...
async function hydrateStaticPage() {
    ...
    // 2. "Оживляем" анимации для контента, который уже есть на странице (БЕЗ мигания)
    ...
    // 3. Запускаем наблюдателей для будущих элементов
    setupObservers();

    // 4. Загружаем данные из Firebase в фоне
    try {
        siteData = await loadData();
        
        // 5. Находим данные для ТЕКУЩЕЙ страницы
        ...
        if (match) {
            ...
            if (currentItem) {
                ...
                // 7. Рендерим блок "Related Posts"
                renderRelatedPosts(collection, slug, itemLang);
                
                // 8. Навешиваем анимацию на новые элементы (они появятся при скролле)
                ...
                setupObservers(); // Перезапускаем обсерверы для новых элементов
            } else {
...
async function initApp() { firebase.initializeApp(firebaseConfig); db = firebase.firestore(); initStaticEventListeners(); const isStaticPage = document.body.dataset.staticPage === 'true'; if (isStaticPage) { await hydrateStaticPage(); const loader = document.getElementById('loader'); if (loader) loader.classList.add('hidden'); } else { try { siteData = await loadData(); renderMenu(); routeAndRender(); mainContentEl.classList.remove('loading'); const loader = document.getElementById('loader'); if (loader) loader.style.display = 'none'; } catch (error) { console.error("Failed to initialize app:", error); mainContentEl.innerHTML = "<h1>Error loading site data. Please try again later.</h1>"; mainContentEl.classList.remove('loading'); } } }
