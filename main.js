...
// === ОБЩАЯ ФУНКЦИЯ ДЛЯ ПРОКРУТКИ К ЭЛЕМЕНТУ (С ЦЕНТРИРОВАНИЕМ ИЛИ СМЕЩЕНИЕМ) ===
// Эта функция теперь используется для центрирования якорных ссылок
function scrollToElement(elementId, behavior = 'smooth', block = 'center', offset = 0) {
    ...
    if (block === 'center') {
        element.scrollIntoView({
            behavior: behavior,
            block: 'center' // Центрирует элемент в видимой области
        });
    } else { // 'start' (top) with offset
...
async function navigateToHome(hash = '') {
    try {
        ...
        if (newMain) {
            ...
            if (hash) {
                // Используем новую функцию для центрирования якоря
                setTimeout(() => {
                    scrollToElement(hash.substring(1)); // block: 'center' по умолчанию
                }, 100);
            }
        }
    } catch (error) {
...
async function routeAndRender(isPopState = false, hash = '') {
    ...
    // setupObservers теперь вызывается после того, как весь контент с классами анимации находится в DOM
    setupObservers(); 
    document.documentElement.style.setProperty('--main-visibility', 'visible');
    
    updateScrollButtonVisibility();
    
    // Если есть хеш, он будет обработан scrollToElement в navigateToHome
    // Иначе, прокручиваем на самый верх (мгновенно, чтобы избежать "прыжка")
