...
// --- HYDRATION LOGIC (Новая функция для статических страниц) ---
async function hydrateStaticPage() {
    console.log("Hydrating static content...");
    
    // 1. Рендерим меню (в статике оно пустое)
    renderMenu();

    // 2. Рендерим футер (год и копирайт)
    ...
    // 3. "Оживляем" анимации БЕЗ мигания (добавляем is-visible сразу)
    
    // Заголовки
    const h1 = document.querySelector('h1');
    if (h1) h1.classList.add('animate-always', 'is-visible');

    // Подзаголовки/цены
    document.querySelectorAll('.detail-price, .hero-subtitle-container').forEach(el => {
        el.classList.add('animate-always', 'is-visible');
    });

    // Основной текст статьи
    document.querySelectorAll('.detail-content > *').forEach(el => {
        el.classList.add('floating-item', 'is-visible');
    });

    // Карточки и прочее
    const otherAnimated = document.querySelectorAll('#services .animated-container, #portfolio .animated-container, .detail-main-image');
    otherAnimated.forEach(el => {
        el.classList.add('animate-on-scroll', 'is-visible');
    });

    // 4. Запускаем наблюдателей
    setupObservers();

    // 5. Загружаем данные из Firebase в фоне
    try {
        siteData = await loadData();
        
        // 6. Рендерим блок "Related Posts"
        const path = window.location.pathname;
        ...
        if (match) {
            ...
            // Навешиваем анимацию на новые элементы (уже без принудительного is-visible, пусть красиво появляются)
            const relatedCards = document.querySelectorAll('#related-posts .item-card, #related-posts h2');
            relatedCards.forEach(el => {
...
async function initApp() {
    ...
    // Проверяем, статическая ли это страница (по атрибуту в body)
    const isStaticPage = document.body.dataset.staticPage === 'true';

    if (isStaticPage) {
        // --- СЦЕНАРИЙ 1: Статическая страница ---
        // Не очищаем DOM, просто подключаем функционал
        await hydrateStaticPage();
        
        // Убираем лоадер, если он есть
        const loader = document.getElementById('loader');
        if (loader) loader.classList.add('hidden'); // Используем класс из статического HTML

    } else {
        // --- СЦЕНАРИЙ 2: SPA (Главная или пустая страница) ---
        try {
            siteData = await loadData();
            renderMenu();
            routeAndRender();
            ...
            // Скрываем лоадер главной страницы
            const loader = document.getElementById('loader');
