...
function renderSection(key, title, items) {
    ...
    // FIX: Using English names for all languages
...
function initFloatingTocToggle() {
    ...
    toggleBtn.addEventListener('click', (event) => {
        event.stopPropagation(); // FIX: Prevent closing immediately
...
function initDesktopCarousels() {
    document.querySelectorAll('.desktop-carousel-container').forEach(carousel => {
        ...
        function goToSlide(index) {
            ...
            // Explicitly toggle active class to ensure only one is visible
            slides.forEach((s, i) => {
                if (i === currentIndex) {
...
function initStaticEventListeners() { 
    ...
    // FIX: GLOBAL MENU CLOSE LISTENER & BUTTON HANDLER SEPARATION
    ...
    if (mt) { 
        mt.addEventListener('click', (e) => { 
            e.stopPropagation(); // ВАЖНО: Остановить всплытие, чтобы глобальный клик не закрыл меню сразу
    ...
    // 2. Global Click (Close Menu/TOC when clicking outside)
    document.addEventListener('click', (e) => {
        const isMenuOpen = document.body.classList.contains('nav-is-open');
        const clickedInsideMenu = e.target.closest('.nav-menu');
        const clickedButton = e.target.closest('.menu-toggle');
        
        // TOC logic handles itself via initFloatingTocToggle, but Menu needs this:
        ...
        // Also ensure TOC closes if clicked outside (redundant safety)
        const tocPanel = document.getElementById('toc-content-panel');
        const tocBtn = document.getElementById('toc-toggle-btn');
        const clickedInsideToc = e.target.closest('#floating-toc-wrapper');
        
        if (tocPanel && tocPanel.classList.contains('is-visible') && !clickedInsideToc) {
