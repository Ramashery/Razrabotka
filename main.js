...
function formatContentHtml(content) {
    ...
    const html = blocks.map(block => {
        ...
        // Это более универсальная проверка, которая сработает для ЛЮБОГО тега.
        const isAlreadyHtml = /^<[a-zA-Z1-6\/!]/i.test(trimmedBlock);
        const isEscapedHtml = /^&lt;[a-zA-Z1-6\?\/!]/i.test(trimmedBlock);
