...
function renderDetailPage(collection, slug, lang) {
    ...
    // --- ИСПРАВЛЕНИЕ: Скроллим к заголовку h1 после отрисовки ---
    setTimeout(() => {
        const h1Element = document.querySelector('.detail-page-header h1');
        if (h1Element) {
...
async function navigateToHome() {
    try {
        ...
        if (newMain) {
            ...
            // --- ИСПРАВЛЕНИЕ: Скроллим к верху при переходе на главную ---
            setTimeout(() => {
                window.scrollTo({ 
                    top: 0, 
                    behavior: 'smooth' 
