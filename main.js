...
function formatContentHtml(content, siteData = null, lang = 'en') {
    ...
    const html_parts = blocks.map(block => {
        ...
        if (carouselMatch && siteData) {
            ...
            // НОВЫЙ ДИЗАЙН КАРУСЕЛИ (КАК ITEM-CARD)
...
function renderSection(key, title, items) {
    ...
    // langNames MAP (Needed for desktop titles)
    const langNames = { 'en': 'English', 'ka': 'ქართული', 'ru': 'Русский', 'uk': 'Українська' };
...
// --- SLIDER INITIALIZATION FUNCTIONS (UN-MINIFIED) ---
...
function initDesktopCarousels() {
    document.querySelectorAll('.desktop-carousel-container').forEach(carousel => {
        ...
        function goToSlide(index) {
            ...
            // FIX: Explicitly toggle active class to ensure only one is visible
            slides.forEach((s, i) => {
                if (i === currentIndex) {
...
function initMobileSliders() {
    document.querySelectorAll('.language-slider-block').forEach(sliderBlock => {
        ...
        // Handle case where nav might be missing if we have only 1 slide
...
function initStaticEventListeners() { 
    ...
    // FIX 3: GLOBAL MENU CLOSE LISTENER
    document.addEventListener('click', (e) => {
        const isMenuOpen = document.body.classList.contains('nav-is-open');
        const clickedInsideMenu = e.target.closest('.nav-menu');
        const clickedButton = e.target.closest('.menu-toggle');
        
        // Если меню открыто, и клик не по самому списку меню и не по кнопке гамбургера -> закрыть
