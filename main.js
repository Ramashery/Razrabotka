...
function formatContentHtml(content, siteData = null, lang = 'en') {
    ...
    const html_parts = blocks.map(block => {
        ...
        if (carouselMatch && siteData) {
            ...
            return `<p style="margin: 1.5em 0;"><img src="${trimmedBlock}" loading="lazy" alt="Embedded content" style="max-width: 100%; height: auto; display: block; margin: 0 auto; border-radius: 4px; border: 1px solid var(--color-border);" /></p>`;
        } else {
