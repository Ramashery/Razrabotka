...
async function hydrateStaticPage() {
    ...
    // Load Firebase data with a 5-second timeout.
    // This prevents Googlebot from hanging on the Firestore long-polling
    // connection (XHR Listen/channel), which blocked JS execution on detail
    // pages and caused them to never complete rendering for the indexer.
    // Static HTML already contains the full page content, so the site works
    // correctly even when Firebase data is unavailable (e.g. for bots).
    try {
        const firebaseTimeout = new Promise((_, reject) =>
            setTimeout(() => reject(new Error('Firebase load timeout')), 5000)
        );
        siteData = await Promise.race([loadData(), firebaseTimeout]);
        console.log("[hydrateStaticPage] Firebase data loaded successfully.");
    } catch (error) {
        console.log("[hydrateStaticPage] Firebase unavailable or timed out — using static HTML content.", error.message);
    ...
    if (match) {
        // Detail page: static HTML already contains full content — do NOT
        // call routeAndRender/renderDetailPage here, which would overwrite it.
        // Only apply dynamic enhancements that don't touch the main content.
        const [, lang, col, slug] = match;
        ...
        // Homepage: content is rendered dynamically from Firebase / preloaded-data.
        hydrateHomePageContent();
    }
    initializeCarousel();
}
