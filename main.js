...
function handleNavigation(e) {
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            window.history.pushState({}, '', targetUrl.href);
            mainContentEl.style.transition = 'none'; // Отключаем, чтобы мгновенно сменить контент
            routeAndRender(); 
            requestAnimationFrame(() => {
                requestAnimationFrame(() => {
                    // ИСПРАВЛЕНИЕ: Включаем анимацию для мягкого появления
                    mainContentEl.style.transition = 'opacity 0.6s ease-out, transform 0.6s cubic-bezier(0.2, 0.8, 0.2, 1)';
