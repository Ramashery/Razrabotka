    ...
    const detailPageHTML = `
        <section>
            <div class="detail-page-header">
                <!-- Явно добавляем is-visible для H1, чтобы он был виден сразу -->
...
function handleNavigation(e) {
    ...
    if (isMenuOpen) {
        ...
        navOverlay.classList.remove('is-active'); // Здесь ранее была ошибка, но для закрытия меню это уже сделано в другом месте
    }
...
function hydrateHomePageContent() {
    ...
    if (preloadedDataEl) {
        try {
            ...
            // Используем preloaded data только если siteData для коллекций еще не загружена из Firestore.
            // Это приоритет для быстрой первой отрисовки, но после loadData() всегда используем полные данные.
            ['services', 'portfolio', 'blog', 'contact'].forEach(key => {
                if (!siteData[key] || siteData[key].length === 0 || !siteData[key][0].h1) { // Проверяем наличие полных данных
    ...
    // Применяем классы анимации для статически отрисованных секций главной страницы
    document.querySelectorAll('#services .item-card, #portfolio .item-card, #blog .item-card, #contact .item-card').forEach(el => el.classList.add('animate-on-scroll'));
    document.querySelectorAll('section h2').forEach(el => el.classList.add('animate-on-scroll')); 
...
function initStaticEventListeners() { 
    ...
    if (menuToggle && navOverlay) { 
        menuToggle.addEventListener('click', () => { 
            ...
            navOverlay.classList.toggle('is-active'); // ИСПРАВЛЕНО: было .remove, теперь .toggle
        }); 
    }
