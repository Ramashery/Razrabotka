...
// renderSection теперь используется только для ДИНАМИЧЕСКИХ секций (например, связанных постов).
// Для главной страницы логика перенесена в hydrateHomePageContent.
...
function initFloatingTocToggle() {
    // Эта функция должна вызываться только один раз для установки event listeners
    // и только если элементы TOC присутствуют и их display не 'none'.
...
function renderDetailPage(collection, slug, lang) {
    ...
    // Применяем классы анимации сразу после вставки HTML
    ...
    if (floatingTocWrapper && tocContentPanel && tocToggleBtn) {
        if (tocHtmlContent) {
            ...
            initFloatingTocToggle(); // Инициализация event listeners, если еще не были
        } else {
...
function hydrateHomePageContent() {
    ...
    if (preloadedDataEl) {
        try {
            ...
            // console.log("Preloaded Sections:", preloadedSections);

            // Если siteData.home еще не загружен (например, при первом SPA переходе на главную),
            // используем данные из прелоадера.
            if (!siteData.home || Object.keys(siteData.home).length === 0) {
                 siteData.home = preloadedSections.home || {};
            }
            // Для коллекций: если siteData уже содержит полные данные (с `h1`), то оставляем их.
            // Иначе (если siteData пуста или содержит только "облегченные" данные),
            // заполняем ее "облегченными" данными из прелоадера, пока не загрузим полные.
            ['services', 'portfolio', 'blog', 'contact'].forEach(key => {
                const hasFullData = siteData[key] && siteData[key].length > 0 && siteData[key][0].h1; // Прокси для "полных данных"
                if (!hasFullData) {
                    siteData[key] = preloadedSections[key] || [];
                    // console.log(`  Updating siteData[${key}] with preloaded lean data. New length: ${siteData[key]?.length}`);
                }
            });
            preloadedDataEl.remove();
        } catch (e) { console.error("Ошибка при парсинге предзагруженных данных:", e); }
    }
    // console.log("siteData AFTER preloaded-data check:", JSON.parse(JSON.stringify(siteData)));
    ...
    sectionsToAdapt.forEach(key => {
        ...
        if (sectionEl) {
            const isMobile = window.matchMedia("(max-width: 767px)").matches;
            const existingGrid = sectionEl.querySelector('.item-grid'); // Это статическая сетка
            let mobileSlidersContainer = sectionEl.querySelector('.mobile-sliders-container');

            // Заголовок H2 уже статически отрисован, просто убеждаемся, что класс анимации есть
            ...
            if (isMobile) {
                // Если мобильная версия, скрываем статическую сетку
                if (existingGrid) {
                    existingGrid.style.display = 'none';
                }

                // Создаем или получаем контейнер для мобильных слайдеров
                ...
                // Динамически генерируем и наполняем HTML для мобильных слайдеров
                const itemsToRender = siteData[key] || []; // Используем глобальный siteData (полные данные)
                const langOrder = ['en', 'ka', 'uk', 'ru'];
                const langNames = { en: 'English', ka: 'Georgian', uk: 'Ukrainian', ru: 'Russian' };
                const itemsByLang = {};
                itemsToRender.forEach(item => {
                ...
                // Убеждаемся, что статическая сетка видна
                if (existingGrid) {
    ...
    // Инициализируем слайдеры после того, как все динамические контейнеры созданы
    initMobileSliders();
    initDesktopCarousels(); // Вызывается, чтобы инициализировать карусели, если они есть (на десктопе они статичны, но могут иметь навигацию)

    // Footer и прокрутка к хешу
