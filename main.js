...
// --- ФУНКЦИЯ ДЛЯ СКРОЛЛА К ЦЕНТРУ ЭКРАНА ---
function scrollToElementCenter(elementId) {
    ...
    // Вычисляем высоту окна просмотра
    const windowHeight = window.innerHeight;
    
    // Рассчитываем позицию, чтобы элемент оказался в центре экрана
    // Центр элемента должен совпадать с центром окна просмотра
...
function handleNavigation(e) {
    ...
    if (targetUrl.pathname === window.location.pathname && targetUrl.hash) {
        setTimeout(() => {
            ...
            // Используем новую функцию для центрирования заголовка
            scrollToElementCenter(hash);
        }, menuCloseDelay);
        return;
    }
...
function hydrateHomePageContent() {
    ...
    if (hash) {
        requestAnimationFrame(() => {
            scrollToElementCenter(hash.substring(1));
        });
    }
}
