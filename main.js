...
function renderSeoTags(data) {
    ...
    // --- SEO FIX: Улучшенная обработка Schema.org ---
...
async function seedDatabaseIfEmpty() { 
    // Эта функция оставлена для истории, но отключена в loadData для ускорения
    ...
    if (!homeDoc.exists) { 
        ...
        for (const key of ['services', 'portfolio', 'blog', 'contact']) { 
            (initialSiteData[key] || []).forEach(item => { const { id, ...data } = item; batch.set(db.collection(key).doc(id), data); }); 
        } 
        await batch.commit(); 
    } 
};
...
async function loadData() { 
    const freshSiteData = {}; 
    try { 
        // --- PERFORMANCE FIX: Отключена проверка заполнения БД при каждой загрузке ---
...
function initApp() {
    ...
    // --- FAILSAFE: Принудительное скрытие лоадера через 4 секунды ---
    // Это спасет PageSpeed, если Firebase ответит слишком медленно
    setTimeout(() => {
