...
const initialSiteData = {
    ...
    services: [], portfolio: [], blog: [], contact: [], carouselItems: [] // FIX: Added carouselItems to initialSiteData
};
...
async function loadData() {
    const freshSiteData = {};
    try {
        const collections = ['services', 'portfolio', 'blog', 'contact', 'carouselItems']; // FIX: Added carouselItems to collections
...
// FIX: formatContentHtml теперь принимает siteData и lang
function formatContentHtml(content, siteData = null, lang = 'en') {
    ...
    const html_parts = blocks.map(block => {
        ...
        if (carouselMatch && siteData) {
            ...
            // FIX: Используем переданный lang для фильтрации carouselItems
...
function initFloatingTocToggle() {
    ...
    // FIX: Закрываем TOC при клике на ссылку внутри него
    contentPanel.addEventListener('click', (event) => {
        if (event.target.closest('a')) {
            // Небольшая задержка, чтобы скролл успел начаться
...
function renderDetailPage(collection, slug, lang) {
    ...
    if (rawContent.trim().startsWith('[TOC]')) {
        ...
        // FIX: Передаем siteData и lang в formatContentHtml
...
function calculateQuizResult() {
    ...
    if (scores.hybrid >= 5 || (scores.hybrid >= scores.landing && scores.hybrid >= scores.website)) { resultHTML = `<h3>Your Result: Hybrid Approach</h3><p>Consider a phased approach: start with a core website (5-8 pages) and add campaign-specific landing pages as needed. This balances long-term brand building with short-term conversion optimization.</p>`; resultStyle = 'background-color: rgba(255, 251, 230, 0.1); border-color: #ffe58f;'; } else if (scores.landing >= 5 || scores.landing > scores.website) { resultHTML = `<h3>Your Result: Landing Page</h3><p>You need a focused, conversion-optimized landing page. Perfect for campaigns, single offers, events, or testing new business ideas. Expected investment: $500-$2,000 for professional landing page design in Tbilisi.</p>`; resultStyle = 'background-color: rgba(230, 247, 255, 0.1); border-color: #91d5ff;'; } else { resultHTML = `<h3>Your Result: Multi-Page Website</h3><p>You need comprehensive site development. Invest in a multi-page website to build brand credibility, rank in search engines, and support complex sales processes. Expected investment: $3,000-$15,000+ depending on features.</p>`; resultStyle = 'background-color: rgba(246, 255, 237, 0.1); border-color: #b7eb8f;'; }
...
function initMobileSliders() { document.querySelectorAll('.language-slider-block').forEach(sliderBlock => { const slider = sliderBlock.querySelector('.cross-fade-slider'); const slides = slider.querySelectorAll('.item-card'); const nav = sliderBlock.querySelector('.slider-nav'); const dots = nav.querySelectorAll('.slider-dot'); if (slides.length <= 1) return; let currentIndex = 0; let touchStartX = 0; let autoSlideInterval; function goToSlide(index) { currentIndex = (index + slides.length) % slides.length; slides.forEach((s, i) => s.classList.toggle('active', i === currentIndex)); dots.forEach((d, i) => d.classList.toggle('active', i === currentIndex)); } function startAutoSlide() { stopAutoSlide(); autoSlideInterval = setInterval(() => goToSlide(currentIndex + 1), 5000); } function stopAutoSlide() { clearInterval(autoSlideInterval); } goToSlide(0); startAutoSlide(); nav.addEventListener('click', e => { if (e.target.matches('.slider-dot')) { stopAutoSlide(); goToSlide(parseInt(e.target.dataset.index)); startAutoSlide(); } }); slider.addEventListener('touchstart', e => { touchStartX = e.changedTouches[0].screenX; stopAutoSlide(); }, { passive: true }); slider.addEventListener('touchend', e => { const touchEndX = e.changedTouches[0].screenX; const touchEndY = e.changedTouches[0].screenY; const diffX = touchStartX - touchEndX; const diffY = Math.abs(touchStartY - touchEndY); if (Math.abs(diffX) > 50 && Math.abs(diffX) > diffY) { if (diffX > 0) { goToSlide(currentIndex + 1); } else { goToSlide(currentIndex - 1); } startAutoSlide(); } }, { passive: true }); }); }
    ...
    if (preloadedDataEl) {
        try {
            ...
            ['services', 'portfolio', 'blog', 'contact'].forEach(key => {
                if (!siteData[key] || siteData[key].length === 0 || !siteData[key][0].h1) {
...
function initStaticEventListeners() { 
    ...
    if (menuToggle && navOverlay) { 
        ...
        // FIX: Исправленная логика закрытия меню при клике на оверлей
        navOverlay.addEventListener('click', (e) => {
            // Проверяем, что клик был именно по самому оверлею, а не по его дочерним элементам (меню)
