...
function renderDetailPage(collection, slug, lang) {
    ...
    if (floatingTocWrapper && tocContentPanel && tocToggleBtn) {
        if (tocHtmlContent) {
            // Обновляем текст кнопки + стрелка
            tocToggleBtn.innerHTML = `${tocTitle} <span class="toc-arrow"></span>`;
            
            tocContentPanel.innerHTML = tocHtmlContent; 
            floatingTocWrapper.style.display = 'flex'; // Sticky контейнер виден (flex для центровки btn)
    ...
    `;
    mainContentEl.insertAdjacentHTML('beforeend', scrollButtonHTML); // Добавляем в конец main
...
function handleNavigation(e) {
    ...
    setTimeout(() => {
        // ИСПРАВЛЕНИЕ: Мягкое исчезновение перед загрузкой
        mainContentEl.style.transition = 'opacity 0.4s ease-out, transform 0.4s ease-out';
        mainContentEl.classList.add('is-transitioning');
        
        setTimeout(async () => { // Добавлено 'async'
            window.history.pushState({}, '', targetUrl.href);
            // УДАЛЕНО: mainContentEl.style.transition = 'none';

            // Ждем завершения рендеринга нового контента
            await routeAndRender(); 
            
            // Принудительная перерисовка для сброса состояния CSS
            mainContentEl.offsetHeight; 

            // ИСПРАВЛЕНИЕ: Включаем анимацию для мягкого появления
            mainContentEl.classList.remove('is-transitioning');
            if (!targetUrl.hash) { window.scrollTo({ top: 0, behavior: 'auto' }); }

            // Очищаем inline-стиль transition после завершения анимации, чтобы избежать конфликтов
            setTimeout(() => {
                mainContentEl.style.transition = '';
            }, 600); // Соответствует длительности transition в CSS
        }, 400); // Задержка для завершения анимации исчезновения
    }, menuCloseDelay);
}
    ...
    try {
        ...
        if (match) {
            ...
            if (currentItem) {
                ...
                // Инициализация плавающего TOC после загрузки статической страницы
                const floatingTocWrapper = document.getElementById('floating-toc-wrapper');
                if (floatingTocWrapper && floatingTocWrapper.style.display !== 'none') {
                    initFloatingTocToggle();
                }
