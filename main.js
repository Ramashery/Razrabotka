...
const GEORGIAN_TRANSLIT_MAP = { 'ა': 'a', 'ბ': 'b', 'გ': 'g', 'დ': 'd', 'ე': 'e', 'ვ': 'v', 'ზ': 'z', 'თ': 't', 'ი': 'i', 'კ': 'k', 'ლ': 'l', 'მ': 'm', 'ნ': 'n', 'ო': 'o', 'პ': 'p', 'ჟ': 'zh', 'რ': 'r', 'ს': 's', 'ტ': 't', 'უ': 'u', 'ფ': 'p', 'ქ': 'k', 'ღ': 'gh', 'ყ': 'q', 'შ': 'sh', 'ჩ': 'ch', 'ც': 'ts', 'ძ': 'dz', 'წ': 'ts', 'ჭ': 'ch', 'ხ': 'kh', 'ჯ': 'j', 'ჰ': 'h' };
const CYRILLIC_TRANSLIT_MAP = { 'а': 'a', 'б': 'b', 'в': 'v', 'г': 'g', 'д': 'd', 'е': 'e', 'ё': 'yo', 'ж': 'zh', 'з': 'z', 'и': 'i', 'й': 'y', 'к': 'k', 'л': 'l', 'м': 'm', 'н': 'n', 'о': 'o', 'п': 'p', 'р': 'r', 'с': 's', 'т': 't', 'у': 'u', 'ф': 'f', 'х': 'kh', 'ц': 'ts', 'ч': 'ch', 'ш': 'sh', 'щ': 'shch', 'ъ': '', 'ы': 'y', 'ь': '', 'э': 'e', 'ю': 'yu', 'я': 'ya', 'ґ': 'g', 'є': 'ye', 'і': 'i', 'ї': 'yi' };

function slugify(text) {
    text = String(text).toLowerCase();
    let hasGeorgian = false;
    for (let i = 0; i < text.length; i++) { const charCode = text.charCodeAt(i); if (charCode >= 0x10D0 && charCode <= 0x10FF) { hasGeorgian = true; break; } }
    if (hasGeorgian) { let transliterated = ''; for (let i = 0; i < text.length; i++) transliterated += GEORGIAN_TRANSLIT_MAP[text[i]] || text[i]; text = transliterated; }
    let hasCyrillic = false;
    for (let i = 0; i < text.length; i++) { const charCode = text.charCodeAt(i); if (charCode >= 0x0400 && charCode <= 0x04FF) { hasCyrillic = true; break; } }
    if (hasCyrillic) { let transliterated = ''; for (let i = 0; i < text.length; i++) transliterated += CYRILLIC_TRANSLIT_MAP[text[i]] || text[i]; text = transliterated; }
...
function setupObservers() {
    ...
    floatingObserver = new IntersectionObserver((entries) => { entries.forEach(entry => { const target = entry.target; const isAboveViewport = entry.boundingClientRect.top < 0 && !entry.isIntersecting; if (entry.isIntersecting) { target.classList.add('is-visible'); target.classList.remove('is-above'); } else { target.classList.remove('is-visible'); if (isAboveViewport) target.classList.add('is-above'); else target.classList.remove('is-above'); } }); }, { threshold: 0, rootMargin: "-50px 0px -50px 0px" });
    animateOnceObserver = new IntersectionObserver((entries, observer) => { entries.forEach(entry => { if (entry.isIntersecting) { entry.target.classList.add('is-visible'); observer.unobserve(entry.target); } }); }, { threshold: 0.1, rootMargin: "0px 0px -50px 0px" });
    animateAlwaysObserver = new IntersectionObserver((entries) => { entries.forEach(entry => { if (entry.isIntersecting) entry.target.classList.add('is-visible'); else entry.target.classList.remove('is-visible'); }); }, { threshold: 0.1, rootMargin: "0px 0px -50px 0px" });
...
async function loadData() {
    ...
    try {
        ...
        const dataPromises = [ db.collection('home').doc('content').get(), ...collections.map(col => { if (col === 'carouselItems') return db.collection(col).orderBy('order').get(); return db.collection(col).get(); }) ];
        ...
        collections.forEach((col, index) => { freshSiteData[col] = snapshots[index].docs.map(doc => ({ id: doc.id, ...processDocData(doc.data()) })).filter(item => item.status !== 'archived'); });
        return freshSiteData;
    } catch (error) { console.error("Error loading data from Firebase:", error); return JSON.parse(JSON.stringify(initialSiteData)); }
}

function formatContentHtml(content, siteData = null, lang = 'en') {
    if (!content) return '';
    let processedContent = content.replace(/<pre(.*?)>([\s\S]*?)<\/pre>/gim, function(match, attrs, inner) { const codeMatch = inner.match(/^\s*<code(.*?)>([\s\S]*?)<\/code>\s*$/i); if (codeMatch) { const codeAttrs = codeMatch[1]; const codeContent = codeMatch[2]; const escapedContent = codeContent.replace(/</g, '&lt;').replace(/>/g, '&gt;'); return `<pre${attrs}><code${codeAttrs}>${escapedContent}</code></pre>`; } else { const escapedInner = inner.replace(/</g, '&lt;').replace(/>/g, '&gt;'); return `<pre${attrs}>${escapedInner}</pre>`; } });
    ...
    const html_parts = blocks.map(block => {
        ...
        if (carouselMatch && siteData) {
            ...
            const carouselItems = (siteData.carouselItems || []).filter(item => item.groupKey === groupKey && item.lang === lang);
            if (carouselItems.length === 0) return '';
            const carouselId = `glowCarousel-${groupKey}-${Math.floor(Math.random() * 9000) + 1000}`;
            const slidesHtml = carouselItems.map(item => { const isLink = item.linkUrl && item.linkUrl !== '#'; const tag = isLink ? 'a' : 'div'; const hrefAttr = isLink ? `href="${item.linkUrl}" target="_blank" rel="noopener noreferrer"` : ''; return ` <${tag} ${hrefAttr} class="card"> <div class="card-image-bg" style="background-image: url('${item.imageUrl || ''}');"></div> <div class="card-inner-content"> <h4>${item.title || ''}</h4> ${item.role ? `<div class="card-subtitle">${item.role}</div>` : ''} <div class="card-desc"> ${item.content || ''} </div> </div> </${tag}> `; }).join('');
            return ` <div class="carousel-container" id="${carouselId}"> <button class="nav-arrow left">‹</button> <div class="carousel-track">${slidesHtml}</div> <button class="nav-arrow right">›</button> <div class="dots"></div> </div> `;
        } else if (/^<(p|div|h[1-6]|ul|ol|li|blockquote|hr|table|pre)/i.test(trimmedBlock)) { return trimmedBlock; } else if (youtubeMatch && youtubeMatch[1]) { return `<div class="embedded-video" style="position: relative; padding-bottom: 56.25%; height: 0; overflow: hidden; max-width: 100%; background: #000; margin: 1.5em 0; border-radius: 4px; border: 1px solid var(--color-border);"><iframe style="position: absolute; top: 0; left: 0; width: 100%; height: 100%;" src="https://www.youtube.com/embed/${youtubeMatch[1]}" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe></div>`; } else if (imageMatch) { return `<p style="margin: 1.5em 0;"><img src="${trimmedBlock}" alt="Embedded content" style="max-width: 100%; height: auto; display: block; margin: 0 auto; border-radius: 4px; border: 1px solid var(--color-border);" /></p>`; } else { return `<p>${trimmedBlock.replace(/\n/g, '<br>')}</p>`; }
    }).filter(Boolean);
    const groupedHtml = [];
    const GROUP_SIZE = 3;
    let temp_group = [];
    html_parts.forEach(part => { if (part.includes('carousel-container')) { if (temp_group.length > 0) { groupedHtml.push(`<div class="content-group">${temp_group.join('')}</div>`); temp_group = []; } groupedHtml.push(part); } else { temp_group.push(part); if (temp_group.length >= GROUP_SIZE) { groupedHtml.push(`<div class="content-group">${temp_group.join('')}</div>`); temp_group = []; } } });
    if (temp_group.length > 0) groupedHtml.push(`<div class="content-group">${temp_group.join('')}</div>`);
    return groupedHtml.join('\n');
}

// FIX 2: ВОССТАНАВЛИВАЕМ ФУНКЦИЮ ДЛЯ СЛАЙДЕРОВ НА ГЛАВНОЙ
...
function initFloatingTocToggle() {
    ...
    const closeToc = () => { toggleBtn.setAttribute('aria-expanded', 'false'); contentPanel.setAttribute('aria-hidden', 'true'); contentPanel.classList.remove('is-visible'); toggleBtn.classList.remove('is-active'); };
    toggleBtn.addEventListener('click', (event) => { event.stopPropagation(); const isExpanded = toggleBtn.getAttribute('aria-expanded') === 'true'; if (isExpanded) { closeToc(); } else { toggleBtn.setAttribute('aria-expanded', 'true'); contentPanel.setAttribute('aria-hidden', 'false'); contentPanel.classList.add('is-visible'); toggleBtn.classList.add('is-active'); } });
    document.addEventListener('click', (event) => { if (contentPanel.classList.contains('is-visible') && !floatingTocWrapper.contains(event.target)) { closeToc(); } });
    contentPanel.addEventListener('click', (event) => { if (event.target.closest('a')) { setTimeout(closeToc, 100); } });
...
function initializeCarousel() {
    ...
    if (carouselRoots.length > 0) { carouselRoots.forEach(root => { if (root.id && typeof window.initGlowCarousel === 'function') { const instance = window.initGlowCarousel(root.id); if (instance) glowCarouselInstances.push(instance); } }); console.log(`Carousel: Initialized ${glowCarouselInstances.length} instances.`); }
}
...
function renderDetailPage(collection, slug, lang) {
    ...
    if (!item) { mainContentEl.innerHTML = `<section class="detail-page-header"><h1>404 - Not Found</h1><p>The page you were looking for does not exist.</p><a href="/">Go back home</a></section>`; if (floatingTocWrapper) floatingTocWrapper.style.display = 'none'; initializeCarousel(); return; }
    renderSeoTags(item);
    ...
    if (rawContent.trim().startsWith('[TOC]')) {
        ...
        const tocItems = [];
        doc.querySelectorAll('h2, h3').forEach(header => { const headerText = header.innerText.trim(); if (headerText) { const headerSlug = slugify(headerText); header.id = headerSlug; tocItems.push({ level: header.tagName.toLowerCase(), text: headerText, slug: headerSlug }); } });
        if (tocItems.length > 0) { let tocListHtml = '<ul>'; tocItems.forEach(item => { const className = item.level === 'h3' ? 'toc-level-h3' : ''; tocListHtml += `<li class="${className}"><a href="#${item.slug}">${item.text}</a></li>`; }); tocListHtml += '</ul>'; tocHtmlContent = tocListHtml; }
        finalContentHtml = doc.body.innerHTML;
    } else { finalContentHtml = formatContentHtml(rawContent, siteData, lang); }
    mainContentEl.innerHTML = ` <section> <div class="detail-page-header"> <h1 class="animate-always is-visible">${item.h1 || item.title || ''}</h1> ${item.price ? `<div class="detail-price animate-on-scroll"><span>${item.price}</span></div>` : ''} </div> <div class="detail-content">${finalContentHtml}</div> </section> <button id="scroll-to-top-btn" title="Наверх"> <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24"> <path fill="none" d="M0 0h24v24H0z"/> <path d="M13 7.828V20h-2V7.828l-5.364 5.364-1.414-1.414L12 4l7.778 7.778-1.414 1.414L13 7.828z"/> </svg> </button> `;
    document.querySelectorAll('.detail-content > .content-group').forEach(el => el.classList.add('floating-item'));
    if (floatingTocWrapper && tocContentPanel && tocToggleBtn) { if (tocHtmlContent) { tocToggleBtn.innerHTML = `${tocTitle} <span class="toc-arrow"></span>`; tocContentPanel.innerHTML = tocHtmlContent; floatingTocWrapper.style.display = 'flex'; tocToggleBtn.setAttribute('aria-expanded', 'false'); tocContentPanel.setAttribute('aria-hidden', 'true'); contentPanel.classList.remove('is-visible'); tocToggleBtn.classList.remove('is-active'); initFloatingTocToggle(); } else { floatingTocWrapper.style.display = 'none'; tocContentPanel.innerHTML = ''; } }
    renderRelatedPosts(collection, slug, lang); 
    if (ssrCarouselHtml) { const tempDiv = document.createElement('div'); tempDiv.innerHTML = ssrCarouselHtml; while (tempDiv.firstChild) mainContentEl.appendChild(tempDiv.firstChild); }
    initializeCarousel(); 
...
function renderRelatedPosts(currentCollection, currentSlug, currentLang) {
    ...
    const itemsHTML = relatedItems.map(item => { const langPrefix = item.lang ? `/${item.lang}` : ''; let itemUrl = `${langPrefix}/${item.collection}/${item.urlSlug}/`; const mediaArray = item.media || []; const imageUrl = (mediaArray.find && mediaArray.find(url => !/youtube|vimeo/.test(url))) || ''; return `<a href="${itemUrl}" class="item-card animate-on-scroll"><div class="item-card__image" style="background-image: url('${imageUrl}')"></div><div class="item-card__content"><h3>${item.title}</h3><div class="card-subtitle">${item.subtitle}</div><p>${item.description}</p></div></a>` }).join('');
...
function applyCustomBackground(item) {
    ...
    if (customCode && customCode.trim() !== "") { if (iframe.srcdoc === customCode && iframe.style.display === 'block') return; iframe.classList.remove('is-visible'); iframe.onload = () => { iframe.classList.add('is-visible'); iframe.onload = null; }; iframe.style.display = 'block'; iframe.srcdoc = customCode; } else { iframe.classList.remove('is-visible'); }
}

// FIX 1: ВОССТАНАВЛИВАЕМ ФУНКЦИЮ ДЛЯ КВИЗА
function calculateQuizResult() {
    ...
    if (scores.hybrid >= 5 || (scores.hybrid >= scores.landing && scores.hybrid >= scores.website)) { resultHTML = `<h3>Your Result: Hybrid Approach</h3><p>Consider a phased approach...</p>`; resultStyle = 'background-color: rgba(255, 251, 230, 0.1); border-color: #ffe58f;'; } else if (scores.landing >= 5 || scores.landing > scores.website) { resultHTML = `<h3>Your Result: Landing Page</h3><p>You need a focused landing page...</p>`; resultStyle = 'background-color: rgba(230, 247, 255, 0.1); border-color: #91d5ff;'; } else { resultHTML = `<h3>Your Result: Multi-Page Website</h3><p>You need comprehensive development...</p>`; resultStyle = 'background-color: rgba(246, 255, 237, 0.1); border-color: #b7eb8f;'; }
...
// FIX 2: ВОССТАНАВЛИВАЕМ ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ ДЛЯ СЛАЙДЕРОВ
...
function scrollToElementWithOffset(elementId, offset = 120) { const element = document.getElementById(elementId); if (!element) return; const elementTop = element.getBoundingClientRect().top + window.pageYOffset; const targetPosition = elementTop - offset; window.scrollTo({ top: targetPosition, behavior: 'smooth' }); }
...
async function navigateToHome(hash = '') {
    try {
        ...
        if (newMain) { mainContentEl.innerHTML = newMain.innerHTML; mainContentEl.classList.remove('loading'); document.title = doc.querySelector('title')?.textContent || 'Digital Craft'; }
        applyCustomBackground(siteData.home);
        hydrateHomePageContent();
        initializeCarousel();
        requestAnimationFrame(() => { const h1 = document.querySelector('.hero h1'); const sub = document.querySelector('.hero-subtitle-container'); if (h1) h1.classList.add('is-visible'); if (sub) sub.classList.add('is-visible'); });
        if (hash) setTimeout(() => { scrollToElementWithOffset(hash.substring(1), 120); }, 100);
    } catch (error) { console.error("Navigation error:", error); } finally { const toc = document.getElementById('floating-toc-wrapper'); if (toc) toc.style.display = 'none'; }
}
...
async function routeAndRender(isPopState = false, hash = '') {
    ...
    if (match) { const [, lang, col, slug] = match; renderDetailPage(col, slug, lang || 'en'); } else { await navigateToHome(hash || window.location.hash); }
...
function handleNavigation(e) {
    ...
    if (targetUrl.pathname === window.location.pathname && targetUrl.hash) { setTimeout(() => { window.history.pushState({}, '', targetUrl.href); scrollToElementWithOffset(targetUrl.hash.substring(1), 120); }, delay); return; }
    if (targetUrl.href === window.location.href) { window.scrollTo({ top: 0, behavior: 'smooth' }); return; }
    setTimeout(() => { mainContentEl.classList.add('is-transitioning'); setTimeout(() => { window.history.pushState({}, '', targetUrl.href); routeAndRender(false, targetUrl.hash); requestAnimationFrame(() => { requestAnimationFrame(() => mainContentEl.classList.remove('is-transitioning')); }); }, 400); }, delay);
}

function hydrateHomePageContent() {
    const preData = document.getElementById('preloaded-data');
    if (preData) { try { const data = JSON.parse(preData.textContent); ['services', 'portfolio', 'blog', 'contact'].forEach(k => { if (!siteData[k] || siteData[k].length === 0) siteData[k] = data[k] || []; }); preData.remove(); } catch (e) {} }
    applyCustomBackground(siteData.home);
    // FIX 2: ВЫЗЫВАЕМ ВОССТАНОВЛЕННУЮ ФУНКЦИЮ
...
async function hydrateStaticPage() {
    ...
    try {
        ...
        if (match) { const [, lang, col, slug] = match; const item = siteData[col]?.find(d => d.urlSlug === slug && d.lang === (lang || 'en')); if (item) { applyCustomBackground(item); if (!document.getElementById('related-posts')) renderRelatedPosts(col, slug, lang || 'en'); } } else { hydrateHomePageContent(); }
        initializeCarousel();
    } catch (error) {}
}
...
function initStaticEventListeners() { 
    ...
    if (menuToggle && navOverlay) {
        // Слушатель для кнопки-гамбургера (открыть/закрыть)
        ...
        // FIX 3: УЛУЧШЕННАЯ ЛОГИКА ЗАКРЫТИЯ МЕНЮ
        // Теперь мы слушаем клик по всему документу
        document.addEventListener('click', (e) => {
            const isMenuOpen = document.body.classList.contains('nav-is-open');
            // Проверяем, что меню открыто и клик был НЕ по кнопке и НЕ внутри оверлея
            if (isMenuOpen && !menuToggle.contains(e.target) && !navOverlay.contains(e.target)) {
...
function updateScrollButtonVisibility() { const b = document.getElementById('scroll-to-top-btn'); if (b) window.scrollY > 300 ? b.classList.add('visible') : b.classList.remove('visible'); }
...
async function initApp() { 
    ...
    if (document.body.dataset.staticPage === 'true') { await hydrateStaticPage(); routeAndRender(false, window.location.hash); } else { siteData = await loadData(); renderMenu(); await routeAndRender(); } 
}
