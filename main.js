...
function formatContentHtml(content, siteData = null, lang = 'en') {
    ...
    const html_parts = blocks.map(block => {
        ...
        // Изменено: добавлен |img в список исключений для оборачивания в <p>
        } else if (/^<(p|div|h[1-6]|ul|ol|li|blockquote|hr|table|pre|img)/i.test(trimmedBlock)) {
            return trimmedBlock;
            ...
            // Изменено: используются новые стили article-image и loading="lazy"
            return `<img src="${trimmed_block}" loading="lazy" class="article-image" alt="Embedded content" onload="this.classList.add('is-loaded')" />`;
        } else {
    ...
    // Добавляем lazy loading и анимацию ко всем картинкам, даже если они были вставлены через HTML
    let finalHtml = groupedHtml.join('\n');
    finalHtml = finalHtml.replace(/<img[^>]+>/gi, (match) => {
        let imgTag = match;
        ...
        // Добавляем класс анимации и onload, если нет
        if (!imgTag.includes('class=')) {
             // Нет классов вообще
            imgTag = imgTag.replace('<img', '<img class="article-image" onload="this.classList.add(\'is-loaded\')"');
        } else if (!imgTag.includes('article-image')) {
            // Классы есть, но нет нашего
            imgTag = imgTag.replace('class="', 'class="article-image ');
            // Убеждаемся, что есть триггер анимации
            if (!imgTag.includes('onload=')) {
                imgTag = imgTag.replace('<img', '<img onload="this.classList.add(\'is-loaded\')"');
            }
        }
        return imgTag;
    });
