...
// --- ЛОГИКА ДЛЯ СТАТИЧЕСКИХ СТРАНИЦ ---
...
async function loadDataForStaticPage() {
    try {
        const collections = ['services', 'blog', 'home'];
        const dataPromises = collections.map(col => db.collection(col).get());
        const [servicesSnapshot, blogSnapshot] = await Promise.all(dataPromises);

        siteData.services = servicesSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
        siteData.blog = blogSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
        const homeDoc = await db.collection('home').doc('content').get();
        siteData.home = homeDoc.exists ? homeDoc.data() : {};

        const path = window.location.pathname;
        const detailPageRegex = /^\/(?:([a-z]{2})\/)?(services|portfolio|blog)\/([a-zA-Z0-9-]+)\/?$/;
        ...
        if (match) {
            ...
            // Рендерим похожие посты только если их нет на странице
        ...
        console.error("Failed to load data for static page hydration:", error);
    }
}


// --- ЛОГИКА ДЛЯ SPA (ГЛАВНОЙ СТРАНИЦЫ) ---
async function initializeSpa() {
    console.log("SPA shell detected. Initializing full application...");
...
// --- ОБЩИЕ ФУНКЦИИ (ВАШ ОРИГИНАЛЬНЫЙ КОД) ---
...
function renderSeoTags(data) { 
    ...
    canonical.href = canonicalBaseUrl + cleanPath; document.head.appendChild(canonical); 
...
function renderSection(key, title, items) { 
    ...
    const desktopGridsHTML = langOrder.map(lang => { 
        ...
        const slidesHTML = slides.map((slideItems, index) => { 
            const cardsHTML = slideItems.map(item => { 
                let itemUrl = `/${item.lang}/${key}/${item.urlSlug}/`; 
                return `<a href="${itemUrl}" class="item-card"><div class="item-card__image" style="background-image: url('${(item.media || []).find(url => !/youtube|vimeo/.test(url)) || ''}')"></div><div class="item-card__content"><h3>${item.title}</h3><div class="card-subtitle">${item.subtitle}</div><p>${item.description}</p></div></a>`; }).join(''); 
    ...
    const mobileSlidersHTML = langOrder.map(lang => { 
        ...
        const slidesHTML = langItems.map((item, index) => { 
            let itemUrl = `/${item.lang}/${key}/${item.urlSlug}/`; 
            return `<a href="${itemUrl}" class="item-card ${index === 0 ? 'active' : ''}"><div class="item-card__image" style="background-image: url('${(item.media || []).find(url => !/youtube|vimeo/.test(url)) || ''}')"></div><div class="item-card__content"><h3>${item.title}</h3><div class="card-subtitle">${item.subtitle}</div><p>${item.description}</p></div></a>` }).join(''); 
...
function renderRelatedPosts(currentCollection, currentSlug, currentLang) { 
    ...
    const itemsHTML = relatedItems.map(item => { 
        let itemUrl = `/${item.lang}/${item.collection}/${item.urlSlug}/`; 
...
function initDesktopCarousels() { /*... Ваш оригинальный код ...*/ }
function initMobileSliders() { /*... Ваш оригинальный код ...*/ }
...
function handleNavigation(e) { 
    ...
    if (isMenuOpen) { document.body.classList.remove('nav-is-open'); menuToggle.classList.remove('is-active'); navOverlay.classList.remove('is-active'); } 
    if (targetUrl.hash) { 
        const menuCloseDelay = isMenuOpen ? 350 : 0; 
        setTimeout(() => { 
            if (targetUrl.pathname === window.location.pathname) { const targetElement = document.getElementById(targetUrl.hash.substring(1)); if (targetElement) { window.history.pushState({}, '', targetUrl.href); targetElement.scrollIntoView({ behavior: 'smooth' }); } return; } 
                    ...
                    requestAnimationFrame(() => { 
                        ...
                        requestAnimationFrame(() => { 
                            requestAnimationFrame(() => { 
                                ...
                                if (targetElement) { setTimeout(() => targetElement.scrollIntoView({ behavior: 'smooth' }), 50); } else { window.scrollTo({ top: 0, behavior: 'smooth' }); } 
                            }); 
                        }); 
                    }); 
                }); 
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (isStaticPage) {
        // Если это статически сгенерированная страница, "оживляем" ее
        initializeStaticPage();
    } else {
        // Если это главная страница, запускаем полноценное SPA
        initializeSpa();
    }

    // Общие обработчики событий для всех страниц
