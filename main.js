...
function renderDetailPage(collection, slug, lang) {
    ...
    // --- НОВАЯ ЛОГИКА ДЛЯ АВТОМАТИЧЕСКОГО ОГЛАВЛЕНИЯ (TOC) ---
    ...
    if (rawContent.trim().startsWith('[TOC]')) {
        ...
        if (tocItems.length > 0) {
            ...
            tocListHtml += '</ul>';
            tocHtml = `<div id="toc-container" class="toc-container"><h2>Содержание статьи</h2>${tocListHtml}</div>`;
        }
    ...
    const detailPageHTML = `
        <section>
            ...
            ${tocHtml} <!-- Вставляем готовый HTML для TOC -->
...
function renderRelatedPosts(currentCollection, currentSlug, currentLang) {
    ...
    // ИСПРАВЛЕНИЕ: Увеличено количество ссылок с 3 до 6 для соответствия с серверной частью.
