...
function renderDetailPage(collection, slug, lang) { 
    ...
    mainContentEl.innerHTML = `<section><div class="detail-page-header"><h1 class="fade-in-up" style="animation-delay: 0.5s;">${item.h1 || ''}</h1>${item.price ? `<div class="detail-price fade-in-up" style="animation-delay: 0.7s;">${item.price}</div>` : ''}</div><img class="detail-main-image fade-in-up" src="${(item.media || []).find(url => !/youtube|vimeo/.test(url)) || ''}" alt="${item.mainImageAlt || item.title}" style="animation-delay: 0.6s;"><div class="detail-content">${formattedContent}</div></section>`; 
...
function renderHero() { 
    // Восстанавливаем статический контент при навигации
    const heroSection = document.getElementById('hero');
    if (!heroSection) return;

    heroSection.innerHTML = `
        <h1>Web Development <br class="mobile-break"> & SEO in Tbilisi</h1>
        <!-- Задержка 0.3s для подзаголовка, чтобы он появлялся после H1 -->
        ...
        entries.forEach(entry => { 
            ...
            if (entry.isIntersecting) { 
                animatedElements.forEach((el, index) => { 
                    // Задержка теперь управляется через HTML (style="animation-delay"), 
                    // поэтому здесь просто добавляем класс
                    el.classList.add('fade-in-up'); 
                }); 
            } else { 
                // Убираем класс, когда элемент уходит с экрана, чтобы анимация перезапустилась при возврате
...
function initApp() {
    // Инициализируем слушатели событий (меню) сразу же
    ...
    // --- ПРОВЕРКА НА СТАТИЧЕСКУЮ СТРАНИЦУ ---
    ...
    // Скрываем лоадер сразу, так как статический контент уже есть
