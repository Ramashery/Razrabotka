...
// Переменная для хранения SSR карусели (если она была вставлена сервером)
let ssrCarouselHtml = ''; 
...
const GEORGIAN_TRANSLIT_MAP = {
    ...
    'ტ': 't', 'უ': 'u', 'ფ': 'p', 'ქ': 'k', 'ღ': 'gh', 'ყ': 'q', 'ш': 'sh', 'ჩ': 'ch', 'ც': 'ts',
    'ძ': 'dz', 'წ': 'ts', 'ჭ': 'ch', 'ხ': 'kh', 'ჯ': 'j', 'ჰ': 'h',
};

const CYRILLIC_TRANSLIT_MAP = {
    'а': 'a', 'б': 'b', 'в': 'v', 'г': 'g', 'д': 'd', 'е': 'e', 'ё': 'yo', 'ж': 'zh', 'з': 'z',
    'и': 'i', 'й': 'y', 'к': 'k', 'л': 'l', 'м': 'н', 'н': 'n', 'о': 'o', 'п': 'p', 'р': 'r',
...
// --- ОБНОВЛЕННЫЙ ФОРМАТТЕР КОНТЕНТА ---
function formatContentHtml(content, siteData = null, lang = 'en') {
    ...
    const html_parts = blocks.map(block => {
        ...
        // NEW: Проверка на тег карусели [CAROUSEL:group-key]
        ...
        if (carouselMatch && siteData) {
            ...
            const carouselId = `mzaCarousel-${groupKey}-${Math.floor(Math.random() * 9000) + 1000}`;
            
            const slidesHtml = carouselItems.map((item, index) => `
                <article class="mzaCarousel-slide" role="group" aria-roledescription="slide" aria-label="${index + 1} of ${carouselItems.length}">
                    <div class="mzaCard" style="--mzaCard-bg:url('${item.imageUrl}');">
                        <header class="mzaCard-head mzaPar-1">
                            <h2 class="mzaCard-title">${item.title}</h2>
                            <p class="mzaCard-kicker">${item.kicker}</p>
                        </header>
                        <p class="mzaCard-text mzaPar-2">${item.description}</p>
                        <footer class="mzaCard-actions mzaPar-3">
                            <a href="${item.buttonLink}" class="mzaBtn">${item.buttonText}</a>
                        </footer>
                    </div>
                </article>
            `).join('');
            ...
            return `
            <section id="carousel-section-${carouselId}" style="margin-top: 80px; margin-bottom: 80px;">
                <div class="mzaCarousel" id="${carouselId}" aria-roledescription="carousel" aria-label="Featured cards">
                    <div class="mzaCarousel-viewport" tabindex="0">
                        <div class="mzaCarousel-track">${slidesHtml}</div>
                    </div>
    ...
    // Группировка в content-group, пропуская карусели
    const groupedHtml = [];
    const GROUP_SIZE = 3;
    let temp_group = [];
    
    html_parts.forEach(part => {
        if (part.includes('mzaCarousel')) {
...
// --- ОБНОВЛЕННАЯ ИНИЦИАЛИЗАЦИЯ КАРУСЕЛЕЙ ---
let mzaCarouselInstances = [];
function initializeCarousel() {
    // Очищаем старые экземпляры
    mzaCarouselInstances = []; 
    // Находим все карусели на странице
    const carouselRoots = document.querySelectorAll('.mzaCarousel');
    if (carouselRoots.length > 0) {
        carouselRoots.forEach(root => {
            if (root.id && typeof window.initMzaCarousel === 'function') {
                const instance = window.initMzaCarousel(root.id, { transitionMs: 900 });
                if (instance) mzaCarouselInstances.push(instance);
            }
        });
        console.log(`MzaCarousel: Initialized ${mzaCarouselInstances.length} instances.`);
    }
}
...
function renderDetailPage(collection, slug, lang) {
    ...
    // Вставляем SSR карусель, если она была захвачена при гидратации (обычно в конце)
    if (ssrCarouselHtml) {
