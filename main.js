...
// --- ВСПОМОГАТЕЛЬНАЯ ФУНКЦИЯ ДЛЯ ПРОКРУТКИ К ЯКОРЮ С ЗАДЕРЖКОЙ ---
function scrollWithDelay(elementId, delay = 100) {
    const element = document.getElementById(elementId);
    if (!element) return;

    // Даем браузеру время на отрисовку и применение стилей
    setTimeout(() => {
        element.scrollIntoView({
            behavior: 'smooth',
            block: 'start' // 'start' в сочетании со scroll-margin-top обеспечивает правильное позиционирование
        });
    }, delay);
}
...
async function navigateToHome(hash = '') {
    try {
        ...
        if (newMain) {
            ...
            if (hash) {
                // Используем новую функцию scrollWithDelay для якорей главной страницы
                scrollWithDelay(hash.substring(1), 150); 
            }
        }
    } catch (error) {
...
async function routeAndRender(isPopState = false, hash = '') {
    ...
    // Если hash был передан, он уже обработан в navigateToHome
    // Если его нет, прокручиваем на самый верх мгновенно.
...
function handleNavigation(e) {
    ...
    if (targetUrl.pathname === window.location.pathname && targetUrl.hash) {
        setTimeout(() => {
            ...
            // Используем scrollWithDelay для якорных ссылок на той же странице
            scrollWithDelay(hash, 100); 
        }, menuCloseDelay);
        return;
    }
...
function hydrateHomePageContent() {
    ...
    if (hash) {
        // Используем scrollWithDelay для якорей при начальной загрузке главной страницы с хешем
        requestAnimationFrame(() => {
            scrollWithDelay(hash.substring(1), 150); 
        });
    }
...
function initStaticEventListeners() { 
    ...
    mainContentEl.addEventListener('click', function(event) {
        ...
        // Кнопка "Наверх" должна работать глобально, без якоря, поэтому window.scrollTo.
