...
function renderDetailPage(collection, slug, lang) {
    ...
    // FIX: Clear content IMMEDIATELY to prevent "Home content remains" bug
...
// --- SETUP EVENT LISTENERS (SEPARATED) ---
function setupMenuListeners() {
    ...
    if (overlay) {
        overlay.addEventListener('click', (e) => {
            // FIX: Close ONLY if clicked on overlay (free space), not on list items
            if (e.target === overlay) {
...
function setupTocListeners() {
    ...
    if (tocBtn && tocPanel) {
        tocBtn.addEventListener('click', (e) => {
            e.stopPropagation();
            const isExpanded = tocBtn.getAttribute('aria-expanded') === 'true';
            tocBtn.setAttribute('aria-expanded', !isExpanded);
            tocPanel.setAttribute('aria-hidden', isExpanded);
            tocPanel.classList.toggle('is-visible', !isExpanded);
            tocBtn.classList.toggle('is-active', !isExpanded);
        });

        // FIX: Close TOC when clicking a link inside it
        ...
        document.addEventListener('click', (e) => {
            if (tocPanel.classList.contains('is-visible') && !tocWrapper.contains(e.target)) {
                tocBtn.setAttribute('aria-expanded', 'false');
                tocPanel.setAttribute('aria-hidden', 'true');
                tocPanel.classList.remove('is-visible');
                tocBtn.classList.remove('is-active');
...
function initStaticEventListeners() { 
    ...
    // Setup specific UI listeners
    setupMenuListeners();
    // TOC listeners are initialized dynamically in renderDetailPage because TOC is created there
}
