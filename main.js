...
// --- НОВАЯ УТИЛИТА ДЛЯ ГЕНЕРАЦИИ ЯКОРЕЙ (SLUGS) ---
...
// --- НОВАЯ ФУНКЦИЯ: Инициализация переключателя плавающего TOC ---
function initFloatingTocToggle() {
    ...
    if (toggleBtn && contentPanel) {
        ...
        document.addEventListener('click', (event) => {
            if (!floatingTocWrapper.contains(event.target) && contentPanel.classList.contains('is-visible')) {
...
function renderDetailPage(collection, slug, lang) {
    ...
    let tocHtmlContent = ''; // Переименовано, чтобы избежать конфликта с переменной шаблона
    ...
    if (rawContent.trim().startsWith('[TOC]')) {
        ...
        doc.querySelectorAll('h2, h3').forEach(header => {
            ...
            if (headerText) {
                ...
                header.id = headerSlug; // Добавляем id к заголовку
                tocItems.push({
        ...
        if (tocItems.length > 0) {
            ...
            tocListHtml += '</ul>';
            // Это содержимое для панели, а не вся обертка
            tocHtmlContent = `<h2>Содержание статьи</h2>${tocListHtml}`;
        }
    ...
    const detailPageHTML = `
        <section>
            ...
            <div class="detail-content">${finalContentHtml}</div> <!-- Вставляем контент с ID -->
        </section>
    ...
    // === ИНЖЕКЦИЯ ПЛАВАЮЩЕГО TOC ЧЕРЕЗ JS ===
    let floatingTocWrapper = document.getElementById('floating-toc-wrapper');
    if (!floatingTocWrapper) { // Создаем, если не существует (например, при первой клиентской загрузке)
        floatingTocWrapper = document.createElement('div');
        floatingTocWrapper.id = 'floating-toc-wrapper';
        floatingTocWrapper.classList.add('floating-toc-wrapper');
        // Вставляем перед основным контентом, но после overlay
        const navOverlay = document.querySelector('.nav-overlay');
        if (navOverlay) {
            navOverlay.parentNode.insertBefore(floatingTocWrapper, navOverlay.nextSibling);
        } else {
            document.body.appendChild(floatingTocWrapper); // Fallback
        }
    }

    if (tocHtmlContent) {
        floatingTocWrapper.innerHTML = `
            <button id="toc-toggle-btn" class="toc-toggle-btn" aria-expanded="false" aria-controls="toc-content-panel">
                Table of Contents <span class="toc-arrow"></span>
            </button>
            <div id="toc-content-panel" class="toc-content-panel" aria-hidden="true">
                ${tocHtmlContent}
            </div>
        `;
        floatingTocWrapper.style.display = 'flex'; // Делаем обертку видимой (flex для кнопки и панели)
        initFloatingTocToggle(); // Инициализируем логику переключения
    } else {
        floatingTocWrapper.style.display = 'none'; // Скрываем, если TOC не нужен
    }
    // === КОНЕЦ ИНЖЕКЦИИ ПЛАВАЮЩЕГО TOC ===
...
function handleNavigation(e) {
    ...
    // Позволяем якорным ссылкам на той же странице работать нативно
    const targetUrl = new URL(link.href);
    if (targetUrl.pathname === window.location.pathname && targetUrl.hash) {
        // Если это ссылка из TOC, позволяем браузеру обработать ее для плавной прокрутки
        if (link.closest('.toc-content-panel')) { // Изменено с .toc-container на .toc-content-panel
            // Если панель открыта, закрываем ее после клика по якорю
            const contentPanel = document.getElementById('toc-content-panel');
            const toggleBtn = document.getElementById('toc-toggle-btn');
            if (contentPanel && toggleBtn && contentPanel.classList.contains('is-visible')) {
                toggleBtn.setAttribute('aria-expanded', 'false');
                contentPanel.setAttribute('aria-hidden', 'true');
                contentPanel.classList.remove('is-visible');
                toggleBtn.classList.remove('is-active');
        ...
        setTimeout(() => {
            ...
            requestAnimationFrame(() => {
                requestAnimationFrame(() => {
                    ...
                    if (!targetUrl.hash) { window.scrollTo({ top: 0, behavior: 'auto' }); }
                });
            });
        }, 400);
    }, menuCloseDelay);
    ...
    try {
        ...
        if (match) {
            ...
            if (currentItem) {
                ...
                // Инициализация плавающего TOC после загрузки статической страницы, если он есть
                if (document.getElementById('floating-toc-wrapper') && document.getElementById('floating-toc-wrapper').style.display !== 'none') {
...
function initStaticEventListeners() { 
    ...
    mainContentEl.addEventListener('click', function(event) {
        ...
        // Изменено: кнопка "Наверх" теперь всегда прокручивает в самый верх страницы
