...
function renderSeoTags(data) {
    ...
    // УЛУЧШЕННАЯ ОБРАБОТКА JSON-LD СХЕМЫ
    ...
    // Если есть валидная схема из админки - используем её
    ...
    // Иначе генерируем базовую схему как запасной вариант
    else {
...
function generateFallbackSchema(data) {
    // Не создаём схему, если нет минимальных данных
    ...
    const schema = {
        ...
        "@type": "WebPage",
        "name": data.h1 || data.seoTitle || data.title || document.title,
        "url": baseUrl + cleanPath
    };
    
    // Добавляем описание, если есть
    if (data.metaDescription || data.description) {
        schema.description = data.metaDescription || data.description;
    }
...
function updateMetaTags(itemData = {}) { 
    // Убрана проверка seoTitle - достаточно наличия данных
    const dataToRender = (itemData && Object.keys(itemData).length > 0) 
        ? itemData 
        : siteData.home; 
