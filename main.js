...
function renderHero() { 
    ...
    // UPDATED: Added hero-anim-elem class
    let heroContent = `<h1 class="hero-anim-elem">${h1 || ''}</h1>`; 
    if (subtitle) { 
        const formattedSubtitle = formatContentHtml(subtitle); 
        // UPDATED: Added hero-anim-elem class
        heroContent += `<div class="hero-subtitle-container hero-anim-elem">${formattedSubtitle}</div>`; 
    } 
    heroSection.innerHTML = heroContent; 
    
    // Re-init observer to catch new elements
    initHomePageObservers();
};
...
function initHomePageObservers() { 
    ...
    homePageObserver = new IntersectionObserver((entries) => { 
        entries.forEach(entry => { 
            // UPDATED: Logic for HERO section (re-trigger animation)
            if (entry.target.id === 'hero') {
                const animElements = entry.target.querySelectorAll('.hero-anim-elem');
                
                if (entry.isIntersecting) {
                    // Section visible: add class to start animation
                    animElements.forEach(el => el.classList.add('is-visible'));
                } else {
                    // Section hidden: remove class to reset animation for next scroll
                    animElements.forEach(el => el.classList.remove('is-visible'));
                }
            } 
    ...
    if (hero) homePageObserver.observe(hero);
