...
function renderSeoTags(data) {
    ...
    // Устанавливаем базовые SEO-данные
    ...
    // --- ИСПРАВЛЕННАЯ ЛОГИКА КАНОНИЧЕСКОГО URL ---
    const canonical = document.createElement('link');
    canonical.rel = 'canonical';
    const canonicalBaseUrl = 'https://digital-craft-tbilisi.site';
    let path = window.location.pathname;

    // Гарантируем, что у всех внутренних страниц есть слэш в конце
...
function handleNavigation(e) {
    ...
    // --- НАЧАЛО ИСПРАВЛЕНИЯ ---
    // Создаем URL и сразу же исправляем его, добавляя слэш при необходимости
    let finalUrl = new URL(link.href);
    ...
    if (finalUrl.pathname === window.location.pathname && finalUrl.hash) {
        setTimeout(() => {
            const targetElement = document.getElementById(finalUrl.hash.substring(1));
            if (targetElement) {
                window.history.pushState({}, '', finalUrl.href); // Используем исправленный URL
        ...
        setTimeout(() => {
            ...
            requestAnimationFrame(() => {
                requestAnimationFrame(() => {
                    ...
                    if (!finalUrl.hash) { window.scrollTo({ top: 0, behavior: 'auto' }); }
                });
            });
        }, 400);
    }, menuCloseDelay);
