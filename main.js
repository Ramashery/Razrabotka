...
// --- RENDER FUNCTIONS (ИСПРАВЛЕННАЯ ВЕРСИЯ) ---
function formatContentHtml(content) {
    if (!content) return '';

    // ШАГ 1: Умное экранирование HTML внутри тегов <pre>
    let processedContent = content.replace(/<pre(.*?)>([\s\S]*?)<\/pre>/gim, function(match, attrs, inner) {
        // Проверяем, есть ли внутри тег <code> (игнорируем пробелы по краям)
        const codeMatch = inner.match(/^\s*<code(.*?)>([\s\S]*?)<\/code>\s*$/i);

        if (codeMatch) {
            // ВАРИАНТ А: Внутри есть <code>. Оставляем сам тег <code> живым, экранируем только нутро.
            ...
            // ВАРИАНТ Б: Тега <code> нет. Экранируем всё содержимое <pre>.
    ...
    // ШАГ 2: Стандартная обработка остального контента
    ...
    }).filter(Boolean); // Удаляем пустые строки, если таковые появились
    
    // НОВОЕ: Группировка элементов для уменьшения ширины DOM
    const groupedHtml = [];
    const GROUP_SIZE = 10; // Количество элементов в одной группе
...
async function routeAndRender(isPopState = false) {
    ...
    // Обратите внимание: класс 'floating-item' добавляется к .content-group, а не к каждому дочернему элементу .detail-content
    ...
    // Блок прокрутки к якорю отсюда удален, т.к. он перенесен в hydrateHomePageContent
...
function hydrateHomePageContent() {
    ...
    // === ИСПРАВЛЕНИЕ: Прокрутка к якорю после отрисовки ===
    const hash = window.location.hash;
    if (hash) {
        // Используем requestAnimationFrame для гарантии, что прокрутка произойдет после отрисовки
        requestAnimationFrame(() => {
...
async function hydrateStaticPage() {
    ...
    // Изменено: теперь 'floating-item' применяется к группам контента
