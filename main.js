...
// --- ФУНКЦИЯ ДЛЯ ПРОКРУТКИ К ЭЛЕМЕНТУ С ЗАДАННЫМ СМЕЩЕНИЕМ ОТ ВЕРХА ---
function scrollToElementWithOffset(elementId, offset = 120, behavior = 'smooth') {
...
async function navigateToHome(hash = '') {
    try {
        ...
        if (newMain) {
            ...
            if (hash) {
                // Используем scrollToElementWithOffset для главной страницы
                setTimeout(() => {
...
function handleNavigation(e) {
    ...
    if (targetUrl.pathname === window.location.pathname && targetUrl.hash) {
        setTimeout(() => {
            ...
            // === ИСПРАВЛЕНО: Для TOC ссылок используем scrollIntoView с центрированием ===
