...
function renderHomePage() {
    ...
    if (heroSection) {
        ...
        const subtitleContainer = heroSection.querySelector('.hero-subtitle-container');
        const heroSubtitleEl = subtitleContainer?.querySelector('p');
        const heroContactsList = heroSection.querySelector('.hero-contact-list');

        // Update hero content with data from Firestore
        if (heroTitleEl) {
            heroTitleEl.classList.add('animate-always');
            heroTitleEl.textContent = home.h1 || 'Web Development & SEO in Tbilisi';
        }
        if (subtitleContainer && heroSubtitleEl) {
            subtitleContainer.classList.add('animate-always');
            heroSubtitleEl.innerHTML = home.subtitle || '';
        }
        if (heroContactsList && Array.isArray(home.contacts) && home.contacts.length > 0) {
            const nextHtml = home.contacts
                .map(c => `<li><a href="${c.href}" target="_blank" rel="noopener noreferrer">${c.label}</a></li>`)
                .join('');
            if (heroContactsList.innerHTML.trim() !== nextHtml) {
                heroContactsList.innerHTML = nextHtml;
            }
        }
    } else {
