...
async function routeAndRender(isPopState = false, hash = '') {
    ...
    // --- ДОБАВИТЬ ЭТОТ БЛОК ---
    if (typeof analyticsSessionRef !== 'undefined' && analyticsSessionRef !== null) {
        let currentPage = window.location.pathname;
        analyticsSessionRef.update({
            pagesVisited: firebase.firestore.FieldValue.arrayUnion(currentPage)
        }).catch(()=>{});
    }
}
...
function getVisitorDeviceData() {
    ...
    if (/bot|googlebot|crawler|spider|robot|crawling|bingbot|yandexbot/i.test(ua)) isBot = true;
...
function getVisitorSource() {
    ...
    let type = "Direct", source = null;
    if (referrer) {
        try {
            const refHost = new URL(referrer).hostname;
            if (refHost.includes(window.location.hostname)) {
                type = "Internal"; 
            } else if (/google\.|yandex\.|bing\.|yahoo\./.test(refHost)) {
                type = "Search"; source = refHost.replace('www.', '');
            } else if (/facebook\.|t\.me|instagram\.|vk\.com|twitter\./.test(refHost)) {
                type = "Social"; source = refHost.replace('www.', '');
            } else {
                type = "Referral"; source = refHost.replace('www.', '');
            }
        } catch(e) {}
    }
    return { type, source };
...
async function startAnalyticsTracker() {
    ...
    const visitorId = localStorage.getItem('visitor_id') || 'vid_' + Date.now().toString(36);
    localStorage.setItem('visitor_id', visitorId);

    const devData = getVisitorDeviceData();
    const sourceData = getVisitorSource();

    if(sourceData.type === "Internal") return;
    ...
    } catch(e) { console.warn("Analytics issue:", e); }
}
