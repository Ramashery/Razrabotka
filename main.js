...
function setupObservers() {
    // Отключаем старые наблюдатели перед переинициализацией
    ...
    // Подключаем наблюдателей ко всем элементам с соответствующими классами,
    // которые уже должны быть в DOM на этот момент.
...
// renderSection теперь используется только для динамически генерируемого контента
// (например, связанных постов)
function renderSection(key, title, items) {
    const section = document.getElementById(key);
    if (!section) return;

    const isMobile = window.matchMedia("(max-width: 767px)").matches;
    const itemsToRender = items || siteData[key] || [];
    ...
    if (isMobile) {
        const mobileSlidersHTML = langOrder.map(lang => {
            ...
            const slidesHTML = langItems.map((item, index) => {
                ...
                return `<a href="${itemUrl}" class="item-card ${index === 0 ? 'active' : ''} animate-on-scroll"><div class="item-card__image" style="background-image: url('${imageUrl}')"></div><div class="item-card__content"><h3>${item.title}</h3><div class="card-subtitle">${item.subtitle}</div><p>${item.description}</p></div></a>`
            }).join('');
        ...
        generatedContentHtml = `<div class="mobile-sliders-container">${mobileSlidersHTML}</div>`;
    } else {
        // Для десктопа в renderSection (например, связанных постов) тоже генерируем сетку.
        // На главной странице используется уже статически сгенерированный item-grid.
        const desktopGridsHTML = langOrder.map(lang => {
            const langItems = itemsByLang[lang];
            if (!langItems || langItems.length === 0) return '';
            const slides = []; // Здесь slides - это просто группы по 3, не слайды карусели
            for (let i = 0; i < langItems.length; i += 3) {
                slides.push(langItems.slice(i, i + 3));
            }
            const cardsHTML = slides.map(slideItems => { 
            ...
            generatedContentHtml = `<div class="desktop-grid-wrapper"><div class="item-grid">${cardsHTML}</div></div>`;
        }).join('');
    }
    // Здесь мы перезаписываем содержимое section (для динамически генерируемых секций)
    section.innerHTML = `<div class="animated-container"><h2>${title}</h2></div>${generatedContentHtml}`;
}


// --- Инициализация переключателя TOC (persistent element) ---
let floatingTocToggleInitialized = false; // Этот флаг нужен, т.к. floating-toc-wrapper находится вне <main> и не перезаписывается
...
function renderDetailPage(collection, slug, lang) {
    ...
    const floatingTocWrapper = document.getElementById('floating-toc-wrapper'); // Глобальный элемент, который не перезаписывается
    ...
    if (!item) {
        ...
        if (floatingTocWrapper) floatingTocWrapper.style.display = 'none'; // Скрыть TOC если страница не найдена
        return;
    }
    renderSeoTags(item);
    ...
    // Применяем классы анимации сразу после вставки HTML для динамического контента
    document.querySelectorAll('.detail-content > .content-group').forEach(el => el.classList.add('floating-item'));

    // Управление глобальным TOC-wrapper'ом
    if (floatingTocWrapper && tocContentPanel && tocToggleBtn) {
        if (tocHtmlContent) {
            ...
            floatingTocWrapper.style.display = 'flex'; // Делаем TOC видимым
            
            // Сброс состояния TOC при загрузке новой страницы, чтобы она всегда была закрыта по умолчанию
            ...
            initFloatingTocToggle(); // Инициализация event listeners (сработает только один раз благодаря guard)
        } else {
            floatingTocWrapper.style.display = 'none'; // Скрываем TOC
            tocContentPanel.innerHTML = ''; // Очищаем содержимое
        }
    } else {
        // Это ветка, если floatingTocWrapper вообще не был найден в DOM, что не должно происходить на detail pages
        console.warn("floatingTocWrapper elements not found on detail page. Check template.html");
        // Но на всякий случай, если он вдруг где-то появился, но без contentPanel/toggleBtn,
        // убедимся, что он скрыт.
        if (floatingTocWrapper) floatingTocWrapper.style.display = 'none'; 
    }

    // renderRelatedPosts теперь вызывается отдельно, поэтому его элементы и классы добавятся
...
function renderRelatedPosts(currentCollection, currentSlug, currentLang) {
    ...
    relatedSection.innerHTML = `<h2 class="animated-container animate-on-scroll">You Might Also Like</h2><div class="item-grid">${itemsHTML}</div>`;
...
async function navigateToHome(hash = '') {
    try {
        ...
        if (newMain) {
            ...
            hydrateHomePageContent(); // Адаптируем статическое содержимое
            if (hash) {
                setTimeout(() => {
        ...
        // Убедимся, что глобальный TOC-wrapper скрыт на главной странице
        const floatingTocWrapper = document.getElementById('floating-toc-wrapper');
...
async function routeAndRender(isPopState = false, hash = '') {
    ...
    // Вызываем setupObservers ПОСЛЕ того, как ВСЁ динамическое содержимое
    // (включая контент страниц и связанные посты) было отрисовано и
    // применены необходимые классы анимации.
    setupObservers(); 
...
// --- HYDRATION LOGIC (СНОВА ИСПРАВЛЕНО ДЛЯ ГЛАВНОЙ СТРАНИЦЫ) ---

function hydrateHomePageContent() {
    // console.log("Entering hydrateHomePageContent (Homepage adaptation)");
    // Убедимся, что siteData.home загружена полностью
    // Если siteData.home пуста, это будет означать проблему с loadData().
    if (!siteData.home || Object.keys(siteData.home).length === 0 || !siteData.home.h1) {
        console.warn("siteData.home not fully loaded for homepage hydration. Some dynamic features may not work or data is missing.");
        // Возможно, здесь стоит добавить fallback или индикатор ошибки.
    }
    
    applyCustomBackground(siteData.home);

    const sectionsToAdapt = ['services', 'portfolio', 'blog', 'contact'];

    sectionsToAdapt.forEach(key => {
        const sectionEl = document.getElementById(key);
        if (sectionEl) {
            const isMobile = window.matchMedia("(max-width: 767px)").matches;
            const existingGrid = sectionEl.querySelector('.item-grid'); // Статическая сетка (десктоп)
            let mobileSlidersContainer = sectionEl.querySelector('.mobile-sliders-container'); // Динамический контейнер (мобильный)
            const h2El = sectionEl.querySelector('.animated-container h2');

            // Убеждаемся, что заголовок H2 анимирован
            if (h2El && !h2El.classList.contains('animate-on-scroll')) {
                h2El.classList.add('animate-on-scroll');
            }

            if (isMobile) {
                // В мобильной версии скрываем статическую сетку (если есть)
                if (existingGrid) {
                    existingGrid.style.display = 'none'; 
                }

                // Создаем или очищаем и наполняем контейнер для мобильных слайдеров
                if (!mobileSlidersContainer) {
                    mobileSlidersContainer = document.createElement('div');
                    mobileSlidersContainer.classList.add('mobile-sliders-container');
                    sectionEl.appendChild(mobileSlidersContainer);
                } else {
                    mobileSlidersContainer.innerHTML = ''; // Очищаем, если уже был
                }
                
                // Генерируем мобильные слайдеры, используя ПОЛНЫЕ данные из siteData
                const itemsToRender = siteData[key] || []; 
                const langOrder = ['en', 'ka', 'uk', 'ru'];
                const itemsByLang = {};
                itemsToRender.forEach(item => {
                    const lang = item.lang || 'en';
                ...
                mobileSlidersContainer.innerHTML = mobileSlidersHTML;

            } else { // Десктопная версия (используем статическую сетку)
                if (existingGrid) {
                    existingGrid.style.display = ''; // Показываем статическую сетку, если она была скрыта
                    // Применяем классы анимации к статическим карточкам
                    existingGrid.querySelectorAll('.item-card').forEach(card => {
                        if (!card.classList.contains('animate-on-scroll')) {
                            card.classList.add('animate-on-scroll');
                        }
                    });
                }
                // Удаляем динамически добавленный мобильный слайдер, если он есть
                if (mobileSlidersContainer) {
                    mobileSlidersContainer.remove();
                }
            }
        }
    });

    initMobileSliders(); // Инициализируем мобильные слайдеры, если они были созданы
    // initDesktopCarousels() не вызываем здесь, так как десктопные слайдеры
    // в home_template.html генерируются статически и не имеют JS-логики карусели.
    // Они просто item-grid.
...
async function hydrateStaticPage() {
    renderMenu();
    // Эти элементы не существуют на home_template.html, но могут быть на template.html
    ...
    try {
        siteData = await loadData(); // Загружаем полные данные для всего сайта
        const path = window.location.pathname;
        const detailPageRegex = /^\/(?:([a-z]{2})\/)?(services|portfolio|blog|contact)\/([a-zA-Z0-9-]+)\/?$/;
        const match = path.match(detailPageRegex);
        if (match) {
            // Если это detail-страница, вызываем renderDetailPage
            const [, lang, collection, slug] = match;
            renderDetailPage(collection, slug, lang || 'en');
        } else {
            // Если это главная страница, гидрируем ее статическое содержимое
            hydrateHomePageContent(); 
        }
    } catch (error) {
        console.error("Background data load failed during hydration:", error);
    }
    // setupObservers будет вызван из routeAndRender после завершения всего рендеринга.
}
...
function initStaticEventListeners() { 
    ...
    if (menuToggle && navOverlay) { 
        menuToggle.addEventListener('click', () => { 
            ...
            navOverlay.classList.toggle('is-active'); // ИСПРАВЛЕНО: .toggle для корректного открытия/закрытия
        }); 
    }
...
async function initApp() { 
    ...
    if (isStaticPage) { 
        await hydrateStaticPage(); // Ждем завершения гидратации статической страницы
    } else { 
        // Этот блок выполняется, если index.html загружается динамически,
        // но наша home_template.html генерируется как index.html,
        // поэтому эта ветка должна быть для page.html и dynamic SPA.
        try { 
            siteData = await loadData(); 
            renderMenu(); 
            await routeAndRender(); // Ждем завершения роутинга
        } catch (error) { 
