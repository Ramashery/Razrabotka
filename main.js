...
// --- ФОРВАРДИНГ УКАЗАТЕЛЯ В IFRAME ФОНА ---
// custom-background-iframe рисуется с z-index: -1, а body/main лежат поверх
// него в stacking order. Из-за этого pointerdown/pointermove/touchstart над
// страницей никогда физически не долетают до документа внутри iframe —
// браузер отдаёт их родительскому body. Поэтому координаты указателя нужно
// явно пересылать через postMessage, а сама анимация внутри srcdoc должна
// слушать "message" (см. соответствующий обработчик в background_html).
function initBackgroundPointerForwarding() {
    const iframe = document.getElementById('custom-background-iframe');
    if (!iframe || iframe.style.display === 'none') return;
    ...
    window.addEventListener('pointerdown', e => forward('pointerdown', e), { passive: true });
    window.addEventListener('pointermove', e => forward('pointermove', e), { passive: true });
    window.addEventListener('pointerup', e => forward('pointerup', e), { passive: true });
    window.addEventListener('pointercancel', e => forward('pointerup', e), { passive: true });
}
...
// =====================================================================
// РЕДИЗАЙН ГЛАВНОЙ СТРАНИЦЫ (#rd-header) — всё ниже активируется только
// если на странице есть соответствующие элементы, поэтому detail-страницы
// (template.html) не затрагиваются.
// =====================================================================

// --- STICKY HEADER: тёмная подложка + бордер после начала скролла ---
function initStickyHeader() {
    const header = document.getElementById('rd-header');
    if (!header) return;
...
// --- MEGA MENU (Services): hover на десктопе, click/tap на touch-устройствах ---
function initMegaMenu() {
    const items = document.querySelectorAll('.rd-nav-item[data-mega]');
    ...
    items.forEach(item => {
        ...
        const scheduleClose = () => { closeTimer = setTimeout(() => item.classList.remove('is-open'), 150); };
        ...
        // Touch: первый тап по стрелке открывает панель вместо перехода по ссылке.
        if (link) {
            link.addEventListener('click', (e) => {
                if (window.matchMedia('(hover: none)').matches && !item.classList.contains('is-open')) {
                    e.preventDefault();
                    document.querySelectorAll('.rd-nav-item.is-open').forEach(i => i !== item && i.classList.remove('is-open'));
                    open();
                }
            });
        }
    ...
    document.addEventListener('click', (e) => {
        if (!e.target.closest('.rd-nav-item[data-mega]')) {
            document.querySelectorAll('.rd-nav-item.is-open').forEach(i => i.classList.remove('is-open'));
        }
    });
}

// --- HERO CANVAS: лёгкий аналог WebGL-частиц из макета (обычный 2D canvas,
// без зависимостей — надёжно работает на GitHub Pages) ---
function initHeroCanvas() {
    const canvas = document.getElementById('rd-hero-canvas');
    if (!canvas) return;
    if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;

    const ctx = canvas.getContext('2d');
    let width, height, particles, rafId;
    const DENSITY = 12000; // px² на одну частицу — регулирует "плотность" эффекта
...
// --- REVEAL ON SCROLL для новых секций (.rd-reveal) ---
...
function initApp() {
    ...
    initBackgroundPointerForwarding();
