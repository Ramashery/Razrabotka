...
async function loadData() {
    ...
    try {
        ...
        const dataPromises = [
            ...
            // Запрашиваем все документы, как и раньше
        ...
        collections.forEach((col, index) => {
            // Собираем все документы, а затем ФИЛЬТРУЕМ их на клиенте,
            // чтобы логика совпадала с Python-скриптом.
            freshSiteData[col] = snapshots[index].docs
                .map(doc => ({ id: doc.id, ...processDocData(doc.data()) }))
                .filter(item => item.status !== 'archived'); // <-- ДОБАВЛЕН ФИЛЬТР
        });
...
// --- ФУНКЦИЯ ДЛЯ СКРОЛЛА К ЭЛЕМЕНТУ С OFFSET ---
function scrollToElementWithOffset(elementId, offset = 120) {
    ...
    // Рассчитываем конечную позицию с offset
...
// === ИСПРАВЛЕНИЕ ПРОБЛЕМЫ №1 (часть 1) ===
// Функция теперь принимает 'hash' для прокрутки после отрисовки
async function navigateToHome(hash = '') {
    try {
        ...
        if (newMain) {
            ...
            // Если был передан хеш, прокручиваем к нему
            if (hash) {
                setTimeout(() => {
                    scrollToElementWithOffset(hash.substring(1), 120);
                }, 100); // Небольшая задержка для гарантии отрисовки
            }
        }
    } catch (error) {
...
// === ИСПРАВЛЕНИЕ ПРОБЛЕМЫ №1 (часть 2) ===
// Функция теперь принимает и передает 'hash'
async function routeAndRender(isPopState = false, hash = '') {
    ...
    if (match) {
        ...
        // Передаем хеш в функцию отрисовки главной страницы
    ...
    // (кроме случаев, когда есть хеш, который будет обработан отдельно)
...
// === ИСПРАВЛЕНИЕ ПРОБЛЕМЫ №1 (часть 3) ===
// Функция теперь передает 'hash' при навигации
function handleNavigation(e) {
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            ...
            // Передаем хеш из целевого URL в роутер
