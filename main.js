...
function applyCustomBackground(item = null) {
    ...
    if (customCode && customCode.trim() !== "") {
        // Если код фона отличается от текущего, обновляем его
        if (iframe.srcdoc !== customCode) {
            iframe.srcdoc = customCode;
        }
        
        // Даем браузеру минимальное время на отрисовку iframe перед запуском анимации
        setTimeout(() => {
            iframe.classList.add('visible');
        }, 50); // 50ms достаточно для плавного старта

    } else {
        // Если у страницы нет фона, плавно его скрываем
