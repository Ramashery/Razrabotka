...
// --- RENDER FUNCTIONS (ОБНОВЛЕННАЯ ЛОГИКА ОПРЕДЕЛЕНИЯ КОНТЕНТА) ---
function formatContentHtml(content) {
    if (!content) return '';

    // ШАГ 1: Обработка тегов <pre>
    ...
    const html_parts = blocks.map(block => {
        ...
        // Регулярные выражения (они уже были корректными)
        ...
        // --- ИСПРАВЛЕНИЕ: ИЗМЕНЕН ПОРЯДОК ПРОВЕРОК ---
        // Сначала проверяем на YouTube-ссылки
        ...
        // Затем на существующие HTML-теги
        else if (/^<(p|div|h[1-6]|ul|ol|li|blockquote|hr|table|pre)/i.test(trimmedBlock)) {
            return trimmedBlock;
        } 
        // И только в конце оборачиваем всё остальное в <p>
        else {
    ...
    // Группировка для DOM Width
    const chunkSize = 15;
    const groupedBlocks = [];
    for (let i = 0; i < html_parts.length; i += chunkSize) {
        const chunk = html_parts.slice(i, i + chunkSize);
        groupedBlocks.push(`<div class="content-group">${chunk.join('')}</div>`);
    }
...
function renderRelatedPosts(currentCollection, currentSlug, currentLang) {
    ...
    const relatedItems = pool.filter(item => item.lang === currentLang && !(item.collection === currentCollection && item.urlSlug === currentSlug)).sort(() => 0.5 - Math.random()).slice(0, 5);
...
async function routeAndRender(isPopState = false) {
    ...
    // Эти элементы уже будут обернуты в .content-group из formatContentHtml,
    // но если вдруг нет, или если есть другие верхнеуровневые элементы в detail-content,
    // то этот код все равно добавит floating-item.
    document.querySelectorAll('.detail-content > .content-group > *').forEach(el => el.classList.add('floating-item'));
    // А здесь, чтобы .content-group сам был анимирован, если его прямые дети не анимированы
    document.querySelectorAll('.detail-content > .content-group').forEach(el => el.classList.add('floating-item'));
...
async function hydrateStaticPage() {
    ...
    // Учитываем, что контент теперь может быть обернут в .content-group
...
function initStaticEventListeners() { 
    ...
    mainContentEl.addEventListener('click', function(event) {
        ...
        if (event.target.closest('#scroll-to-top-btn')) {
            const tocElement = document.querySelector('#toc, .table-of-contents');
            if (tocElement) {
