...
import html
import random
import sys  # Добавлен для управления кодом выхода
...
try:
    ...
    # ВАЖНО: Проверяем наличие шаблона карусели
...
def get_all_data():
    ...
    try:
        ...
        for col in collections:
            ...
            # Добавляем таймаут или проверку, если зависает именно тут
        ...
        # ВАЖНО: Выходим с ошибкой, чтобы GitHub Action остановился здесь
        sys.exit(1)
        ...
        if carousel_match and all_data:
            ...
            if carousel_items_for_group:
                ...
                try:
                    ...
                    print(f"  ! Ошибка рендера карусели {group_key}: {e}")
            else:
...
def generate_home_page(all_data):
    ...
    try:
        ...
        # Проверка данных карусели для отладки
...
def generate_detail_page(item, all_data, alternates):
    ...
    try:
        ...
        # Здесь мы НЕ передаем carousel_html напрямую в render, так как format_content уже встроил его в final_content_html
        # если там был шорткод [CAROUSEL:xxx].
        # Однако, если вы хотите жестко вставить карусель ВНИЗУ страницы (как было в старом коде), нужно проверить логику.
        # В вашем template.html есть переменная carousel_html, но в python коде выше она пустая: carousel_html = ""
        ...
        with open(path, 'w', encoding='utf-8') as f:
            f.write(html_content)
        # print(f"  ✓ Страница: {collection_name}/{slug}") # Скрыл, чтобы не спамить в лог
    except Exception as e:
        print(f"✗ ОШИБКА при рендере страницы {collection_name}/{slug}: {e}")
        # Не выходим, чтобы ошибка одной страницы не рушила весь сайт, но логируем
...
def main():
    print("!!! ЗАПУСК ГЕНЕРАЦИИ САЙТА (ОБНОВЛЕННАЯ ВЕРСИЯ) !!!")
    
    all_data = get_all_data()
    
    if not all_data:
        print("❌ ОШИБКА: Данные не загружены. Прерывание сборки.")
        sys.exit(1) # ВАЖНО: Код ошибки для GitHub Actions
    ...
    # Финальная проверка наличия index.html
