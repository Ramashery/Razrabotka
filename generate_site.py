...
try:
    ...
    llms_template = env.get_template('llms_template.txt') # <-- ДОБАВЛЕНО ДЛЯ LLMS.TXT
...
def static_asset_version(filename):
    """Короткий хэш содержимого файла для cache-busting query-параметра
    (?v=...), чтобы браузер/CDN GitHub Pages не отдавали устаревший
    закэшированный main.js/new-carousel.js после деплоя новой версии."""
    import hashlib
    try:
...
LANG_NAMES = {'en': 'English', 'ka': 'Georgian', 'ru': 'Russian', 'uk': 'Ukrainian'}
...
# Пункты верхнего меню редизайна (десктоп, sticky header). 'Services' —
# мега-меню, построенное из коллекции services (см. build_services_megamenu).
# 'Approach'/'Industries' — простые якоря на статические секции главной
# (пока без отдельных Firestore-коллекций, см. generate_home_page()).
HEADER_NAV_ITEMS = [
    {'num': '01', 'label': 'Approach', 'href': '/#approach', 'megamenu': False},
    {'num': '02', 'label': 'Services', 'href': '/#services', 'megamenu': True},
    {'num': '03', 'label': 'Industries', 'href': '/#industries', 'megamenu': False},
    {'num': '04', 'label': 'Portfolio', 'href': '/#portfolio', 'megamenu': False},
    {'num': '05', 'label': 'Blog', 'href': '/#blog', 'megamenu': False},
]

def render_menu_html():
    """Плоский список для мобильного/офф-канвас меню (.nav-overlay) —
    оставлен без мега-меню намеренно, на мобильном пункт 'Services'
    просто ведёт на секцию #services."""
    return ''.join(f'<li><a href="{i["href"]}">{i["label"]}</a></li>' for i in MENU_ITEMS)

def build_services_megamenu(services, lang='en', limit=6):
    """Строит данные для мега-меню 'Services' в шапке из документов
    коллекции services (только lang='en', т.к. верхнее меню сайта общее
    для всех языковых версий). Как только появится поле 'shortSummary'
    в админ-панели — оно подхватится автоматически."""
    lang_items = [s for s in services if s.get('lang', 'en') == lang]
    picked = lang_items[:limit]
    out = []
    for idx, item in enumerate(picked, start=1):
        out.append({
            'num': f'{idx:02d}',
            'title': item.get('title', ''),
            'sub': item.get('shortSummary') or item.get('subtitle', ''),
            'href': f"/services/{item.get('urlSlug', '')}/",
        })
    return out

def render_desktop_nav_html(services_megamenu):
    """Рендерит десктопное меню шапки (#rd-header) с мега-меню для Services."""
    parts = []
    for nav_item in HEADER_NAV_ITEMS:
        if nav_item['megamenu']:
            mega_items_html = ''.join(
                f'<a href="{mi["href"]}" class="rd-mega-item">'
                f'<span class="rd-mega-num">{mi["num"]}</span>'
                f'<span class="rd-mega-title">{mi["title"]}</span>'
                f'<span class="rd-mega-sub">{mi["sub"]}</span></a>'
                for mi in services_megamenu
            )
            parts.append(
                f'<div class="rd-nav-item" data-mega="services">'
                f'<a href="{nav_item["href"]}" class="rd-nav-link">'
                f'<span class="rd-nav-num">{nav_item["num"]}</span>{nav_item["label"]}</a>'
                f'<div class="rd-mega">{mega_items_html}'
                f'<div class="rd-mega-footer"><a href="/#services">View all services →</a></div>'
                f'</div></div>'
            )
        else:
            parts.append(
                f'<div class="rd-nav-item"><a href="{nav_item["href"]}" class="rd-nav-link">'
                f'<span class="rd-nav-num">{nav_item["num"]}</span>{nav_item["label"]}</a></div>'
            )
    return ''.join(parts)

def render_footer_nav_html():
    """Плоский список ссылок без <li> — для нового <nav> в футере
    (в отличие от render_menu_html(), который рендерит <li> для .nav-menu)."""
    return ''.join(f'<a href="{i["href"]}">{i["label"]}</a>' for i in MENU_ITEMS)

def render_footer_html():
    return f'© {date.today().year} Digital Craft.'

def resolve_background_html(item, home_data):
    """Повторяет логику applyCustomBackground() из main.js: приоритет у фона
    конкретного item'а, иначе используется фон главной страницы (home)."""
    item_bg = (item or {}).get('backgroundHtml') or ''
    home_bg = (home_data or {}).get('backgroundHtml') or ''
    return item_bg or home_bg or ''

def _card_html(item, collection_key, extra_class=''):
    lang_prefix = f"/{item.get('lang')}" if item.get('lang') else ''
    item_url = f"{lang_prefix}/{collection_key}/{item.get('urlSlug', '')}/"
    media = item.get('media') or []
    image_url = next((u for u in media if u and 'youtube' not in u and 'vimeo' not in u), '')
    alt = item.get('mainImageAlt') or item.get('title') or ''
    # 'animate-on-scroll' обязателен: именно по этому классу
    # animateOnceObserver в main.js подхватывает data-bg-src и
    # подставляет фон карточки (см. setupObservers()).
    cls = f'item-card animate-on-scroll {extra_class}'.strip()
    return (
        f'<a href="{item_url}" class="{cls}"><div class="item-card__image" role="img" '
        f'aria-label="{alt}" data-bg-src="{image_url}"></div>'
        f'<div class="item-card__content"><h3>{item.get("title","")}</h3>'
        f'<div class="card-subtitle">{item.get("subtitle","")}</div>'
        f'<p>{item.get("description","")}</p></div></a>'
    )

def render_home_section_html(collection_key, title, items):
    """Python-эквивалент renderSection() из main.js. Рендерит СРАЗУ оба
    варианта разметки (desktop-сетку и mobile-слайдер) для всех языков —
    CSS-медиа-запросы сами показывают нужный (см. styles.css:
    'section:not(#related-posts) > .desktop-grid-wrapper/.mobile-sliders-container')."""
    items_by_lang = {}
    for item in items:
        items_by_lang.setdefault(item.get('lang', 'en'), []).append(item)

    desktop_groups = []
    for lang in SUPPORTED_LANGS:
        lang_items = items_by_lang.get(lang)
        if not lang_items:
            continue
        slides = [lang_items[i:i + 3] for i in range(0, len(lang_items), 3)]
        slides_html = ''.join(
            f'<div class="desktop-grid-slide{" active" if idx == 0 else ""}">'
            f'{"".join(_card_html(it, collection_key) for it in slide_items)}</div>'
            for idx, slide_items in enumerate(slides)
        )
        dots_html = ''
        if len(slides) > 1:
            dots = ''.join(
                f'<span class="desktop-slider-dot{" active" if i == 0 else ""}" data-index="{i}"></span>'
                for i in range(len(slides))
            )
            dots_html = f'<div class="desktop-slider-nav">{dots}</div>'
        desktop_groups.append(
            f'<div class="desktop-language-group"><h4 class="desktop-lang-title">{LANG_NAMES.get(lang, lang)}</h4>'
            f'<div class="desktop-carousel-container">{slides_html}</div>{dots_html}</div>'
        )
    desktop_html = f'<div class="desktop-grid-wrapper">{"".join(desktop_groups)}</div>'
    ...
    for lang in SUPPORTED_LANGS:
        ...
        if not lang_items:
            continue
        slides_html = ''.join(
            _card_html(it, collection_key, 'active' if i == 0 else '')
            for i, it in enumerate(lang_items)
        )
        dots_html = ''
        if len(lang_items) > 1:
            dots = ''.join(
                f'<span class="slider-dot{" active" if i == 0 else ""}" data-index="{i}"></span>'
                for i in range(len(lang_items))
            )
            dots_html = f'<div class="slider-nav">{dots}</div>'
        mobile_blocks.append(
            f'<div class="language-slider-block"><div class="cross-fade-slider">{slides_html}</div>{dots_html}</div>'
        )
    mobile_html = f'<div class="mobile-sliders-container">{"".join(mobile_blocks)}</div>'

    return f'<h2 class="animate-on-scroll is-visible">{title}</h2>{desktop_html}{mobile_html}'
...
def get_approach_context(home_data):
    """Контент секции 'Approach'. Пока статический (нет отдельной
    Firestore-коллекции) — читает необязательные поля из документа
    home/content, если админка их пришлёт, иначе — дефолтный текст-заглушка.
    TODO(редизайн БД): добавить в home/content поля approachKicker,
    approachTitle, approachText, approachPoints (список строк)."""
    default_points = [
        'Discovery & strategy — we learn your market before we design anything',
        'Design & prototyping — interfaces tested with real users, not guesses',
        'Build & launch — production-grade code, shipped on schedule',
        'Support & growth — we stay on after launch to iterate on real data',
    ]
    points = home_data.get('approachPoints') or default_points
    return {
        'kicker': home_data.get('approachKicker', 'Approach'),
        'title': home_data.get('approachTitle', 'A process built for clarity, not ceremony'),
        'text': home_data.get('approachText', (
            'We are a small, senior team based in Tbilisi. No account managers, '
            'no hand-offs — the people who scope your project are the ones who build it. '
            'That keeps timelines honest and communication direct.'
        )),
        'points': [{'n': f'{i+1:02d}', 'text': p} for i, p in enumerate(points)],
    }

def get_industries_context(home_data):
    """Контент секции 'Industries'. Плейсхолдер-данные до появления
    отдельной коллекции в Firestore.
    TODO(редизайн БД): коллекция 'industries' с полями name/meta/order."""
    default_industries = [
        {'name': 'Hospitality & Tourism', 'meta': 'Hotels, tours, booking platforms'},
        {'name': 'Real Estate', 'meta': 'Listings, virtual tours, CRM integrations'},
        {'name': 'Retail & E-commerce', 'meta': 'Storefronts, catalogs, payments'},
        {'name': 'Professional Services', 'meta': 'Law, finance, consulting firms'},
    ]
    industries = home_data.get('industries') or default_industries
    return [
        {'num': f'{i+1:02d}', 'name': ind.get('name', ''), 'meta': ind.get('meta', '')}
        for i, ind in enumerate(industries)
    ]

def get_hero_context(home_data):
    """Поля hero-секции. home.h1/home.subtitle уже существуют и остаются
    источником правды; остальное — новые необязательные поля с дефолтами."""
    return {
        'kicker': home_data.get('heroKicker', 'DIGITAL AGENCY · TBILISI · SINCE 2019'),
        'accent_word': home_data.get('heroAccentWord', ''),
        'cta_primary_label': home_data.get('heroCtaPrimaryLabel', 'See our work'),
        'cta_primary_href': home_data.get('heroCtaPrimaryHref', '/#portfolio'),
        'cta_secondary_label': home_data.get('heroCtaSecondaryLabel', 'Start a project'),
        'cta_secondary_href': home_data.get('heroCtaSecondaryHref', '/#contact'),
    }
...
def generate_home_page(all_data):
    ...
    try:
        ...
        sections_html = {
            'services': render_home_section_html('services', 'Our Services', all_data.get('services', [])),
            'portfolio': render_home_section_html('portfolio', 'Our Portfolio', all_data.get('portfolio', [])),
            'blog': render_home_section_html('blog', 'Our Blog', all_data.get('blog', [])),
            'contact': render_home_section_html('contact', 'Our Contacts', all_data.get('contact', [])),
        }

        services_megamenu = build_services_megamenu(all_data.get('services', []))
        ...
        html_content = home_template.render(
            ...
            desktop_nav_html=render_desktop_nav_html(services_megamenu),
            footer_nav_html=render_footer_nav_html(),
            footer_html=render_footer_html(),
            background_html=resolve_background_html(None, home_data),
            hero=get_hero_context(home_data),
            approach=get_approach_context(home_data),
            industries=get_industries_context(home_data),
            main_js_v=static_asset_version('main.js'),
            carousel_js_v=static_asset_version('new-carousel.js'),
            redesign_css_v=static_asset_version('redesign.css'),
        )
...
def generate_detail_page(item, all_data, alternates):
    ...
    try:
        # --- ЛОГИКА X-DEFAULT ДЛЯ HTML ---
        x_default_item = next((alt for alt in alternates if alt.get('isXDefault') is True), None)
        if not x_default_item:
            x_default_item = next((alt for alt in alternates if alt.get('lang') == 'en'), None)
            
        x_default_url = None
        if x_default_item:
            xd_lang = x_default_item.get('lang')
            xd_col = x_default_item.get('collection_name')
            xd_slug = x_default_item.get('urlSlug')
            if xd_lang and xd_col and xd_slug:
                x_default_url = f"{BASE_URL}/{xd_lang}/{xd_col}/{xd_slug}/"
        # ----------------------------------
        ...
        html_content = detail_template.render(
            ...
            x_default_url=x_default_url, # ПЕРЕДАЕМ В ШАБЛОН
            toc_html=toc_html,
            toc_title=toc_title,
            ...
            menu_html=render_menu_html(),
            footer_html=render_footer_html(),
            background_html=resolve_background_html(item, all_data.get('home', {})),
            main_js_v=static_asset_version('main.js'),
            carousel_js_v=static_asset_version('new-carousel.js'),
        )
...
# <-- ДОБАВЛЕНО: ФУНКЦИЯ ДЛЯ ГЕНЕРАЦИИ LLMS.TXT -->
def generate_llms_txt(all_data):
    print("--- Генерация llms.txt ---")
    try:
        home_data = all_data.get('home', {})
        
        # Вспомогательная функция для группировки статей по переводам
        def group_items(items):
            grouped = {}
            for item in items:
                if item.get('status') == 'archived': 
                    continue
                # Группируем по translationGroupKey. Если его нет — фоллбэк на urlSlug
                key = item.get('translationGroupKey')
                if not key or not str(key).strip():
                    key = item.get('urlSlug', '')
                grouped.setdefault(str(key).strip(), []).append(item)
            return grouped

        # Формируем словарь, где ключи — английские названия разделов, 
        # а значения — сгруппированные данные
        collections = {
            'Services': group_items(all_data.get('services', [])),
            'Portfolio': group_items(all_data.get('portfolio', [])),
            'Blog': group_items(all_data.get('blog', [])),
            'Contact': group_items(all_data.get('contact', []))
        }

        content = llms_template.render(
            home=home_data,
        ...
        output_path = os.path.join(OUTPUT_DIR, 'llms-full.txt')
        with open(output_path, 'w', encoding='utf-8') as f:
            f.write(content)
        print("✓ Файл llms-full.txt успешно сгенерирован (Strict Machine-Readable Format).")
    except Exception as e:
        print(f"✗ ОШИБКА при генерации llms-full.txt: {e}")
# <-- КОНЕЦ ДОБАВЛЕНИЯ -->
...
def copy_static_assets():
    ...
    ignore_list = {
        ...
        'llms_template.txt', 'analytics.html', 'dc_backend.py',# <-- ДОБАВЛЕНО В ИСКЛЮЧЕНИЯ
...
def generate_sitemap_xml(pages_for_sitemap, all_data):
    ...
    try:
        ...
        for group_key, pages_in_group in grouped.items():
            ...
            # 2. Определяем x-default
            x_default_url = None
            
            # 2a. Ищем галочку
            for page_in_group in pages_in_group:
                if page_in_group.get('isXDefault') is True:
                    x_default_url = build_url_for_sitemap(page_in_group)
                    break 
            
            # 2b. Иначе fallback на английский
            if not x_default_url and 'en' in hreflang_map:
                x_default_url = hreflang_map['en'][0]

            # 3. Генерируем URL блок
            for page in pages_in_group:
                ...
                if len(hreflang_map) > 1:
                    if x_default_url:
                        ET.SubElement(url_el, f"{{{XHTML_NS}}}link", rel="alternate", hreflang="x-default", href=x_default_url)
...
def main():
    ...
    # <-- ДОБАВЛЕНО: ВЫЗОВ НОВОЙ ФУНКЦИИ -->
    generate_llms_txt(all_data)
