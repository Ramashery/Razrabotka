...
def generate_detail_page(item, all_data, alternates):
    ...
    try:
        ...
        # Словарь переводов для заголовка кнопки (без лишних слов, только суть)
        toc_titles = {
        ...
        if raw_content and raw_content.strip().startswith('[TOC]'):
            ...
            # Генерируем HTML для оглавления, если найдены заголовки
            if toc_items:
                # ВАЖНО: Мы больше не добавляем заголовок <h2> внутрь списка
                toc_list_html = '<ul>'
            ...
            # Получаем финальный HTML контента с добавленными ID
            final_content_html = "".join([ET.tostring(child, encoding='unicode', method='html') for child in tree])
        else:
            # Если маркера [TOC] нет, работаем как раньше
