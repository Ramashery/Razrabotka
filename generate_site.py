...
# --- ИЗМЕНЕНИЕ: Импортируем lxml ---
from lxml import etree as ET
import firebase_admin
from firebase_admin import credentials, firestore
from jinja2 import Environment, FileSystemLoader

# --- Блок НАСТРОЙКА и функции до sitemap остаются без изменений ---
try:
...
def generate_sitemap_xml(pages_for_sitemap, all_data):
    ...
    for url_loc in home_urls:
        ...
        ET.SubElement(url_el, "loc").text = url_loc
        ...
        ET.SubElement(url_el, f"{{{XHTML_NS}}}link", rel="alternate", hreflang="x-default", href=f"{BASE_URL}/")
        for lang in SUPPORTED_LANGS:
            lang_href = f"{BASE_URL}/{lang}/" if lang != 'en' else f"{BASE_URL}/"
            ET.SubElement(url_el, f"{{{XHTML_NS}}}link", rel="alternate", hreflang=lang, href=lang_href)
    ...
    for group_key, pages_in_group in grouped.items():
        ...
        for page in pages_in_group:
            ...
            ET.SubElement(url_el, "lastmod").text = datetime.fromisoformat(last_mod_str.replace("Z", "+00:00")).strftime('%Y-%m-%d') if isinstance(last_mod_str, str) else date.today().isoformat()
            ...
            ET.SubElement(url_el, "priority").text = page.get('sitemapPriority') or default_config['priority']
    ...
    print(f"✓ Карта сайта успешно сгенерирована с помощью lxml: {output_path}")
