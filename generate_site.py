...
# --- ОБНОВЛЕННАЯ ФУНКЦИЯ ДЛЯ ОБРАБОТКИ КОНТЕНТА ---
def format_content(content_string):
    if not content_string:
        return ""

    # Шаг 1: Найти все блоки <pre><code>...</code></pre> и экранировать их содержимое.
    # Это предотвратит рендеринг HTML-тегов внутри блоков с кодом.
    def escape_code_content(match):
        pre_attributes = match.group(1)
        code_content = match.group(2)
        # Используем стандартную функцию html.escape для безопасного экранирования
        escaped_content = html.escape(code_content)
        return f'<pre{pre_attributes}><code>{escaped_content}</code></pre>'

    # Ищем все вхождения <pre...><code...>...</code></pre> и применяем к ним функцию-замену
    # re.DOTALL нужен, чтобы `.` соответствовал и символу новой строки
    ...
    # Шаг 2: Применяем существующую логику для обработки абзацев и встраиваемого контента
    ...
    for block in blocks:
        ...
        if html_match:
            # Блок уже содержит HTML (включая наши обработанные <pre> блоки), добавляем как есть
            ...
            # Обычный текст, оборачиваем в <p> и заменяем переносы строк на <br>
    ...
    try:
        ...
        for related_item in pool:
            ...
            if (related_item.get('lang') == lang and 
                ...
                related_item.get('collection_name') != collection_name):
...
def build_url_for_sitemap(page):
    ...
    # URL для sitemap и hreflang должен включать язык
        ...
        for page in pages_in_group:
            ...
            if len(hreflang_map) > 1:
                for lang_code, href_url in hreflang_map.items():
                    ET.SubElement(url_el, f"{{{XHTML_NS}}}link", rel="alternate", hreflang=lang_code, href=href_url)
