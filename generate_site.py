...
def copy_static_assets():
    ...
    ignore_list = {
        ...
        'test_sitemap_data.py', 'template.html', 'home_template.html', 
        'firebase.json', 'README.md', '__pycache__', 'index.html', 
        'page.html', 'admin.txt', 'main.txt', 'sitemap.xml'
    }
...
def build_url_for_sitemap(page):
    """Строит правильный URL для страницы"""
    lang = page['lang']
    collection = page['collection_name']
    slug = page.get('urlSlug', '').strip()
    
    # Убираем слэш в конце slug если есть
    if slug.endswith('/'):
        slug = slug[:-1]
    
    # Формируем URL: BASE_URL/lang/collection/slug/
    return f"{BASE_URL}/{lang}/{collection}/{slug}/"

def generate_sitemap_xml(pages_for_sitemap, all_data):
    """Генерирует sitemap.xml по образцу"""
    
    print("\n=== НАЧАЛО ГЕНЕРАЦИИ SITEMAP ===")
    ...
    print(f"Дата изменения главной: {home_lastmod}")
    
    # Создаем набор всех hreflang ссылок для главной
    home_hreflangs = [
        (f"{BASE_URL}/", "x-default"),
        (f"{BASE_URL}/", "en"),
        (f"{BASE_URL}/ka/", "ka"),
        (f"{BASE_URL}/ru/", "ru"),
        (f"{BASE_URL}/ua/", "ua")
    ...
    # Добавляем 4 URL для главной страницы
    home_urls = [f"{BASE_URL}/", f"{BASE_URL}/ka/", f"{BASE_URL}/ru/", f"{BASE_URL}/ua/"]
    ...
    for loc in home_urls:
        ...
        # Добавляем все hreflang ссылки
        for href, hreflang in home_hreflangs:
            ET.SubElement(
                url_el, 
                f"{{{XHTML_NS}}}link",
                rel="alternate",
        ...
        print(f"  ✓ Добавле"Доба# === 2. ВНУТРЕННИЕ СТРАНИЦЫ ===
    print("\n--- Генерация внутренних страниц ---")
    
    # Группируем страницы по translationGroupKey
    grouped = {}
    loners = []
    ...
    for page in pages_for_sitemap:
        ...
        if key:
            ...
            print(f"  Группа '{key}': {page.get('lang')}/{page.get('urlSlug')}")
        else:
            loners.append(page)
            print(f"  Одиночная страница: {page.get('lang')}/{page.get('urlSlug')}")
    
    print(f"\nВсего групп: {len(grouped)}")
    print(f"Одиночных страниц: {len(loners)}")
    
    # Обрабатываем группы
    for group_key, pages_in_group in grouped.items():
        print(f"\n  Обработка группы '{group_key}' ({len(pages_in_group)} страниц):")
        
        # Создаем карту hreflang для этой группы
        hreflang_map = {}
        for p in pages_in_group:
            if p.get('lang') and p.get('urlSlug'):
                url = build_url_for_sitemap(p)
                hreflang_map[p['lang']] = url
                print(f"    - {p['lang']}: {url}")
        
        # Создаем <url> для каждой страницы в группе
        for page in pages_in_group:
            url_el = ET.SubElement(urlset, "url")
            
            # loc
            page_url = build_url_for_sitemap(page)
            ...
            # changefreq
            default_config = SITEMAP_DEFAULTS.get(
                page['collection_name'], 
                {'changefreq': 'monthly', 'priority': '0.6'}
            )
            changefreq = page.get('sitemapChangefreq') or default_config['changefreq']
            ET.SubElement(url_el, "changefreq").text = changefreq
            
            # priority
            priority = page.get('sitemapPriority') or default_config['priority']
            ET.SubElement(url_el, "priority").text = str(priority)
            
            # hreflang ссылки (только если есть переводы)
            if len(hreflang_map) > 1:
                for lang_code, url in sorted(hreflang_map.items()):
                    ET.SubElement(
                        url_el,
                        f"{{{XHTML_NS}}}link",
                        rel="alternate",
            ...
            print(f"      ✓ Добавлен URL: {page_url}")
    
    # Обрабатываем одиночные страницы (без translationGroupKey)
