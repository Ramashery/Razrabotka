...
import html  # <--- ДОБАВЛЕНО ЭТОТ ИМПОРТ
...
def format_content(content_string):
    ...
    # Python-версия декодировщика HTML
    ...
    for block in blocks:
        ...
        # Универсальная проверка на наличие любого тега
        is_already_html = re.match(r"^<[a-zA-Z1-6\/!]", trimmed_block, re.IGNORECASE)
        # Универсальная проверка на наличие любого ЭКРАНИРОВАННОГО тега
        is_escaped_html = re.match(r"^&lt;[a-zA-Z1-6\?\/!]", trimmed_block, re.IGNORECASE)
        
        youtube_match = re.match(youtube_regex, trimmed_block)
        image_match = re.match(image_regex, trimmed_block)

        if is_already_html:
            # Если это уже обычный HTML, оставляем как есть
            html_parts.append(trimmed_block)
        elif is_escaped_html:
            # Если это экранированный HTML, декодируем его
            html_parts.append(decode_html_entities(trimmed_block))
        elif youtube_match:
