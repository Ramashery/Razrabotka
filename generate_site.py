...
try:
    service_account_info = json.loads(os.environ.get('FIREBASE_SERVICE_ACCOUNT_RAZRABOTKA'))
...
def generate_home_and_copy_assets(all_data):
    ...
    try:
        ...
        ) # Можно добавить больше замен для meta-тегов, если нужно
    ...
    for item_name in os.listdir('.'):
        # Исключаем системные папки, скрипты сборки и уже обработанные файлы
