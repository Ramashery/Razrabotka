    ...
    try:
        ...
        for col in collections:
            ...
            for doc in docs:
                ...
                # === ПРОВЕРКА СТАТУСА (ARCHIVED) ===
                # Если в админке нажали "Archive", статус будет 'archived'.
                # Мы пропускаем этот документ: он не попадет в HTML, sitemap и перелинковку.
...
def generate_detail_page(item, all_data, alternates):
    ...
    try:
        ...
        # Используем random.sample для равномерного распределения ссылок по сайту (защита от сирот)
        if len(candidates) > 3:
...
# --- Основная функция ---
def main():
    print("!!! ВЫПОЛНЯЕТСЯ ОБНОВЛЕННАЯ ВЕРСИЯ generate_site.py (ARCHIVING + RANDOM LINKS) !!!")
