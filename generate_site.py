...
# --- ИЗМЕНЕНИЕ 1: Функция теперь принимает 'alternates' ---
def generate_detail_page(item, all_data, alternates):
    ...
    try:
        ...
        # --- ИЗМЕНЕНИЕ 2: 'alternates' передается в шаблон ---
...
def main():
    ...
    # --- ИЗМЕНЕНИЕ 3: Создаем карту переводов перед циклом ---
        ...
        if collection in all_data:
            for item in all_data[collection]:
                if item.get('urlSlug') and item.get('lang'):
                    # --- ИЗМЕНЕНИЕ 4: Находим альтернативные версии и передаем в функцию ---
