...
HOME_JSON_FIELDS = {
    ...
    'designWordsJson': 'designWords',
    'empowerParagraphsJson': 'empowerParagraphs',
    'trustPillarsJson': 'trustPillars',
    'harmonyGalleryJson': 'harmonyGallery',
    'testimonialsJson': 'testimonials',
    'footerSocialJson': 'footerSocial',
}

def parse_home_rebrand_fields(home_data):
    """Раскодирует JSON-строки, которые хранит админка для повторяющихся
    блоков ребренда (awards, achievements, testimonials и т.д.), в обычные
    Python-списки/словари, с которыми удобно работать в Jinja2-шаблоне.
    Если поле отсутствует или содержит битый JSON — подставляется пустой список,
    чтобы шаблон (через {% if %}) просто не рендерил соответствующую секцию."""
    if not home_data:
        return home_data
    result = dict(home_data)
    for json_field, target_field in HOME_JSON_FIELDS.items():
        raw = result.get(json_field)
        if isinstance(raw, str) and raw.strip():
            try:
                result[target_field] = json.loads(raw)
            except (ValueError, TypeError):
                print(f"  ⚠ Предупреждение: не удалось распарсить home.{json_field} — секция будет скрыта.")
                result[target_field] = []
...
def resolve_hero_animation_html(home_data):
    """Анимация для секции hero — задаётся только на главной странице."""
    return (home_data or {}).get('heroAnimationHtml') or ''

def resolve_menu_animation_html(home_data):
    """Анимация для выдвигающейся панели меню — общая для всего сайта."""
    return (home_data or {}).get('menuAnimationHtml') or ''

def resolve_footer_animation_html(home_data):
    """Анимация для футера — используется только на главной странице
    (на детальных страницах футер скрыт, поэтому туда не передаём)."""
    return (home_data or {}).get('footerAnimationHtml') or ''
...
def generate_home_page(all_data):
    ...
    try:
        home_data = parse_home_rebrand_fields(all_data.get('home'))
        sections_data = {
        ...
        html_content = home_template.render(
            ...
            hero_animation_html=resolve_hero_animation_html(home_data),
            menu_animation_html=resolve_menu_animation_html(home_data),
            footer_animation_html=resolve_footer_animation_html(home_data),
            ...
            home_css_v=static_asset_version('home.css'),
            home_js_v=static_asset_version('home.js'),
...
def generate_detail_page(item, all_data, alternates):
    ...
    try:
        ...
        html_content = detail_template.render(
            ...
            menu_animation_html=resolve_menu_animation_html(all_data.get('home', {})),
            main_js_v=static_asset_version('main.js'),
            carousel_js_v=static_asset_version('new-carousel.js'),
            styles_css_v=static_asset_version('styles.css'),
        )
