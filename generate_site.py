...
def format_content(content_string, all_data=None, lang='en'):
    ...
    for block in blocks:
        ...
        # Обновленный regex для тегов (включая img, чтобы не оборачивать их в p)
        html_tag_regex = r"^\s*<(p|div|h[1-6]|ul|ol|li|blockquote|hr|table|pre|img)"
        ...
        if carousel_match and all_data:
            ...
            # Используем новый класс article-image и loading=lazy
            img_html = f'<img src="{trimmed_block}" loading="lazy" class="article-image" alt="Embedded content" onload="this.classList.add(\'is-loaded\')" />'
    ...
    # ПОСТОБРАБОТКА: Находим все теги img и добавляем loading="lazy" и класс анимации, если их нет.
    # Это позволяет обрабатывать картинки, вставленные вручную через HTML.
    def add_lazy_attrs(match):
        ...
        # Добавляем класс анимации и обработчик onload
        if 'class=' not in img_tag:
            img_tag = img_tag.replace('<img', '<img class="article-image" onload="this.classList.add(\'is-loaded\')"', 1)
        elif 'article-image' not in img_tag:
            img_tag = img_tag.replace('class="', 'class="article-image ', 1)
            # Если класс добавлен к существующему, нужно убедиться, что есть onload для запуска анимации
            if 'onload=' not in img_tag:
                img_tag = img_tag.replace('<img', '<img onload="this.classList.add(\'is-loaded\')"', 1)
                
        return img_tag

    final_content = re.sub(r'<img[^>]+>', add_lazy_attrs, final_content, flags=re.IGNORECASE)
