print("!!! ЗАПУЩЕНА ДИАГНОСТИЧЕСКАЯ ВЕРСИЯ generate_site.py !!!")
import os
import json
import re
import shutil
...
# --- Функции для получения данных и рендеринга страниц (без изменений) ---
def get_all_data():
    site_data = {}
    try:
...
def generate_sitemap_xml(pages_for_sitemap, all_data):
    ...
    # === ДИАГНОСТИЧЕСКИЙ ВЫВОД РЕЗУЛЬТАТОВ ГРУППИРОВКИ ===
    print("\n--- РЕЗУЛЬТАТ ГРУППИРОВКИ СТРАНИЦ ---")
    print(f"Найдено {len(grouped)} групп переводов:")
    for key, pages in grouped.items():
        print(f"  - Группа '{key}':")
        for page in pages:
            print(f"    - lang: {page.get('lang', 'N/A')}, slug: {page.get('urlSlug', 'N/A')}")
    
    print(f"\nНайдено {len(loners)} страниц без группы ('одиночки'):")
    for page in loners:
        print(f"  - lang: {page.get('lang', 'N/A')}, slug: {page.get('urlSlug', 'N/A')}, collection: {page.get('collection_name', 'N/A')}")
    print("--- КОНЕЦ ДИАГНОСТИКИ ГРУППИРОВКИ ---\n")
    # =======================================================
    ...
    # === Генерация главной страницы (без изменений) ===
    ...
    # === Генерация сгруппированных страниц (без изменений) ===
    ...
    # === Генерация страниц-одиночек (без изменений) ===
    for page in loners:
        url_el = ET.SubElement(urlset, "url")
        ET.SubElement(url_el, "loc").text = build_url_for_sitemap(page)
        last_mod_str = page.get('lastModified')
...
def main():
    ...
    # --- ДИАГНОСТИЧЕСКИЙ БЛОК 1: Проверка загруженных данных ---
    print("\n--- ПРОВЕРКА ЗАГРУЖЕННЫХ ДАННЫХ ---")
    for collection in collections_to_generate:
        count = len(all_data.get(collection, []))
        print(f"Коллекция '{collection}': загружено {count} документов.")
    print("-------------------------------------\n")
    # =======================================================
    ...
    # --- ДИАГНОСТИЧЕСКИЙ БЛОК 2: Вывод всех страниц, идущих в sitemap ---
    ...
    if valid_pages_for_sitemap:
        for page in valid_pages_for_sitemap:
            ...
            print(f"lang: {page.get('lang', 'N/A')}, slug: {page.get('urlSlug', 'N/A')}, group_key: '{key}'")
    else:
        print("!!! Ни одной валидной страницы для sitemap не найдено !!!")
    print("-------------------------------------\n")
    # =================================================================
