...
def format_content(content_string):
    ...
    # Шаг 2: Применяем остальную логику для абзацев и медиа
...
def generate_detail_page(item, all_data, alternates):
    ...
    try:
        # === ОБНОВЛЕННАЯ ЛОГИКА ДЛЯ ПЕРЕЛИНКОВКИ (YOU MIGHT ALSO LIKE) ===
        pool = all_data.get('services', []) + all_data.get('blog', []) + all_data.get('portfolio', [])
        
        # Фильтруем пул: 
        # 1. Строго тот же язык (lang == lang)
        # 2. Исключаем текущую страницу (urlSlug != slug)
        # 3. Элемент должен иметь urlSlug
        candidates = [
        ...
        # Используем random.sample для случайного выбора, чтобы избежать Orphan Pages.
        # Ссылки будут меняться при каждой пересборке сайта.
        if len(candidates) > 3:
            related_items = random.sample(candidates, 3)
        else:
        ...
        for page in pages_in_group:
            ...
            if last_mod_str:
                try:
                    ...
                    print(f"    [ПРЕДУПРЕЖДЕНИЕ] Не удалось разобрать дату 'lastModified' для {loc} ({last_mod_str}).")
            ...
            if len(hreflang_map) > 1:
                # Добавляем x-default, если есть английская версия
                ...
                # Добавляем остальные языковые версии
...
# --- Основная функция ---
def main():
    print("!!! ВЫПОЛНЯЕТСЯ ОБНОВЛЕННАЯ ВЕРСИЯ generate_site.py С RANDOM LINKING !!!")
