# diagnostic_generate_site.py (версия, которая создает файл-отчет)
import os
import json
import re
import shutil
...
# --- НАСТРОЙКА ПАПКИ И ФАЙЛА ДЛЯ ОТЧЕТА ---
OUTPUT_DIR = 'public'
os.makedirs(OUTPUT_DIR, exist_ok=True)
LOG_FILENAME = os.path.join(OUTPUT_DIR, 'diagnostic_log.txt')

# --- НАСТРОЙКА СИСТЕМЫ ЛОГГИРОВАНИЯ ---
# Все print() теперь будут записываться в файл diagnostic_log.txt
logging.basicConfig(
    level=logging.INFO,
    format='%(message)s',
    handlers=[
        logging.FileHandler(LOG_FILENAME, mode='w', encoding='utf-8'), # Запись в файл
        logging.StreamHandler(sys.stdout) # Вывод в консоль (для системы сборки)
    ]
)
# --- КОНЕЦ НАСТРОЙКИ ---

logging.info("!!! ЗАПУЩЕНА ДИАГНОСТИЧЕСКАЯ ВЕРСИЯ (создает файл-отчет) !!!")
...
try:
    ...
    db = firestore.client()
    logging.info("Подключение к Firebase успешно.")
...
if os.path.exists(OUTPUT_DIR):
    # Не удаляем всю папку, чтобы не удалить наш лог-файл до его создания
    for item in os.listdir(OUTPUT_DIR):
        if item != 'diagnostic_log.txt':
...
def get_all_data():
    ...
    try:
        ...
        logging.info("Все данные из Firestore успешно загружены.")
        return site_data
    except Exception as e: logging.error(f"Критическая ОШИБКА при загрузке данных: {e}"); return None
...
def generate_home_page(all_data):
    try:
        ...
        logging.info("✓ Главная страница и структура папок для языков успешно сгенерированы.")
    except Exception as e: logging.error(f"[ERROR] Ошибка при генерации главной страницы: {e}")
...
def generate_detail_page(item, all_data):
    ...
    except Exception as e: logging.error(f"[ERROR] Ошибка при рендере страницы {collection_name}/{slug}: {e}")
...
def copy_static_assets():
    ...
    for item_name in os.listdir('.'):
        if item_name not in ignore_list:
            ...
            except Exception as e: logging.error(f"Не удалось скопировать {item_name}: {e}")
    logging.info("Копирование ассетов завершено.")
...
def generate_sitemap_xml(pages_for_sitemap, all_data):
    logging.info("\n--- Запущена ДИАГНОСТИЧЕСКАЯ версия функции generate_sitemap_xml ---")
    grouped = {}
    loners = []
    ...
    logging.info("\n--- РЕЗУЛЬТАТ ГРУППИРОВКИ СТРАНИЦ ---")
    logging.info(f"Найдено {len(grouped)} групп переводов:")
    for key, pages in grouped.items():
        logging.info(f"  - Группа '{key}':")
        for page in pages:
            logging.info(f"    - lang: {page.get('lang', 'N/A')}, slug: {page.get('urlSlug', 'N/A')}")
    
    logging.info(f"\nНайдено {len(loners)} страниц без группы ('одиночки'):")
    for page in loners:
        logging.info(f"  - lang: {page.get('lang', 'N/A')}, slug: {page.get('urlSlug', 'N/A')}, collection: {page.get('collection_name', 'N/A')}")
    logging.info("--- КОНЕЦ ДИАГНОСТИКИ ГРУППИРОВКИ ---\n")
    ...
    logging.info(f"✓ Sitemap успешно сгенерирован: {output_path}")
...
def main():
    ...
    for collection in collections_to_generate:
        count = len(all_data.get(collection, []))
        logging.info(f"Коллекция '{collection}': загружено {count} документов.")
    logging.info("-------------------------------------\n")

    logging.info("Начинаю генерацию страниц...")
    ...
    for collection in collections_to_generate:
        if collection in all_data:
            for item in all_data[collection]:
                ...
                else: logging.warning(f"[WARNING] Пропущен элемент в '{collection}' (ID: {item.get('id', 'N/A')}) из-за отсутствия 'urlSlug' или 'lang'.")
    
    logging.info("\nГенерация страниц завершена. Начинаю копирование ассетов...")
    copy_static_assets()
    
    logging.info("\n--- СПИСОК СТРАНИЦ ДЛЯ SITEMAP ---")
    if valid_pages_for_sitemap:
        for page in valid_pages_for_sitemap:
            key = page.get('translationGroupKey', '!!! НЕ УКАЗАН !!!')
            logging.info(f"lang: {page.get('lang', 'N/A')}, slug: {page.get('urlSlug', 'N/A')}, group_key: '{key}'")
    else:
        logging.warning("!!! Ни одной валидной страницы для sitemap не найдено !!!")
    logging.info("-------------------------------------\n")

    if valid_pages_for_sitemap:
        generate_sitemap_xml(valid_pages_for_sitemap, all_data)
    else: logging.warning("! Не найдено валидных страниц для создания sitemap.xml.")
...
if __name__ == '__main__':
    try:
        ...
        logging.error("\n!!! КРИТИЧЕСКАЯ ОШИБКА ВО ВРЕМЯ ВЫПОЛНЕНИЯ main() !!!")
        logging.error(f"Тип ошибки: {type(e).__name__}")
        logging.error(f"Сообщение: {e}")
        import traceback
