...
# --- НОВАЯ ФУНКЦИЯ: Создание облегченной версии данных ---
def create_lean_preview(items):
    """
    Создает облегченную версию списка элементов для главной страницы.
    Удаляет тяжелый mainContent и лишние SEO-теги.
    """
    previews = []
    for item in items:
        # Безопасное получение медиа
        media_list = item.get('media', [])
        first_image = media_list[0] if isinstance(media_list, list) and len(media_list) > 0 else ''
        
        # Обрезаем описание, если оно вдруг гигантское (на всякий случай)
        ...
        preview_item = {
            ...
            # Передаем только первую картинку, чтобы не грузить лишнее
            'media': [first_image], 
            # ВАЖНО: Мы НЕ передаем 'mainContent', 'schemaJsonLd' и прочее
        }
        previews.append(preview_item)
    return previews

# --- ФУНКЦИЯ ОБРАБОТКИ КОНТЕНТА (ОБНОВЛЕННАЯ) ---
def format_content(content_string):
    ...
    for block in blocks:
        ...
        # ИСПРАВЛЕНИЕ 1: Строгая регулярка для YouTube (добавлено ^ и .*?$)
        ...
        # Используем re.match, который проверяет с самого начала строки
        ...
        # ИСПРАВЛЕНИЕ 3: ИЗМЕНЕН ПОРЯДОК ПРОВЕРОК: Сначала специфичные элементы, затем общие
        if youtube_match:
            video_id = youtube_match.group(1)
            # ИСПРАВЛЕНИЕ 2: Корректный SRC для iframe
    ...
    # Группировка для DOM Width (chunk_size = 15)
    chunk_size = 15
    grouped_html = []
    for i in range(0, len(html_parts), chunk_size):
        chunk = html_parts[i:i + chunk_size]
        # Оборачиваем каждую группу в div.content-group
        grouped_html.append(f'<div class="content-group">{"".join(chunk)}</div>')
...
def generate_home_page(all_data):
    try:
        ...
        # --- ИСПРАВЛЕНИЕ: Используем create_lean_preview для уменьшения размера HTML ---
        sections_data = {
        ...
        print("✓ Главная страница (основная) и языковые папки успешно сгенерированы (ОПТИМИЗИРОВАНО).")
    except Exception as e:
...
# --- Основная функция ---
def main():
    print("!!! ВЫПОЛНЯЕТСЯ ОБНОВЛЕННАЯ (ОПТИМИЗИРОВАННАЯ) ВЕРСИЯ generate_site.py !!!")
