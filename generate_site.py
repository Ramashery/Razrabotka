...
# --- Функции до sitemap (без изменений) ---
def get_all_data():
    site_data = {}
    try:
...
def generate_home_page(all_data):
    try:
        ...
        for lang in SUPPORTED_LANGS:
            if lang != 'en': os.makedirs(os.path.join(OUTPUT_DIR, lang), exist_ok=True)
...
def copy_static_assets():
    print("\nНачинаю копирование ассетов..."); ignore_list = {'.git', '.github', OUTPUT_DIR, 'generate_site.py', 'test_sitemap_data.py', 'template.html', 'home_template.html', 'firebase.json', 'README.md', '__pycache__', 'index.html', 'page.html', 'admin.txt', 'main.txt'}
...
def build_url_for_sitemap(page):
    lang_prefix = f"/{page['lang']}"; slug = page.get('urlSlug', '')
    if not slug.endswith('/'): slug += '/'
    return f"{BASE_URL}{lang_prefix}/{page['collection_name']}/{slug}"

# --- ОТЛАДОЧНАЯ ВЕРСИЯ ФУНКЦИИ SITEMAP ---
def generate_sitemap_xml(pages_for_sitemap, all_data):
    try:
        print("\n--- [SITEMAP DEBUG] Начинаю генерацию sitemap.xml ---")
        XHTML_NAMESPACE = "http://www.w3.org/1999/xhtml"
        # register_namespace('xhtml', XHTML_NAMESPACE) # Попытка регистрации префикса

        urlset = Element('urlset', xmlns="http://www.sitemaps.org/schemas/sitemap/0.9")
        urlset.set('xmlns:xhtml', XHTML_NAMESPACE)
        print("[SITEMAP DEBUG] Корневой элемент <urlset> создан.")

        # --- Блок главной страницы ---
        # ... (этот блок скорее всего работает, оставляем без изменений)
        home_data = all_data.get('home', {}); home_lastmod_iso = home_data.get('lastModified')
        ...
        home_alternates = [{'rel': 'alternate', 'hreflang': 'x-default', 'href': f"{BASE_URL}/"}, {'rel': 'alternate', 'hreflang': 'en', 'href': f"{BASE_URL}/"}]
        for lang in SUPPORTED_LANGS:
            if lang != 'en': home_alternates.append({'rel': 'alternate', 'hreflang': lang, 'href': f"{BASE_URL}/{lang}/"})
        home_urls = [f"{BASE_URL}/"] + [f"{BASE_URL}/{lang}/" for lang in SUPPORTED_LANGS if lang != 'en']
        for url_loc in home_urls:
            url_el = SubElement(urlset, 'url')
            SubElement(url_el, 'loc').text = url_loc; SubElement(url_el, 'lastmod').text = home_lastmod
            SubElement(url_el, 'changefreq').text = SITEMAP_DEFAULTS['home']['changefreq']; SubElement(url_el, 'priority').text = SITEMAP_DEFAULTS['home']['priority']
            for alt in home_alternates:
                SubElement(url_el, f'{{{XHTML_NAMESPACE}}}link', **alt)
        print("[SITEMAP DEBUG] Блоки для главной страницы добавлены.")

        # --- Блок внутренних страниц ---
        grouped = {}; loners = []
        for page in pages_for_sitemap:
            key = page.get('translationGroupKey')
            if key and key.strip(): grouped.setdefault(key.strip(), []).append(page)
            else: loners.append(page)
        print(f"[SITEMAP DEBUG] Страницы разделены: {len(grouped)} групп, {len(loners)} одиночных.")
        ...
        for group_key, pages_in_group in grouped.items():
            ...
            for page in pages_in_group:
                ...
                SubElement(url_el, 'lastmod').text = datetime.fromisoformat(last_mod_str.replace("Z", "+00:00")).strftime('%Y-%m-%d') if isinstance(last_mod_str, str) else date.today().isoformat()
                ...
                if len(hreflang_map) > 1:
                    for lang_code, url in hreflang_map.items():
                        SubElement(url_el, f'{{{XHTML_NAMESPACE}}}link', rel="alternate", hreflang=lang_code, href=url)

        print("[SITEMAP DEBUG] Блоки для сгруппированных страниц добавлены.")
        
        # ... (обработка одиночных страниц)
        for page in loners:
            url_el = SubElement(urlset, 'url')
            SubElement(url_el, 'loc').text = build_url_for_sitemap(page)
            last_mod_str = page.get('lastModified', date.today().isoformat())
            SubElement(url_el, 'lastmod').text = datetime.fromisoformat(last_mod_str.replace("Z", "+00:00")).strftime('%Y-%м-%d') if isinstance(last_mod_str, str) else date.today().isoformat()
            default_config = SITEMAP_DEFAULTS.get(page['collection_name'], {'changefreq': 'monthly', 'priority': '0.6'})
            SubElement(url_el, 'changefreq').text = page.get('sitemapChangefreq') or default_config['changefreq']
            SubElement(url_el, 'priority').text = page.get('sitemapPriority') or default_config['priority']
        print("[SITEMAP DEBUG] Блоки для одиночных страниц добавлены.")

        print("[SITEMAP DEBUG] XML-дерево создано. Преобразую в строку...")
        xml_string = tostring(urlset, 'utf-8', method='xml')
        print("[SITEMAP DEBUG] Строка создана. Форматирую (pretty-printing)...")
        pretty_xml = minidom.parseString(xml_string).toprettyxml(indent="    ")
        print("[SITEMAP DEBUG] Форматирование завершено. Записываю в файл...")
        ...
        print("\n❌❌❌ КРИТИЧЕСКАЯ ОШИБКА ВНУТРИ generate_sitemap_xml ❌❌❌")
        print(f"ОШИБКА: {e}")
        import traceback
        traceback.print_exc()
        print("❌❌❌ SITEMAP НЕ СГЕНЕРИРОВАН ❌❌❌")


def main():
    # ... (main функция без изменений)
