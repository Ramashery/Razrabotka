...
# --- НАСТРОЙКА ---
# (весь блок НАСТРОЙКА остается без изменений)
# Firebase
try:
...
# --- ФУНКЦИИ ---
# (Функции get_all_data, format_content, generate_home_page, generate_detail_page, copy_static_assets, build_url_for_sitemap - остаются без изменений)
def get_all_data():
    site_data = {}
    try:
...
def copy_static_assets():
    ...
    ignore_list = {'.git', '.github', OUTPUT_DIR, 'generate_site.py', 'test_sitemap_data.py', 'template.html', 'home_template.html', 'firebase.json', 'README.md', '__pycache__', 'index.html', 'page.html', 'admin.txt', 'main.txt'}
...
# --- ИЗМЕНЕННАЯ ФУНКЦИЯ SITEMAP (только для этого теста) ---
def generate_sitemap_xml(pages_for_sitemap, all_data):
    print("\n!!! ЗАПУЩЕНА ВЕРСИЯ SITEMAP ДЛЯ ТЕСТА !!!")
    print("!!! В sitemap попадут только страницы с заполненным 'translationGroupKey' !!!")
    ...
    # --- 1. ГЕНЕРАЦИЯ БЛОКОВ ДЛЯ ГЛАВНОЙ СТРАНИЦЫ (остается как есть) ---
    home_alternates = []
    ...
    # Группируем ТОЛЬКО страницы, у которых есть translationGroupKey
    grouped = {}
    pages_with_key = [p for p in pages_for_sitemap if p.get('translationGroupKey')]
    
    print(f"\nНайдено {len(pages_with_key)} страниц с ключом для sitemap из {len(pages_for_sitemap)} всего.")

    for page in pages_with_key:
