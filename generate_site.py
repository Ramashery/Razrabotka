...
def generate_home_page(all_data):
    try:
        ...
        for lang in SUPPORTED_LANGS:
            # Создаем папку для языка, если она нужна (например /en/, /ru/)
...
# --- Функция построения URL для внутренних страниц ---
...
# ==============================================================================
# === ФИНАЛЬНАЯ, ПОЛНОСТЬЮ ИСПРАВЛЕННАЯ ФУНКЦИЯ ГЕНЕРАЦИИ SITEMAP ===
# ==============================================================================
def generate_sitemap_xml(pages_for_sitemap, all_data):
    # Определяем пространства имен для XML
    ...
    # === ШАГ 1: Обработка ГЛАВНОЙ СТРАНИЦЫ (с префиксом /en/) ===
    ...
    # Создаем общий список всех языковых версий для главной страницы
    all_home_hreflangs = [
    ...
    for loc in home_urls:
        ...
        # Добавляем ПОЛНЫЙ набор ссылок в КАЖДЫЙ блок
        for hreflang_attrs in all_home_hreflangs:
            ET.SubElement(url_el, f"{{{XHTML_NS}}}link", **hreflang_attrs)

    # === ШАГ 2: Группировка ВНУТРЕННИХ СТРАНИЦ по translationGroupKey ===
    grouped = {}
    loners = [] # Для страниц без группы
    ...
    for group_key, pages_in_group in grouped.items():
        ...
        for page in pages_in_group:
            ...
            # Если в группе больше одной страницы, добавляем ссылки на альтернативные версии
    ...
    # === ШАГ 4: Обработка страниц-одиночек (без группы) ===
    for page in loners:
        url_el = ET.SubElement(urlset, "url")
        ET.SubElement(url_el, "loc").text = build_url_for_sitemap(page)
    ...
    with open(output_path, 'wb') as f:
        f.write(xml_string)
    print(f"✓ Sitemap успешно сгенерирован по новому образцу: {output_path}")
