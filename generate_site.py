...
def build_url_for_sitemap(page):
    ...
    # Для внутренних страниц - с завершающим слэшем
...
def generate_sitemap_xml(pages_for_sitemap, all_data):
    ...
    print(f"   Добавлено {len(home_urls)} главных страниц")
    
    # === 2. ВНУТРЕННИЕ СТРАНИЦЫ ===
    print(f"2. Обрабатываю {len(pages_for_sitemap)} внутренних страниц...")
    ...
    # Группируем по translationGroupKey - ИСПРАВЛЕННАЯ ЛОГИКА
    grouped = {}
    loners = []
    ...
    for group_key, pages_in_group in grouped.items():
        ...
        print(f"   Обработка группы '{group_key}'...")
    ...
    print(f"3. Добавляю {len(loners)} отдельных страниц...")
    for page in loners:
        url_el = ET.SubElement(urlset, "url")
    ...
    print(f"Группированных страниц: {total_grouped}")
    print(f"Отдельных страниц: {len(loners)}")
    print(f"Главных страниц: {len(home_urls)}")
    
    # Проверяем содержимое файла
    try:
        with open(output_path, 'r', encoding='utf-8') as f:
            content = f.read()
            # Ищем пример группировки
            if 'web-development' in str(pages_for_sitemap):
                print(f"✓ В данных есть группа 'web-development'")
            # Ищем hreflang
            if 'hreflang="en"' in content:
                print(f"✓ Найдены hreflang ссылки в sitemap")
    except:
        pass
...
def main():
    ...
    for page in valid_pages_for_sitemap:
        key = page.get('translationGroupKey')
        if key:
            groups_found.setdefault(key, []).append(page)
    
    for key, pages in groups_found.items():
        print(f"  Группа '{key}': {len(pages)} страниц")
        for page in pages[:2]:  # Покажем первые 2 страницы каждой группы
