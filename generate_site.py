...
try:
    ...
    llms_template = env.get_template('llms_template.txt') # <-- ДОБАВЛЕНО ДЛЯ LLMS
...
def generate_llms_txt(all_data):
    ...
    try:
        ...
        collections = {
            'Услуги (Services)': [item for item in all_data.get('services', []) if item.get('status') != 'archived'],
            'Портфолио (Portfolio)': [item for item in all_data.get('portfolio', []) if item.get('status') != 'archived'],
            'Блог (Blog)': [item for item in all_data.get('blog', []) if item.get('status') != 'archived'],
            'Контакты (Contact)': [item for item in all_data.get('contact', []) if item.get('status') != 'archived']
        }
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            f.write(content)
        print("✓ Файл llms.txt успешно сгенерирован.")
    except Exception as e:
        print(f"✗ ОШИБКА при генерации llms.txt: {e}")
# <-- КОНЕЦ ДОБАВЛЕНИЯ -->
...
def copy_static_assets():
    ...
    ignore_list = {
        ...
        'llms_template.txt', # <-- ДОБАВЛЕНО ДЛЯ LLMS
