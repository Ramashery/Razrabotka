    ...
    try:
        ...
        for col in collections:
            ...
            for doc in docs:
                ...
                # === ОТЛАДОЧНЫЙ БЛОК ДЛЯ ДИАГНОСТИКИ ===
                print(f"\n--- DEBUG: Проверяю документ '{doc.id}' в коллекции '{col}' ---")
                if 'schemaJsonLd' in doc_data and doc_data['schemaJsonLd']:
                    schema_data = doc_data['schemaJsonLd']
                    print(f"  [+] Поле 'schemaJsonLd' НАЙДЕНО.")
                    print(f"  [+] Тип данных: {type(schema_data)}")
                    # Если данные пришли в виде строки, пытаемся их распарсить
                    if isinstance(schema_data, str):
                        print("  [!] Данные пришли как СТРОКА. Пытаюсь преобразовать в ОБЪЕКТ...")
                        try:
                            doc_data['schemaJsonLd'] = json.loads(schema_data)
                            print("  [+] Преобразование УСПЕШНО.")
                        except json.JSONDecodeError:
                            print("  [X] ОШИБКА: Не удалось преобразовать строку. Невалидный JSON.")
                            doc_data['schemaJsonLd'] = None
                else:
                    print(f"  [-] Поле 'schemaJsonLd' НЕ НАЙДЕНО или ПУСТОЕ.")
                # ============================================
                
                site_data[col].append(doc_data)
        
        print("\nВсе данные из Firestore успешно загружены и обработаны.")
        return site_data
    except Exception as e:
...
def generate_detail_page(item, all_data):
    ...
    html_content = template.render(
        page_type='detail', item=item, related_items=related_items,
        site_data=all_data, format_content=format_content
    )
...
def generate_home_and_copy_assets(all_data):
    ...
    try:
        ...
        print("! ВНИМАНИЕ: Файл 'index (1).html' не найден.")
    ...
    for item_name in os.listdir('.'):
        if item_name not in ['.git', '.github', 'public', 'generate_site.py', 'template.html', 'index (1).html', 'firebase.json', 'README.md']:
            ...
            try:
                ...
                elif os.path.isdir(source_path): shutil.copytree(source_path, dest_path)
