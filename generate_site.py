...
# --- ИСПРАВЛЕННАЯ ФУНКЦИЯ ПОСТРОЕНИЯ URL ДЛЯ SITEMAP ---
def build_url_for_sitemap(page):
    """Строит URL для sitemap в соответствии с ожидаемой структурой."""
    ...
    # Для внутренних страниц - БЕЗ завершающего слэша (как в примере)
    return f"{BASE_URL}/{lang}/{collection_name}/{slug}"

# --- ФИНАЛЬНАЯ ФУНКЦИЯ ГЕНЕРАЦИИ SITEMAP (ИСПРАВЛЕННАЯ) ---
def generate_sitemap_xml(pages_for_sitemap, all_data):
    ...
    # Список всех языков главных страниц
    home_pages = [
    ...
    # Для каждой языковой версии главной страницы
    for home_page in home_pages:
        ...
        # Добавляем все hreflang для главных страниц
    ...
    for page in pages_for_sitemap:
        if page.get('collection_name') == 'home':
            continue  # Пропускаем главные страницы
    ...
    # Обрабатываем сгруппированные страницы (с translationGroupKey)
    for group_key, pages_in_group in grouped.items():
        ...
        for page in pages_in_group:
            ...
            # Changefreq и Priority - используем значения из документа или умолчания
            changefreq = page.get('sitemapChangefreq') or SITEMAP_DEFAULTS.get(page['collection_name'], {}).get('changefreq', 'monthly')
            priority = str(page.get('sitemapPriority') or SITEMAP_DEFAULTS.get(page['collection_name'], {}).get('priority', '0.6'))
    ...
    # === ОТДЕЛЬНЫЕ СТРАНИЦЫ (без translationGroupKey) ===
    for page in loners:
        url_el = ET.SubElement(urlset, "url")
...
def main():
    ...
    if valid_pages_for_sitemap:
        # Добавляем главную страницу в список для sitemap
        home_page_data = {
            'collection_name': 'home',
            'lang': 'en',
            'urlSlug': '',
            'lastModified': all_data.get('home', {}).get('lastModified')
        }
        valid_pages_for_sitemap.append(home_page_data)
