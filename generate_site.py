...
from lxml import etree as ET  # Используем надежную библиотеку lxml
...
# --- ИСПРАВЛЕННАЯ ФУНКЦИЯ ГЕНЕРАЦИИ SITEMAP ---
def generate_sitemap_xml(pages_for_sitemap, all_data):
    """
    Генерирует sitemap.xml с поддержкой hreflang для языковых версий.
    """
    # Пространства имен для sitemap и xhtml
    SITEMAP_NS = "http://www.sitemaps.org/schemas/sitemap/0.9"
    XHTML_NS = "http://www.w3.org/1999/xhtml"
    NSMAP = {None: SITEMAP_NS, "xhtml": XHTML_NS}
    
    # Создаем корневой элемент <urlset> с указанием пространств имен
    urlset = ET.Element("urlset", nsmap=NSMAP)

    # --- 1. ОБРАБОТКА ГЛАВНОЙ СТРАНИЦЫ (ОСОБЫЙ СЛУЧАЙ) ---
    home_data = all_data.get('home', {})
    # Пытаемся получить дату модификации, иначе берем сегодняшнюю
    ...
    # Создаем общий набор hreflang ссылок для всех версий главной страницы
    all_home_hreflangs = [
        {"rel": "alternate", "hreflang": "x-default", "href": f"{BASE_URL}/"},
        {"rel": "alternate", "hreflang": "en", "href": f"{BASE_URL}/"}
    ]
    for lang in ['ka', 'ru', 'ua']:
        all_home_hreflangs.append({"rel": "alternate", "hreflang": lang, "href": f"{BASE_URL}/{lang}/"})
        
    # Добавляем URL для каждой языковой версии главной страницы (/, /ka/, /ru/, /ua/)
    home_urls = [f"{BASE_URL}/"] + [f"{BASE_URL}/{lang}/" for lang in ['ka', 'ru', 'ua']]
    for loc in home_urls:
        ...
        # Добавляем полный набор hreflang ссылок к каждой версии
        for hreflang_attrs in all_home_hreflangs:
            ET.SubElement(url_el, f"{{{XHTML_NS}}}link", **hreflang_attrs)

    # --- 2. ОБРАБОТКА ВНУТРЕННИХ СТРАНИЦ ---
    grouped = {}
    loners = [] # Страницы без группы перевода

    # Группируем страницы по ключу перевода
    for page in pages_for_sitemap:
        # Предполагаем, что поле называется 'translationGroupKey'
    ...
    for group_key, pages_in_group in grouped.items():
        # Создаем карту ссылок для hreflang (lang -> url)
        ...
        for page in pages_in_group:
            ...
            # Добавляем hreflang теги, если в группе больше одной страницы
    ...
    # Обрабатываем страницы-одиночки (без группы перевода)
    for page in loners:
        url_el = ET.SubElement(urlset, "url")
        ET.SubElement(url_el, "loc").text = build_url_for_sitemap(page)
    ...
    # --- 3. ЗАПИСЬ В ФАЙЛ ---
    # Преобразуем дерево XML в строку с форматированием
