...
carousel_template = env.get_template('carousel_template.html')
...
# Карта транслитерации для грузинского (Mkhedruli) в латиницу
...
def format_content(content_string, all_data=None, lang='en'):
    ...
    for block in blocks:
        ...
        # Проверяем на наличие тега карусели: [CAROUSEL:group-key]
        ...
        if carousel_match and all_data:
            ...
            # Фильтруем элементы карусели по ключу и языку
            ...
            if carousel_items_for_group:
                # Генерируем уникальный ID для этой карусели
                carousel_id = f"mzaCarousel-{group_key}-{random.randint(1000, 9999)}"
                # Рендерим HTML карусели
                ...
                # Если слайдов нет, ничего не выводим (или можно вывести комментарий HTML)
    ...
    # Группировка обычного контента, но исключение каруселей из <div class="content-group">
    grouped_html = []
    GROUP_SIZE = 3
    temp_group = []
    
    for part in html_parts:
        # Если это карусель (определяем по классу или ID), не добавляем ее в группу p-тегов
        if '<div class="mzaCarousel"' in part or '<section id="carousel-section-' in part:
            if temp_group:
...
def generate_detail_page(item, all_data, alternates):
    ...
    try:
        ...
        if raw_content and raw_content.strip().startswith('[TOC]'):
            ...
            # NEW: Передаем all_data и lang
            final_content_html = format_content(raw_content, all_data, lang)
        
        # --- NEW: Удалена логика автоматического добавления ВСЕХ слайдов в конец страницы. 
        # Теперь карусели добавляются только через тег [CAROUSEL:key] в контенте.
        # Если вы хотите добавить дефолтную карусель в конец, используйте тег [CAROUSEL:default] в тексте статьи.
        carousel_html = "" 
...
def copy_static_assets():
    ...
    ignore_list = {
        ...
        'template.html', 'home_template.html', '404_template.html', 'carousel_template.html',
