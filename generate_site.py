...
def format_content(content_string, all_data=None, lang='en', placeholders_dict=None):
    ...
    for block in blocks:
        ...
        if carousel_match and all_data:
            ...
            if carousel_items_for_group:
                ...
                # --- Генерация HTML карусели с оригинальным дизайном ---
                slides_html = []
                ...
                # ВАЖНО: Если передан placeholders_dict, значит мы готовимся к парсингу через lxml (TOC).
                # lxml ломает структуру <a><div></div></a>. Подменяем карусель на безопасный плейсхолдер.
                if placeholders_dict is not None:
                    ph_id = f"CAROUSEL_PLACEHOLDER_{random.randint(10000, 99999)}"
                    ph_html = f'<div id="{ph_id}" class="carousel-container"></div>'
                    placeholders_dict[ph_id] = carousel_html
                    html_parts.append(ph_html)
                else:
    ...
    for part in html_parts:
        # Условие class="carousel-container" сработает и для обычного кода, и для нашего плейсхолдера
...
def generate_detail_page(item, all_data, alternates):
    ...
    try:
        ...
        if raw_content and raw_content.strip().startswith('[TOC]'):
            ...
            placeholders_dict = {}
            content_html = format_content(content_without_toc_marker, all_data, lang, placeholders_dict)
            ...
            try:
                ...
                # ВАЖНО: Восстанавливаем карусели из плейсхолдеров обратно в готовую HTML строку
                for ph_id, original_carousel_html in placeholders_dict.items():
                    # Безопасная замена через регулярное выражение, чтобы избежать проблем с порядком атрибутов
                    pattern = re.compile(rf'<div[^>]*id="{ph_id}"[^>]*>\s*</div>')
                    final_content_html = pattern.sub(lambda m: original_carousel_html, final_content_html)
