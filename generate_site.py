...
def format_content(content_string):
    ...
    # ИСПРАВЛЕННАЯ СТРОКА: Используем метод .format() вместо f-строки, чтобы избежать ошибки
...
def generate_home_and_copy_assets(all_data):
    ...
    # Шаг 1: Генерация главной страницы
    home_path = os.path.join(OUTPUT_DIR, 'index.html')
    
    # Используем ваш оригинальный index (1).html как основу для главной
    try:
    ...
    # Шаг 2: Копирование всех остальных файлов и папок
    for item_name in os.listdir('.'):
        # Не копируем системные папки, скрипты и сгенерированную папку
        if item_name not in ['.git', '.github', 'public', 'generate_site.py', 'template.html', 'index (1).html', 'deploy.yml.txt', 'main.txt']:
...
def main():
    ...
    # Генерация главной страницы и копирование всех ассетов
