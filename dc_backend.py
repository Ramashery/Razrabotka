"""
dc_backend.py — Digital Craft Tbilisi Analytics Backend v3.2
=============================================================
Эндпоинты:
  GET  /robots.txt              — запрещает индексацию бэкенда поисковиками
...
Новое в v3.2:
  - Эндпоинт /robots.txt — запрещает Googlebot индексировать бэкенд.
    Без этого GSC фиксировал ошибку "Googlebot заблокирован robots.txt"
    при каждом сканировании страниц сайта, которые обращаются к /geo.
...
# ═══════════════════════════════════════════════════════════════════════════════
#  ROBOTS.TXT — ЗАЩИТА ОТ ИНДЕКСАЦИИ БЭКЕНДА
# ═══════════════════════════════════════════════════════════════════════════════

@app.route('/robots.txt')
def robots_txt():
    """
    Запрещает поисковикам индексировать бэкенд.

    Проблема: Googlebot при сканировании страниц сайта видел XHR-запросы
    к razrabotka-2.onrender.com/geo и пытался получить его robots.txt.
    Render не возвращал этот файл → GSC фиксировал ошибку
    "Googlebot заблокирован файлом robots.txt" для каждой страницы сайта
    → страницы помечались как "просканировано, но не проиндексировано".

    Решение: отдаём robots.txt, который явно запрещает индексацию бэкенда.
    Это не влияет на работу /geo — браузеры robots.txt не читают,
    только поисковые боты. А боты теперь не запускают dc-tracker.js
    (защита через navigator.userAgent в dc-tracker.js v3.2).
    """
    content = (
        "User-agent: *\n"
        "Disallow: /\n"
